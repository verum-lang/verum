...
| **Returned-Reference Contract** | `docs/architecture/returned-reference-contract.md` — what a caller's register holds when a function returns `&T`: three producers (`RefField` 0x0C, `StructFieldAddr` FFI 0x4F, `RefListElement` 0x0B), two conventions (ADDRESS vs pre-loaded VALUE), and the fact that the descriptor erases `&` so only the callee's BODY can say which. Root of T1188/T1192/T1197; the `Call`-vs-`CallM` call-site trap. |
