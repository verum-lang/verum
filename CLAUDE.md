...
| **Returned-Reference Contract** | `docs/architecture/returned-reference-contract.md` — what a caller's register holds when a function returns `&T`: four producers (`RefField` 0x0C, `StructFieldAddr` FFI 0x4F, `RefListElement` 0x0B, `DerefValue` 0x1E), two conventions (ADDRESS vs pre-loaded VALUE), and the fact that the descriptor erases `&` so only the callee's BODY can say which. Root of T1188/T1192/T1197; the `Call`-vs-`CallM` call-site trap. |
| **Meta-function registry** | `crates/verum_ast/src/meta_fn.rs` — the ONE roster of `@name(...)` in expression position: each name's arity, its per-position argument expectation, and whether a lowering EXISTS. Four judges read it (parser, attribute validation, type checker, VBC codegen); before T1124 each held a private list and they disagreed — 31 names admitted with no lowering answered `nil`. A name with `Status::Unimplemented` is REFUSED at the call, never typed. Gate: `scripts/ci/check_meta_function_names.py`. |
