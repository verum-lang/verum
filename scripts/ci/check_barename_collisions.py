...
def collect() -> dict[tuple[str, int], set[str]]:
    """(name, arity) -> set of module paths declaring it."""
    ...
    for path in CORE.rglob("*.vr"):
        ...
        for line in text.splitlines():
            m = DECL.match(line)
            if m:
                found[(m.group(1), arity(m.group(2)))].add(rel)
    return found
...
def main() -> int:
    ...
    if not args.check:
        for (name, ar), modules in sorted(coll.items()):
            print(f"{name}/{ar}")
    ...
    if count > baseline:
        ...
        for (name, ar), modules in sorted(coll.items())[:10]:
            print(f"  {name}/{ar}: {', '.join(sorted(modules))}", file=sys.stderr)
        return 1
