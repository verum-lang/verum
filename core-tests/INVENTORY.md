...
| `async/future`           | ~150|  ~150|  ~150|     0| 14 — FutureExt blanket-impl-derived methods (block/map/and_then) not dispatched onto ReadyFuture (task #11). Task #21 closed 2026-05-12 via `register_function_authoritative` + cross-pollination guard + qualified-key path-doubling fix in archive_ctx_loader; `mount core.async.future.{join, select, join3}` now binds path-qualified at the call site. **partial**. |
