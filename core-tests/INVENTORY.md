...
| `collections/list`       | 437 | 187 | 121 |  98 | 6 (List.from / List.of / List.from_elem cross-module UndefinedFunction; runtime memory-layout drift on retain / fill / resize / sort_by). 49 unit + 11 property + 8 integration all pass interp+AOT after 4 new intercepts landed in this branch: contains-needle CBGR-ref auto-deref, List.set, List.get_or, List.swap_remove, List.truncate. **partial**. |
...
| `async/panic_fence`      |  100|   65|   62|   46| 2 — task #11 (`Maybe.take()` mutation through `&mut self` does not flow back to a generic record field, gates the fence's "inner=None after Ready" lifecycle invariant); task #10 (AOT). 12 interpreter tests green: panic_safe factory + record-literal construction (Some/None inner) + Ready(Ok) Int/Text round-trip + fenced-future list consumption summing 15. 1 `@ignore` regression pin for task #11. Panic-arm coverage deferred. **partial**. |
