...
| `async/future`           | ~150|  ~150|  ~150|     0| 4 — Task #11 closed 2026-05-12 via blanket-impl pre-pass + generic-param materialisation skip in vbc/codegen — 6 newly-passing regression tests for `block`/`map`/`and_then`/`lazy.invokes` on flat receivers (ReadyFuture/PendingFuture/Lazy/MapFuture/AndThenFuture). Task #21 closed earlier this day. Remaining: 4 tests pinned as task #24 (`.block()` on Join2/Join3/Select2 combinator receivers — separate dispatch defect surfaces only when receiver is a generic combinator wrapping inner Futures). **partial**. |
