...
impl<'ctx> ExecutionEngine<'ctx> {
    ...
    /// Runs an LLVM function with the given arguments. Unsafe because the function
    /// may perform arbitrary operations; the caller must ensure safety of the JIT'd code.
    ...
    pub unsafe fn run_function_as_main(&self, function: FunctionValue<'ctx>, args: &[&str]) -> c_int {
        ...
        let environment_variables = []; // envp is empty; LLVM requires null-terminated array
