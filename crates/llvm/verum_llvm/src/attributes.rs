...
impl Attribute {
    ...
    pub fn get_enum_kind_id(self) -> u32 {
        assert!(self.get_enum_kind_id_is_valid()); // Precondition: would be enforced by Rust subtypes if available
    ...
    pub fn get_enum_value(self) -> u64 {
        assert!(self.is_enum()); // Precondition: would be enforced by Rust subtypes if available
    ...
    /// always returns a valid pointer for string attributes, so no null check is needed.
    pub fn get_string_kind_id(&self) -> &CStr {
        assert!(self.is_string()); // Precondition: would be enforced by Rust subtypes if available
    ...
    pub fn get_type_value(&self) -> AnyTypeEnum<'_> {
        assert!(self.is_type()); // Precondition: would be enforced by Rust subtypes if available
