...
use verum_llvm_sys::analysis::{LLVMVerifierFailureAction, LLVMVerifyModule};
...
use verum_llvm_sys::core::{LLVMAddModuleFlag, LLVMGetModuleFlag};
use verum_llvm_sys::debuginfo::{LLVMGetModuleDebugMetadataVersion, LLVMStripModuleDebugInfo};
...
pub enum Linkage {
    ...
    ExternalWeak,
    /// Deprecated LLVM linkage type. Retained for C API compatibility.
    ...
    LinkOnceAny,
    /// Deprecated LLVM linkage type. Similar to LinkOnceODR but with automatic hidden visibility.
...
impl<'ctx> Module<'ctx> {
    ...
    pub fn create_mcjit_execution_engine_with_memory_manager(
        ...
        unsafe {
            ...
            verum_llvm_sys::execution_engine::LLVMInitializeMCJITCompilerOptions(
                ...
                std::mem::size_of::<verum_llvm_sys::execution_engine::LLVMMCJITCompilerOptions>(),
            );
        }
        ...
        let mut err_string = MaybeUninit::uninit();
        let code = unsafe {
            verum_llvm_sys::execution_engine::LLVMCreateMCJITCompilerForModule(
                execution_engine.as_mut_ptr(),
                self.module.get(),
                &mut options,
                std::mem::size_of::<verum_llvm_sys::execution_engine::LLVMMCJITCompilerOptions>(),
    ...
    fn get_borrowed_data_layout(module: LLVMModuleRef) -> DataLayout {
        let data_layout = unsafe {
            use verum_llvm_sys::core::LLVMGetDataLayoutStr;
    ...
    pub fn parse_bitcode_from_buffer(
        ...
        if success != 0 {
            return Err(LLVMString::create_from_str("Failed to parse bitcode\0"));
        }
    ...
    pub fn get_source_file_name(&self) -> &CStr {
        use verum_llvm_sys::core::LLVMGetSourceFileName;
    ...
    pub fn set_source_file_name(&self, file_name: &str) {
        use verum_llvm_sys::core::LLVMSetSourceFileName;
    ...
    pub fn link_in_module(&self, other: Self) -> Result<(), LLVMString> {
        ...
        use libc::c_void;
        use verum_llvm_sys::linker::LLVMLinkModules2;
    ...
    pub fn get_or_insert_comdat(&self, name: &str) -> Comdat {
        use verum_llvm_sys::comdat::LLVMGetOrInsertComdat;
