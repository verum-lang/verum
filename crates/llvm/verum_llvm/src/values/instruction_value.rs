...
impl<'ctx> InstructionValue<'ctx> {
    ...
    pub fn get_tail_call_kind(self) -> Option<super::LLVMTailCallKind> {
        if self.get_opcode() == InstructionOpcode::Call {
            unsafe { verum_llvm_sys::core::LLVMGetTailCallKind(self.as_value_ref()) }.into()
        } else {
            None
        }
    }
    ...
    pub fn get_fast_math_flags(self) -> Result<FastMathFlags, InstructionValueError> {
        if self.can_use_fast_math_flags() {
            let raw = unsafe { verum_llvm_sys::core::LLVMGetFastMathFlags(self.as_value_ref()) };
    ...
    pub fn get_non_negative_flag(self) -> Option<bool> {
        (self.get_opcode() == InstructionOpcode::ZExt)
            .then(|| unsafe { verum_llvm_sys::core::LLVMGetNNeg(self.as_value_ref()) == 1 })
    }
    ...
    pub fn set_non_negative_flag(self, flag: bool) {
        if self.get_opcode() == InstructionOpcode::ZExt {
            unsafe { verum_llvm_sys::core::LLVMSetNNeg(self.as_value_ref(), flag as i32) };
        }
    }
    ...
    pub fn get_disjoint_flag(self) -> Option<bool> {
        (self.get_opcode() == InstructionOpcode::Or)
            .then(|| unsafe { verum_llvm_sys::core::LLVMGetIsDisjoint(self.as_value_ref()) == 1 })
    }
    ...
    pub fn set_disjoint_flag(self, flag: bool) {
        if self.get_opcode() == InstructionOpcode::Or {
            unsafe { verum_llvm_sys::core::LLVMSetIsDisjoint(self.as_value_ref(), flag as i32) };
        }
    }
...
bitflags::bitflags! {
    ...
    pub struct FastMathFlags: u32 {
        ...
        const AllowReassoc = verum_llvm_sys::LLVMFastMathAllowReassoc;
        /// Arguments and results are assumed not-NaN.
        const NoNaNs = verum_llvm_sys::LLVMFastMathNoNaNs;
        /// Arguments and results are assumed not-infinite.
        const NoInfs = verum_llvm_sys::LLVMFastMathNoInfs;
        /// Can ignore the sign of zero.
        const NoSignedZeros = verum_llvm_sys::LLVMFastMathNoSignedZeros;
        /// Can use reciprocal multiply instead of division.
        const AllowReciprocal = verum_llvm_sys::LLVMFastMathAllowReciprocal;
        /// Can be floating-point contracted (FMA).
        const AllowContract = verum_llvm_sys::LLVMFastMathAllowContract;
        /// Allows approximations of math library functions or intrinsics
        const ApproxFunc = verum_llvm_sys::LLVMFastMathApproxFunc;
    }
}
