...
impl<'ctx> GlobalValue<'ctx> {
    ...
    /// Sets thread-local storage. Passing `true` is equivalent to setting `GeneralDynamicTLSModel`.
    ...
    pub fn get_comdat(self) -> Option<Comdat> {
        use verum_llvm_sys::comdat::LLVMGetComdat;
    ...
    pub fn set_unnamed_address(self, address: UnnamedAddress) {
        use verum_llvm_sys::core::LLVMSetUnnamedAddress;
