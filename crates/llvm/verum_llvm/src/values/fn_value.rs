...
impl<'ctx> FunctionValue<'ctx> {
    ...
    /// Verifies the function. Returns `true` if verification succeeds.
    /// LLVM's C API returns 0 for success, 1 for error.
    ...
    /// Deletes this function from its module. Unsafe because any remaining references
    /// to this function value become dangling (use-after-free cannot be prevented by types).
    ...
    /// The personality function determines how exceptions are caught and propagated.
    pub fn has_personality_function(self) -> bool {
        use verum_llvm_sys::core::LLVMHasPersonalityFn;
