...
use verum_llvm_sys::core::LLVMCreateTypeAttribute;

use verum_llvm_sys::core::LLVMBFloatTypeInContext;
use verum_llvm_sys::core::LLVMGetInlineAsm;

use verum_llvm_sys::core::LLVMGetTypeByName2;
use verum_llvm_sys::core::LLVMMetadataTypeInContext;
...
impl ContextImpl {
    ...
    /// Returns a 128-bit integer type. Uses `custom_width_int_type(128)` which is
    /// equivalent to `LLVMInt128TypeInContext` available since LLVM 11.
    ...
    fn metadata_string<'ctx>(&self, string: &str) -> MetadataValue<'ctx> {
        ...
        unsafe {
            let md = LLVMMDStringInContext2(self.0, c_string.as_ptr(), c_string.to_bytes().len());
            MetadataValue::new(LLVMMetadataAsValue(self.0, md))
        }
    }
