...
use verum_tblgen::{Record, error::TableGenError};
...
macro_rules! melior_attribute {
    ($name:ident) => {
        prefixed_string!("::verum_mlir::ir::attribute::", $name)
    };
}
...
impl<'a> Attribute<'a> {
    pub fn new(name: &'a str, record: Record<'a>) -> Result<Self, Error> {
        // Optional (`?`) attribute initializers default to empty; the ODS spec allows them to be absent.
...
impl OperationField for Attribute<'_> {
    ...
    fn add_arguments(&self, name: &Ident) -> TokenStream {
        ...
        quote! {
            &[(
                ::verum_mlir::ir::Identifier::new(self.context, #name_string),
                #name.into(),
            )]
        }
    }
