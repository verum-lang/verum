...
/// A trait for block-like types. Combines read and write operations; a split into
/// `BlockLike` (read) and `BlockLikeMut` (write) would improve safety but requires
/// reworking the MLIR C API bindings' lifetime model.
pub trait BlockLike<'c, 'a>: Display + 'a {
    ...
    /// Returns a parent region. The returned reference borrows from the block's
    /// lifetime, though ideally the region lifetime would be tracked separately.
    ...
    /// Inserts an operation at the given position. Takes `&self` rather than
    /// `&mut self` because MLIR's C API operates on raw pointers; mutation
    /// safety is maintained through operation ownership transfer.
