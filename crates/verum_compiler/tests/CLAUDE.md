...
| `stdlib_method_dispatch_correctness.rs` | wrong-method dispatch | Methods with the same simple name across stdlib types (`unwrap` on Result/Maybe/Poll, `is_some` / `map` / `expect` etc.) resolve to the receiver's actual type, not the first-registered candidate.  Pins method-dispatch narrowing's receiver-narrowing safety net in `compile_method_call`.  Each contract is a small `@expected-panic` fixture under `vcs/specs/L0-critical/_codegen_regressions/`. |
