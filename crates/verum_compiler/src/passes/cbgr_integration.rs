...
mod tests {
    ...
    fn make_test_function(name: &str) -> MirFunction {
        use crate::phases::mir_lowering::MirSignature;
    ...
    fn make_function_with_refs(name: &str, num_refs: usize) -> MirFunction {
        use crate::phases::mir_lowering::MirSignature;

        let mut locals = List::new();
        for i in 0..num_refs {
            locals.push(MirLocal {
                id: LocalId(i),
                name: Text::from(format!("ref_{}", i)),
                ty: MirType::Ref {
                    inner: Box::new(MirType::Int),
                    mutable: false,
                    layout: ReferenceLayout::ThinRef,
                },
                kind: LocalKind::Var,
                ssa_version: 0,
                needs_drop: false,
