                            ...
                            "order" => {
                                if let ExprKind::Literal(lit) = &right.kind {
                                    if let verum_ast::LiteralKind::Int(n) = &lit.kind {
                                        let parsed = n.value as u32;
                                        config.order = parsed;
                                        // Surface the inert-defense gap around
                                        // `DifferentiableConfig.order`: the
                                        // attribute parses values > 1 but the
                                        // VJP / JVP generator paths only emit
                                        // first-order derivatives. Higher-order
                                        // autodiff would require recursively
                                        // applying VJP to the generated VJP,
                                        // which is not realised at this layer.
                                        // Embedders see the warning and can
                                        // either drop the order argument or
                                        // hand-write nested @differentiable
                                        // declarations to compose orders
                                        // explicitly.
                                        if parsed > 1 {
                                            tracing::warn!(
                                                "@differentiable(order = {}) is parsed but only \
                                                 first-order derivatives are generated by the \
                                                 VJP/JVP paths; higher-order autodiff requires \
                                                 nested @differentiable declarations",
                                                parsed
                                            );
                                        }
                                    }
