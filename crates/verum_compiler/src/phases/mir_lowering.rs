...
impl LoweringContext {
    ...
    /// Convert expression to place (for assignment targets) - immutable version
    ///
    /// Use this for simple cases where the expression is just a variable, field access,
    /// or simple index. For complex index expressions that need evaluation,
    /// use `expr_to_place_mut` instead.
    ///
    /// This method does not modify the MIR builder state, making it suitable for
    /// analysis passes that need to query place information without side effects.
    #[allow(dead_code)] // Available for analysis passes and simple expression handling
    fn expr_to_place(&self, expr: &Expr) -> Result<Place, Diagnostic> {
        match &expr.kind {
            ExprKind::Path(path) => {
                if let Some(ident) = path.as_ident() {
                    if let Some(local_id) = self.lookup_var(ident.as_str()) {
                        return Ok(Place::local(local_id));
                    }
                }
                Err(DiagnosticBuilder::new(Severity::Error)
            ...
            ExprKind::Field { expr: obj, field } => {
                let base_place = self.expr_to_place(obj)?;
                // Resolve field index using type registry lookup.
                // The resolve_field_index method queries the type cache for the local's type,
                // then looks up the field name in the struct definition to get its index.
                let field_idx = self.resolve_field_index(base_place.local, field.name.as_str());
                Ok(base_place.with_field(field_idx))
            }
            ExprKind::Index { expr: array, index } => {
                let base_place = self.expr_to_place(array)?;
                // For immutable version, only handle simple variable indices
                match &index.kind {
                    ExprKind::Path(path) => {
                        if let Some(ident) = path.as_ident() {
                            if let Some(index_local) = self.lookup_var(ident.as_str()) {
                                return Ok(base_place.with_index(index_local));
                            }
                        }
                        Err(DiagnosticBuilder::new(Severity::Error)
                            .message("complex index expression requires evaluation - use expr_to_place_mut")
                            .span_label(self.span_to_diag(index.span), "use expr_to_place_mut for complex indices")
                            .build())
                    }
                    _ => Err(DiagnosticBuilder::new(Severity::Error)
                        .message(
                            "complex index expression requires evaluation - use expr_to_place_mut",
                        )
                        .span_label(
                            self.span_to_diag(index.span),
                            "use expr_to_place_mut for complex indices",
                        )
                        .build()),
                }
            }
            ExprKind::Unary {
                ...
                let base_place = self.expr_to_place(inner)?;
                Ok(base_place.with_deref())
            }
            _ => Err(DiagnosticBuilder::new(Severity::Error)
                .message("invalid assignment target")
                .build()),
