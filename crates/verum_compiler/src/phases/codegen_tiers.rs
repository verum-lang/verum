...
impl CodegenTiersPhase {
    ...
    /// Generate code for Tier 1: Baseline JIT
    ///
    /// Uses ORC JIT for fast compilation with all safety checks preserved.
    /// Target: ~1ms per function compilation time.
    ///
    /// ## Features
    /// - Lazy compilation (compile on first call)
    /// - Function caching
    /// - CBGR memory manager integration
    /// - Parallel compilation support
    ///
    /// ## Performance Characteristics
    /// - Compilation: ~1ms/function
    /// - CBGR overhead: ~15ns per check
    /// - Bounds checking: 2-3% overhead
    /// - Expected speedup: 5-10x vs interpreter
    ///
    /// PERF: Takes llvm_ctx by reference to avoid creating new Context per call.
    /// Context creation leaks ~800KB each; reusing prevents 25GB+ memory leak.
    #[cfg(feature = "llvm")]
    fn codegen_baseline_jit(
        &mut self,
        modules: &[Module],
        llvm_ctx: &inkwell::context::Context,
    ) -> Result<()> {
        use verum_codegen::{
            Codegen, CompilationStrategy, OrcJitBuilder, OrcJitEngine, OrcJitStats,
        };

        tracing::debug!("Generating baseline JIT code with ORC engine");

        // Configure ORC JIT engine
        let strategy = if self.jit_config.lazy_compilation {
            CompilationStrategy::Threshold(self.jit_config.lazy_threshold)
        } else {
            CompilationStrategy::Eager
        };

        let mut jit_builder = OrcJitBuilder::new(llvm_ctx).strategy(strategy);

        if self.jit_config.use_cbgr_memory_manager {
            jit_builder = jit_builder.with_cbgr();
        }

        if self.jit_config.parallel_threads > 1 {
            jit_builder = jit_builder.parallel_threads(self.jit_config.parallel_threads);
        }

        jit_builder = jit_builder.lazy_threshold(self.jit_config.lazy_threshold);

        let mut orc_jit = jit_builder
            .build()
            .map_err(|e| anyhow::anyhow!("Failed to create ORC JIT engine: {}", e))?;

        // Generate code for each module
        for (idx, module) in modules.iter().enumerate() {
            let module_name = format!("jit_module_{}", idx);
            let start = Instant::now();

            // Create codegen context and generate LLVM IR
            ...
            match codegen.generate_items(&module.items) {
                Ok(()) => {
                    ...
                    if let Err(e) = codegen.verify() {
                        tracing::warn!("JIT module verification warning: {}", e);
                    }

                    // Get the LLVM module
                    let llvm_module = llvm_ctx.create_module(&module_name);

                    // Add module to JIT engine
                    if let Err(e) = orc_jit.add_module(&module_name, llvm_module) {
                        tracing::error!("Failed to add module to JIT: {}", e);
                        return Err(anyhow::anyhow!("JIT module addition failed: {}", e));
                    }
                ...
                Err(e) => {
                    tracing::error!("JIT codegen error for module {}: {}", idx, e);
                    return Err(anyhow::anyhow!("JIT codegen failed: {}", e));
                }
            }
        }

        // Get JIT statistics
        if let Ok(jit_stats) = orc_jit.get_stats() {
            self.stats.functions_lazy_compiled = jit_stats.lazy_resolutions;
            tracing::debug!(
                "JIT stats: {} modules, {} lazy compilations",
                jit_stats.modules_added,
                jit_stats.lazy_resolutions
            );
        }

        // Check CBGR memory manager stats
        if let Some(cbgr_stats) = orc_jit.get_cbgr_stats() {
            self.stats.code_size_bytes = cbgr_stats.code_bytes;
            tracing::debug!(
                "CBGR memory: {} code bytes, avg {}ns alloc time",
                cbgr_stats.code_bytes,
                cbgr_stats.avg_alloc_time_ns
            );
        }

        tracing::info!(
            "Baseline JIT: {} functions compiled ({}  lazy), {} CBGR checks, {:.2}ms total",
            self.stats.functions_compiled,
            self.stats.functions_lazy_compiled,
            self.stats.cbgr_checks_inserted,
            self.stats.total_compile_time_ns as f64 / 1_000_000.0
        );

        Ok(())
    }

    /// Fallback baseline JIT without LLVM feature (unused when cfg gates skip it)
    #[cfg(not(feature = "llvm"))]
    #[allow(dead_code)]
    fn codegen_baseline_jit(&mut self, modules: &[Module]) -> Result<()> {
        tracing::warn!(
            "LLVM support not enabled. Baseline JIT falls back to interpreter preparation."
        );

        // Fall back to interpreter-style preparation
        self.codegen_interpreter(modules)
    }

    /// Generate code for Tier 2: Optimizing JIT
    ///
    /// Applies escape analysis to promote references and eliminate checks.
    ///
    /// ## Features
    /// - Full Tier 1 capabilities
    /// - Escape analysis for reference promotion
    /// - Partial CBGR check elimination
    /// - Hot path optimization
    ///
    /// ## Performance Characteristics
    /// - CBGR overhead: ~5ns for promoted refs
    /// - Expected speedup: 15-30x vs interpreter
    ///
    /// PERF: Takes llvm_ctx by reference to avoid creating new Context per call.
    #[cfg(feature = "llvm")]
    fn codegen_optimizing_jit(
        &mut self,
        modules: &[Module],
        llvm_ctx: &inkwell::context::Context,
    ) -> Result<()> {
        use verum_codegen::{CBGROptimizationPass, Codegen, OrcJitBuilder, SbglAnalyzer};

        tracing::debug!("Generating optimizing JIT code with escape analysis");

        // Start with baseline JIT setup
        let mut jit_builder = OrcJitBuilder::new(llvm_ctx)
            .strategy(verum_codegen::CompilationStrategy::Lazy)
            .with_cbgr()
            .lazy_threshold(self.jit_config.lazy_threshold);
        ...
        let mut orc_jit = jit_builder
            .build()
            .map_err(|e| anyhow::anyhow!("Failed to create optimizing JIT: {}", e))?;

        // Generate and optimize code for each module
        for (idx, module) in modules.iter().enumerate() {
            let module_name = format!("opt_jit_module_{}", idx);
            let start = Instant::now();
            ...
            match codegen.generate_items(&module.items) {
                Ok(()) => {
                    // Run SBGL escape analysis
                    if self.enable_escape_analysis {
                        let sbgl_analyzer = SbglAnalyzer::new();

                        for item in &module.items {
                            if let verum_ast::decl::ItemKind::Function(func) = &item.kind {
                                // Analyze each function for escape behavior
                                let decision = sbgl_analyzer.analyze_function(func);

                                match decision {
                                    verum_codegen::SbglDecision::Promote => {
                                        self.stats.references_promoted += 1;
                                    }
                                    verum_codegen::SbglDecision::Keep => {
                                        // Keep full CBGR checks
                                    }
                                    verum_codegen::SbglDecision::Reject(reason) => {
                                        tracing::debug!(
                                            "SBGL rejection for {}: {:?}",
                                            func.name,
                                            reason
                    ...
                    // Apply CBGR optimization pass
                    let opt_config = verum_codegen::CBGROptimizationConfig::default();
                    let mut cbgr_opt = CBGROptimizationPass::new(opt_config);

                    // Note: In production, this would operate on the LLVM IR
                    // For now, we track statistics based on AST analysis
                    ...
                    if let Err(e) = codegen.verify() {
                        tracing::warn!("Optimizing JIT verification warning: {}", e);
                    }

                    let llvm_module = llvm_ctx.create_module(&module_name);
                    if let Err(e) = orc_jit.add_module(&module_name, llvm_module) {
                        return Err(anyhow::anyhow!(
                            "Optimizing JIT module addition failed: {}",
                            e
                        ));
                    }
                    ...
                    // Count compiled functions
                    for item in &module.items {
                        if let verum_ast::decl::ItemKind::Function(_) = &item.kind {
                            self.stats.functions_compiled += 1;
                        }
                    }
                }
                Err(e) => {
                    tracing::error!("Optimizing JIT codegen error: {}", e);
                    return Err(anyhow::anyhow!("Optimizing JIT failed: {}", e));
                }
            }
        }

        tracing::info!(
            "Optimizing JIT: {} functions, {:.1}% CBGR elimination, {} refs promoted",
            self.stats.functions_compiled,
            self.stats.cbgr_elimination_ratio() * 100.0,
            self.stats.references_promoted
        );

        Ok(())
    }

    /// Fallback optimizing JIT without LLVM feature (unused when cfg gates skip it)
    #[cfg(not(feature = "llvm"))]
    #[allow(dead_code)]
    fn codegen_optimizing_jit(&mut self, modules: &[Module]) -> Result<()> {
        tracing::warn!("LLVM support not enabled. Optimizing JIT falls back to baseline.");
        self.codegen_baseline_jit(modules)
    }
    ...
    /// Fallback AOT without LLVM feature (unused when cfg gates skip it)
    #[cfg(not(feature = "llvm"))]
    #[allow(dead_code)]
    fn codegen_aot_llvm(&mut self, modules: &[Module]) -> Result<()> {
        tracing::warn!("LLVM support not enabled. AOT compilation not available.");
        tracing::info!("Falling back to interpreter preparation.");
