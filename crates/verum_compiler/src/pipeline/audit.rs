...
impl<'s> CompilationPipeline<'s> {
    ...
    pub fn count_stdlib_body_errors(
        &mut self,
    ) -> (usize, std::collections::HashMap<String, usize>, Vec<String>) {
        // Load stdlib modules if not yet loaded
        if self.modules.is_empty() {
            if let Err(e) = self.load_stdlib_modules() {
                warn!("Failed to load stdlib modules: {}", e);
        ...
        let mut stdlib_modules: Vec<(Text, std::sync::Arc<Module>)> = self
            .modules
            .iter()
            .filter(|(k, _)| k.as_str().starts_with("core"))
            .filter(|(k, _)| {
        ...
        {
            use verum_types::{Type, TypeScheme, TypeVar};
            for &(name, pc) in &[
                ("num_cpus", 0),
                ("getpagesize", 0),
                ("raw_read", 3),
                ...
                ("embedded_ctx_get", 1),
                ("embedded_ctx_set", 2),
                ("embedded_ctx_clear", 1),
                ("embedded_ctx_push_frame", 0),
                ("embedded_ctx_pop_frame", 0),
                ("__load_i64", 1),
            ] {
                let p: verum_common::List<Type> =
                    (0..pc).map(|_| Type::Var(TypeVar::fresh())).collect();
                let r = Type::Var(TypeVar::fresh());
                let v: verum_common::List<TypeVar> = p
                    .iter()
                    .filter_map(|t| if let Type::Var(v) = t { Some(*v) } else { None })
                    .chain(std::iter::once(if let Type::Var(v) = &r {
                        *v
                    } else {
                        unreachable!()
                    }))
            ...
            for item in &stdlib_mod.items {
                ...
                if let Err(e) = check_result {
                    ...
                    let category = if error_str.contains("TypeNotFound") {
                        "TypeNotFound"
                    } else if error_str.contains("UnresolvedName")
                        || error_str.contains("UndefinedVariable")
                    {
                        "UnresolvedName"
                    } else if error_str.contains("TypeMismatch")
                        || error_str.contains("Mismatch")
                        || error_display.contains("Type mismatch")
                        || error_display.contains("type mismatch")
                    {
                        "TypeMismatch"
                    } else if error_str.contains("NotCallable")
                        || error_display.contains("not a function type")
                    {
                        "NotCallable"
                    } else if error_str.contains("FieldNotFound")
                        || (error_display.contains("field") && error_display.contains("not found"))
                    {
                        "FieldNotFound"
                    } else if error_str.contains("MethodNotFound")
                        || (error_display.contains("method") && error_display.contains("not found"))
                    {
                        "MethodNotFound"
                    } else if error_str.contains("ArityMismatch")
                        || error_display.contains("wrong number of arguments")
                        || error_display.contains("Function requires")
                        || error_display.contains("Function accepts")
                    {
                        "ArityMismatch"
                        ...
                        || error_display.contains("Unknown variant constructor")
                    {
                        "NotImplemented"
                    } else if error_str.contains("unbound variable")
                        || error_display.contains("unbound variable")
                    {
                        "UnboundVariable"
                    } else if error_str.contains("super keyword") {
                        "SuperKeyword"
                    } else if error_str.contains("undefined context")
                        || error_display.contains("undefined context")
                        || error_display.contains("missing context")
                    {
                        "UndefinedContext"
                    } else if error_display.contains("Pattern expects")
                        || error_display.contains("Expected reference type for reference pattern")
                    {
                        "PatternError"
                    } else if error_str.contains("infinite type")
                        || error_display.contains("recursion")
                        || error_display.contains("stack overflow")
                    {
                        "InfiniteType"
                    } else if error_str.contains("invalid cast")
                        || error_str.contains("InvalidCast")
                        || error_display.contains("invalid cast")
                        || error_display.contains("cannot cast")
                    {
                        "InvalidCast"
                    ...
                    *category_counts.entry(category.to_string()).or_insert(0) += 1;
                    ...
                    let item_name = match &item.kind {
                        ...
                        verum_ast::ItemKind::Impl(impl_decl) => match &impl_decl.kind {
        ...
        details.push(format!(
            "[SUMMARY] checked={}, skipped_timeout={}, errors={}",
            checked_count, skipped_count, total_errors
        ));
