//! `@derive(TypedRow)` — auto-generate the
//! `core.database.postgres.typed_row.TypedRow` protocol impl
//! for record / newtype types.
//!
//! Closes the V1 follow-up declared in
//! `core/database/postgres/typed_row.vr` line 27:
//! "User-defined structs are out of V0 — they need either a
//! derive macro or hand-written impls. V1 will ship
//! `@derive(TypedRow)`."
//!
...
//!
//! ```verum
//! implement TypedRow for User {
//!     fn from_typed_row(
//!         row: &List<TypedValue>,
//!         desc: &Maybe<RowDescription>,
//!     ) -> Result<User, DbError> {
//!         core.database.postgres.typed_row.expect_arity(row, 3)?;
//!         let id: Int =
//!             core.database.postgres.typed_row.decode_field(row, 0, desc)?;
//!         let name: Text =
//!             core.database.postgres.typed_row.decode_field(row, 1, desc)?;
//!         let age: Maybe<Int> =
//!             core.database.postgres.typed_row.decode_field(row, 2, desc)?;
//!         Ok(User { id: id, name: name, age: age })
//!     }
//! }
//! ```
//!
//! For a newtype `type UserId is (Int);` the macro emits:
//!
//! ```verum
//! implement TypedRow for UserId {
//!     fn from_typed_row(
//!         row: &List<TypedValue>,
//!         desc: &Maybe<RowDescription>,
//!     ) -> Result<UserId, DbError> {
//!         core.database.postgres.typed_row.expect_arity(row, 1)?;
//!         let inner: Int =
//!             core.database.postgres.typed_row.decode_field(row, 0, desc)?;
//!         Ok(UserId(inner))
//!     }
...
//!
//! The user's module doesn't have to mount `decode_field` /
//! `expect_arity` / `TypedValue` / `RowDescription` — the
//! generated code reaches into them by absolute path. Same
//! pattern as `@derive(Display)` reaching `Formatter`.
//!
//! # Out of V0
//!
//! - Sum types (enums) — they're runtime-discriminated; can't
//!   generically decode without a tag column convention. Caller
//!   writes a hand-rolled impl that pattern-matches on a
//!   discriminator column. We reject with a friendly error.
//! - `Type<T1, T2>` records with custom generic parameters — V0
//!   only supports concrete record types. Generic records work
//!   if the consumer passes the inner type at the call site
//!   (the let-binding annotation pins T).
//! - `@rename("col")` field attribute for column-name overrides
//!   — V0 decodes positionally; column-name resolution happens
//!   in a future iteration that consumes `desc` for the lookup.
...
/// `@derive(TypedRow)` macro implementation.
pub struct DeriveTypedRow;

impl DeriveMacro for DeriveTypedRow {
    ...
    fn expand(&self, ctx: &DeriveContext) -> DeriveResult<Item> {
        ...
        // Sum types (enums) — runtime-discriminated, can't be
        // derived generically. The caller picks a tag column +
        // hand-writes the impl.
        if type_info.is_enum {
            return Err(DeriveError::UnsupportedTypeKind {
                kind: Text::from("enum"),
                hint: Text::from(
                    "Cannot derive TypedRow for sum types — they need a \
                     hand-rolled impl that picks a discriminator column. \
                     Implement TypedRow manually.",
                ),
                span,
            });
        }
    ...
    fn doc_comment(&self) -> &'static str {
        "Auto-generated TypedRow implementation — positional column decode \
         via `core.database.postgres.typed_row.decode_field`."
    }
}
...
impl DeriveTypedRow {
    ...
    /// ```text
    /// expect_arity(row, N)?;
    /// let f1: T1 = decode_field(row, 0, desc)?;
    /// ...
    /// Ok(TypeName { f1: f1, ... })
    /// ```
    fn generate_struct_body(&self, type_info: &TypeInfo, span: Span) -> DeriveResult<Block> {
        ...
        // For each field: let <name>: <T> = decode_field(row, i, desc)?;
    ...
    /// ```text
    /// expect_arity(row, 1)?;
    /// let inner: T = decode_field(row, 0, desc)?;
    /// Ok(TypeName(inner))
    /// ```
    fn generate_newtype_body(&self, type_info: &TypeInfo, span: Span) -> DeriveResult<Block> {
        ...
        // The newtype's single synthesized field has name "0";
        // bind to a friendlier `inner` for the constructor call.
        let field = type_info
            .fields
            .iter()
            .next()
    ...
    /// `fn from_typed_row(row: &List<TypedValue>, desc: &Maybe<RowDescription>)
    ///     -> Result<Self, DbError>`
    fn create_from_typed_row_method(
        &self,
        ctx: &DeriveContext,
        body: Block,
        span: Span,
        ...
        let desc_ty = self.ref_type(self.generic_type(
            "Maybe",
            vec![self.qualified_type(
                &[
                    "core",
        ...
        let desc_param = self.param("desc", desc_ty, span);

        ctx.method(
            "from_typed_row",
            List::from(vec![row_param, desc_param]),
            return_ty,
            body,
            span,
        )
    ...
    /// `&<inner>` — the function takes `row` / `desc` BY REFERENCE,
    /// so we wrap with a Ref-prefix unary op to thread them
    /// through. `decode_field(row, i, desc)?` accepts `row` /
    /// `desc` as-is because they're already bound as refs in the
    /// outer signature; no Ref wrap needed at the inner call.
    /// Helper kept for symmetry should a future pass need it.
    #[allow(dead_code)]
    fn ref_expr(&self, inner: Expr, span: Span) -> Expr {
        Expr::new(
            ExprKind::Unary {
                op: UnOp::Ref,
                expr: Heap::new(inner),
    ...
    /// `core.database.postgres.typed_row.<fname>(args)`
    fn qualified_call(&self, fname: &str, args: List<Expr>, span: Span) -> Expr {
        let path = Path::new(
            List::from(vec![
                ...
                PathSegment::Name(Ident::new("postgres", span)),
...
// ----------------------------------------------------------------------
// Suppress unused-import warnings on `Visibility` — the type is only
// referenced through the `ctx.method` helper today, but the import is
// load-bearing for future fields (e.g. private impls under
// `Visibility::Private`).
// ----------------------------------------------------------------------
#[allow(dead_code)]
...
mod tests {
    ...
    fn record_two_fields_compiles() {
        ...
        let derive = DeriveTypedRow;
        let item = derive.expand(&ctx).expect("expand should succeed");
        // Smoke check: result is an Impl item.
        match item.kind {
    ...
    fn enum_rejected() {
        ...
        let ctx = DeriveContext::from_type_decl(&decl, Span::default()).unwrap();
        let derive = DeriveTypedRow;
        let err = derive.expand(&ctx).expect_err("enum should be rejected");
        match err {
    ...
    fn newtype_compiles() {
        ...
        let ctx = DeriveContext::from_type_decl(&decl, Span::default()).unwrap();
        let derive = DeriveTypedRow;
        let item = derive.expand(&ctx).expect("newtype expand should succeed");
        match item.kind {
            verum_ast::decl::ItemKind::Impl(_) => {}
            other => panic!("expected ItemKind::Impl, got {:?}", other),
        }
