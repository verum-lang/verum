...
use crate::semantic_query::SemanticQueryCache;
...
pub struct CompilationPipeline<'s> {
    ...
    /// Semantic query cache for content-addressed caching.
    /// Uses semantic hashes instead of file paths, enabling:
    /// - Deduplication across semantically identical items
    /// - Cross-project artifact reuse
    /// - Smart invalidation based on semantic changes
    ///
    /// Content-addressed caching using semantic hashes (not file paths) for
    /// deduplication across identical items and cross-project artifact reuse.
    #[allow(dead_code)]
...
impl<'s> CompilationPipeline<'s> {
    ...
    pub fn new(session: &'s mut Session) -> Self {
        ...
        Self {
            ...
            // Semantic query cache for content-addressed caching
            semantic_cache: SemanticQueryCache::new(),
    ...
    fn clear_non_compilable_stdlib_modules(&mut self) {
        ...
        let retained: Map<Text, Arc<Module>> = self.modules
            ...
            .filter(|(path, _module)| {
                let p = path.as_str();
                ALWAYS_INCLUDE.iter().any(|inc| p == *inc)
            })
            .collect();
    ...
    /// Generate C runtime stubs for CBGR and stdlib functions (legacy inline version)
    /// This function is kept temporarily during migration but is no longer called.
    #[allow(dead_code)]
    fn _generate_runtime_stubs_legacy(&self, temp_dir: &Path) -> Result<PathBuf> {
        let stubs_path = temp_dir.join("verum_runtime_stubs.c");

        // Legacy: production-grade C runtime stubs (inline)
        // Now replaced by LLVM IR emission in verum_codegen::llvm::runtime
        let stubs_code = r#"
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
// ========================================================================
// CBGR Memory Management - Production Implementation
// Counter-Based Generational References: each reference carries a generation counter
// validated against the allocation's current generation on dereference (~15ns overhead).
// Three tiers: &T (managed, ~15ns), &checked T (proven safe, 0ns), &unsafe T (manual, 0ns).
// ========================================================================

// Generation values for CBGR validation
#define GEN_UNALLOCATED 0
#define GEN_FREE 0xFFFFFFFFU
#define GEN_INITIAL 1
#define GEN_MAX 0xFFFFFFFEU

// Capability flags (match core/mem/capability.vr)
#define CAP_READ      0x0001  // Bit 0: Can read data
#define CAP_WRITE     0x0002  // Bit 1: Can write data
#define CAP_EXECUTE   0x0004  // Bit 2: Can execute
#define CAP_DELEGATE  0x0008  // Bit 3: Can create sub-refs
#define CAP_REVOKE    0x0010  // Bit 4: Can invalidate
#define CAP_BORROWED  0x0020  // Bit 5: Non-owning
#define CAP_MUTABLE   0x0040  // Bit 6: Can mutate
#define CAP_NO_ESCAPE 0x0080  // Bit 7: Cannot escape scope
#define CAP_ALL       0x00FF  // All capabilities

// Allocation flags (atomic)
#define FLAG_MOVED   0x01
#define FLAG_REVOKED 0x02
#define FLAG_PINNED  0x04

// Allocation header (32 bytes, placed before user data)
typedef struct __attribute__((aligned(32))) {
    _Atomic(uint32_t) generation; // Current generation (increments on dealloc)
    uint32_t size;                // Allocation size
    _Atomic(uint16_t) epoch;      // Epoch counter for wraparound protection
    uint16_t capabilities;        // Capability flags (read/write/move)
    _Atomic(uint32_t) ref_count;  // Active reference count
    _Atomic(uint64_t) flags;      // Atomic flags (moved, revoked, etc.)
    void* next_free;              // Free list pointer
} AllocationHeader;
...
typedef struct {
    void* ptr;
    uint32_t generation;
    uint32_t epoch_caps;  // Packed: epoch (16 bits) | capabilities (16 bits)
} ThinRef;

// FatRef: 24-byte reference for slices/dynamic types
...
// Per-thread execution state including TLS slots, context stack, and CBGR stats.
// ========================================================================

// Maximum stack trace depth for panic handler
#define MAX_STACK_DEPTH 64

// Maximum number of context providers
#define MAX_CONTEXT_PROVIDERS 256
...
// Thread-local execution context
typedef struct {
    // CBGR epoch cache for fast validation
    uint64_t cached_epoch;

    // Current execution tier (0-3)
    ...
    StackFrame stack_frames[MAX_STACK_DEPTH];
    size_t stack_depth;

    // Context providers (Level 2 DI)
    ContextEntry contexts[MAX_CONTEXT_PROVIDERS];
    size_t context_count;

    // Backtrace capture enabled
...
// Global epoch counter (increments on generation wraparound)
static _Atomic(uint64_t) global_epoch = 0;

// Per-thread execution context
static pthread_key_t execution_context_key;
...
static _Atomic(uint64_t) total_cbgr_checks = 0;
static _Atomic(uint64_t) failed_cbgr_checks = 0;

// Panic handler configuration
static _Atomic(bool) panic_backtrace_enabled = true;
static void (*custom_panic_hook)(const char*, size_t, const char*, uint32_t) = NULL;

// Forward declarations
static void context_key_destructor(void* ctx);
static void init_context_key(void);
static ExecutionContext* get_or_create_context(void);

// ========================================================================
// Context Key Initialization (Thread-Local Storage)
// ========================================================================

static void context_key_destructor(void* ctx) {
    if (ctx) {
        ExecutionContext* exec_ctx = (ExecutionContext*)ctx;
        // Clean up context providers
        for (size_t i = 0; i < exec_ctx->context_count; i++) {
            if (exec_ctx->contexts[i].destructor && exec_ctx->contexts[i].value) {
                exec_ctx->contexts[i].destructor(exec_ctx->contexts[i].value);
            }
        }
        free(exec_ctx);
    }
...
static void init_context_key(void) {
    pthread_key_create(&execution_context_key, context_key_destructor);
}

static ExecutionContext* get_or_create_context(void) {
    pthread_once(&context_key_once, init_context_key);

    ExecutionContext* ctx = (ExecutionContext*)pthread_getspecific(execution_context_key);
    if (!ctx) {
        ctx = (ExecutionContext*)calloc(1, sizeof(ExecutionContext));
        if (ctx) {
            ctx->cached_epoch = atomic_load_explicit(&global_epoch, memory_order_acquire);
            ctx->execution_tier = 3; // AOT tier by default
            ctx->thread_id = pthread_self();
            ctx->backtrace_enabled = atomic_load(&panic_backtrace_enabled);
            pthread_setspecific(execution_context_key, ctx);
        }
    }
    return ctx;
}
...
static inline AllocationHeader* get_header(void* ptr) {
    return (AllocationHeader*)((char*)ptr - sizeof(AllocationHeader));
}

// CBGR allocate with header
void* verum_cbgr_allocate(size_t size) {
    // Round up to alignment boundary
    size_t total_size = sizeof(AllocationHeader) + ((size + 31) & ~31UL);

    // Use posix_memalign for portable aligned allocation
    void* raw = NULL;
    if (posix_memalign(&raw, 32, total_size) != 0) {
        return NULL;
    }

    // Initialize header with proper atomic initialization
    AllocationHeader* header = (AllocationHeader*)raw;
    atomic_init(&header->generation, GEN_INITIAL);
    header->size = (uint32_t)size;
    atomic_init(&header->epoch, (uint16_t)(atomic_load(&global_epoch) & 0xFFFF));
    header->capabilities = CAP_FULL;
...
// CBGR deallocate with generation increment
void verum_cbgr_deallocate(void* ptr) {
    if (!ptr) return;

    AllocationHeader* header = get_header(ptr);

    // Increment generation atomically (invalidates existing references)
    // Use acquire-release for proper memory ordering
    uint32_t old_gen = atomic_fetch_add_explicit(&header->generation, 1, memory_order_acq_rel);
    uint32_t new_gen = old_gen + 1;

    // Check for wraparound - increment global epoch if needed
    if (new_gen >= GEN_MAX || new_gen < old_gen) {
        atomic_fetch_add_explicit(&global_epoch, 1, memory_order_release);
        atomic_store_explicit(&header->generation, GEN_INITIAL, memory_order_release);
        atomic_fetch_add_explicit(&header->epoch, 1, memory_order_release);
    }

    atomic_store_explicit(&header->ref_count, 0, memory_order_release);
    atomic_fetch_add_explicit(&total_deallocations, 1, memory_order_relaxed);

    // Free the entire allocation (including header)
    free((char*)ptr - sizeof(AllocationHeader));
}

// ========================================================================
// CBGR Validation - Production Implementation (~15ns target)
// Validates generation counter match + epoch bounds on each dereference.
// ========================================================================

// CBGR validation check - ~15ns target on cache-hot path
// Returns true if reference is valid, false if use-after-free/epoch mismatch
// Performance: Best 15ns (L1 hit), Typical 20-30ns (L2/L3), Worst 50-100ns (contended)
bool verum_cbgr_check(ThinRef* ref) {
    // Fast null check
    if (__builtin_expect(!ref || !ref->ptr, 0)) return false;

    atomic_fetch_add_explicit(&total_cbgr_checks, 1, memory_order_relaxed);

    AllocationHeader* header = get_header(ref->ptr);

    // Load current generation with acquire semantics for proper memory ordering
    uint32_t actual_gen = atomic_load_explicit(&header->generation, memory_order_acquire);

    // Fast path: generation match (most common case)
    if (__builtin_expect(ref->generation != actual_gen, 0)) {
        atomic_fetch_add_explicit(&failed_cbgr_checks, 1, memory_order_relaxed);
        return false;
    }

    // Extract epoch from reference's packed field
    uint16_t ref_epoch = (uint16_t)(ref->epoch_caps & 0xFFFF);

    // Get thread-local epoch cache for fast validation
    ExecutionContext* ctx = get_or_create_context();
    uint64_t current_epoch = atomic_load_explicit(&global_epoch, memory_order_acquire);

    // Epoch check with thread-local caching
    if (__builtin_expect(current_epoch != ctx->cached_epoch, 0)) {
        // Epoch advanced - update cache and re-check
        ctx->cached_epoch = current_epoch;

        uint16_t actual_epoch = atomic_load_explicit(&header->epoch, memory_order_acquire);
        if (ref_epoch != actual_epoch) {
            atomic_fetch_add_explicit(&failed_cbgr_checks, 1, memory_order_relaxed);
            return false;
        }
    }

    // Extract and check capabilities (fast path skip if all caps)
    uint16_t ref_caps = (uint16_t)(ref->epoch_caps >> 16);
    if (__builtin_expect(ref_caps != CAP_FULL, 0)) {
        if ((header->capabilities & ref_caps) != ref_caps) {
            atomic_fetch_add_explicit(&failed_cbgr_checks, 1, memory_order_relaxed);
            return false;
        }
    }

    // Check for revoked flag
    uint64_t flags = atomic_load_explicit(&header->flags, memory_order_acquire);
    if (__builtin_expect(flags & FLAG_REVOKED, 0)) {
        atomic_fetch_add_explicit(&failed_cbgr_checks, 1, memory_order_relaxed);
        return false;
    }
...
// CBGR check with write capability verification
bool verum_cbgr_check_write(ThinRef* ref) {
    if (!verum_cbgr_check(ref)) return false;

    // Verify write capability
    uint16_t ref_caps = (uint16_t)(ref->epoch_caps >> 16);
    if (!(ref_caps & CAP_WRITE)) {
        atomic_fetch_add_explicit(&failed_cbgr_checks, 1, memory_order_relaxed);
        return false;
    }
...
// FatRef validation (includes bounds check)
bool verum_cbgr_check_fat(FatRef* ref) {
    if (!ref || !ref->ptr) return false;

    ThinRef thin = { ref->ptr, ref->generation, ref->epoch_caps };
    if (!verum_cbgr_check(&thin)) return false;

    // Bounds check
    AllocationHeader* header = get_header(ref->ptr);
    if (ref->len > header->size) {
        atomic_fetch_add(&failed_cbgr_checks, 1);
        return false;
    }
...
// Create a ThinRef from allocation
ThinRef verum_cbgr_create_thin_ref(void* ptr) {
    ThinRef ref = { NULL, GEN_UNALLOCATED, 0 };
    if (!ptr) return ref;

    AllocationHeader* header = get_header(ptr);
    ref.ptr = ptr;
    ref.generation = header->generation;
    ref.epoch_caps = ((uint32_t)header->capabilities << 16) | (uint32_t)header->epoch;
    return ref;
}

// Create a FatRef from allocation with length
FatRef verum_cbgr_create_fat_ref(void* ptr, uint64_t len) {
    FatRef ref = { NULL, GEN_UNALLOCATED, 0, 0 };
    if (!ptr) return ref;
...
// Epoch management
void verum_cbgr_epoch_begin(void) {
    // Mark start of epoch-protected region
    ExecutionContext* ctx = get_or_create_context();
    if (ctx) {
        ctx->cached_epoch = atomic_load_explicit(&global_epoch, memory_order_acquire);
    }
...
void verum_cbgr_epoch_end(void) {
    // Epoch region ended - next access will re-validate
    // No-op in current implementation (validation happens on access)
}

uint64_t verum_cbgr_get_epoch(void) {
    return atomic_load_explicit(&global_epoch, memory_order_acquire);
}

// GC root tracking (for precise collection)
#define MAX_ROOTS 4096
static void* gc_roots[MAX_ROOTS];
static _Atomic(size_t) gc_root_count = 0;

void verum_cbgr_register_root(void* ptr) {
    size_t idx = atomic_fetch_add(&gc_root_count, 1);
    if (idx < MAX_ROOTS) {
        gc_roots[idx] = ptr;
    }
}

void verum_cbgr_unregister_root(void* ptr) {
    size_t count = atomic_load(&gc_root_count);
    for (size_t i = 0; i < count && i < MAX_ROOTS; i++) {
        if (gc_roots[i] == ptr) {
            gc_roots[i] = NULL;
            break;
        }
...
// Revoke a reference (for explicit lifetime control)
void verum_cbgr_revoke(void* ptr) {
    if (!ptr) return;
    AllocationHeader* header = get_header(ptr);

    // Increment generation atomically without freeing memory
    atomic_fetch_add_explicit(&header->generation, 1, memory_order_acq_rel);

    // Mark as revoked in flags atomically
    atomic_fetch_or_explicit(&header->flags, FLAG_REVOKED, memory_order_release);
}

// Get CBGR statistics for profiling
void verum_cbgr_get_stats(uint64_t* allocs, uint64_t* deallocs, uint64_t* checks, uint64_t* failed) {
    if (allocs) *allocs = atomic_load_explicit(&total_allocations, memory_order_relaxed);
    if (deallocs) *deallocs = atomic_load_explicit(&total_deallocations, memory_order_relaxed);
    if (checks) *checks = atomic_load_explicit(&total_cbgr_checks, memory_order_relaxed);
    if (failed) *failed = atomic_load_explicit(&failed_cbgr_checks, memory_order_relaxed);
}

// Reference counting helpers
void verum_cbgr_ref_acquire(void* ptr) {
    if (!ptr) return;
    AllocationHeader* header = get_header(ptr);
    atomic_fetch_add_explicit(&header->ref_count, 1, memory_order_relaxed);
}

void verum_cbgr_ref_release(void* ptr) {
    if (!ptr) return;
    AllocationHeader* header = get_header(ptr);
    uint32_t old_count = atomic_fetch_sub_explicit(&header->ref_count, 1, memory_order_acq_rel);
    if (old_count == 1) {
        // Last reference - deallocate
        verum_cbgr_deallocate(ptr);
    }
}

uint32_t verum_cbgr_ref_count(void* ptr) {
    if (!ptr) return 0;
    AllocationHeader* header = get_header(ptr);
...
// ========================================================================
// Verum Standard Library Stubs
// ========================================================================

void verum_print_text(const char* str, size_t len) {
    if (str && len > 0) {
        fwrite(str, 1, len, stdout);
    }
}

void verum_print_int(int64_t value) {
    printf("%lld", (long long)value);
}

void verum_print_float(double value) {
    printf("%g", value);
}

void verum_print_bool(bool value) {
    printf("%s", value ? "true" : "false");
}

void verum_println(void) {
    putchar('\n');
    fflush(stdout);
}

// ========================================================================
// Panic Handler with Stack Unwinding Support
// Implements setjmp/longjmp-based unwinding with stack trace collection.
// ========================================================================

// Push stack frame for debugging
void verum_push_stack_frame(const char* function_name, const char* file_name,
                            uint32_t line, uint32_t column) {
    ExecutionContext* ctx = get_or_create_context();
    if (!ctx || ctx->stack_depth >= MAX_STACK_DEPTH) return;

    StackFrame* frame = &ctx->stack_frames[ctx->stack_depth++];
...
// Pop stack frame
void verum_pop_stack_frame(void) {
    ExecutionContext* ctx = get_or_create_context();
...
static void print_stack_trace(ExecutionContext* ctx) {
    if (!ctx || ctx->stack_depth == 0) {
        fprintf(stderr, "  <no stack trace available>\n");
        fprintf(stderr, "  note: run with `VERUM_PANIC_BACKTRACE=1` for a backtrace\n");
        return;
    }

    fprintf(stderr, "\nStack backtrace:\n");
    for (size_t i = ctx->stack_depth; i > 0; i--) {
        StackFrame* frame = &ctx->stack_frames[i - 1];
        fprintf(stderr, "   %zu: %s\n", ctx->stack_depth - i,
...
void verum_set_panic_hook(void (*hook)(const char*, size_t, const char*, uint32_t)) {
    custom_panic_hook = hook;
}

// Configure panic backtrace
void verum_set_panic_backtrace(bool enabled) {
    atomic_store(&panic_backtrace_enabled, enabled);
}

// Check if panic recovery is available
bool verum_can_catch_panic(void) {
    ExecutionContext* ctx = get_or_create_context();
    return ctx && ctx->panic_state.has_recovery_point;
}

// Set panic recovery point (called by catch_unwind)
// Returns 0 on initial call, non-zero on panic recovery
int verum_set_panic_recovery(void) {
    ExecutionContext* ctx = get_or_create_context();
    if (!ctx) return -1;

    ctx->panic_state.has_recovery_point = true;
    return setjmp(ctx->panic_state.recovery_point);
}

// Clear panic recovery point
void verum_clear_panic_recovery(void) {
    ExecutionContext* ctx = get_or_create_context();
    if (ctx) {
        ctx->panic_state.has_recovery_point = false;
        ctx->panic_state.panic_message = NULL;
        ctx->panic_state.panic_message_len = 0;
    }
}

// Get last panic message (for catch_unwind)
const char* verum_get_panic_message(size_t* len) {
    ExecutionContext* ctx = get_or_create_context();
    if (ctx && len) {
        *len = ctx->panic_state.panic_message_len;
    }
    return ctx ? ctx->panic_state.panic_message : NULL;
}

// Main panic function with stack trace and recovery support
void verum_panic(const char* msg, size_t len) {
    ExecutionContext* ctx = get_or_create_context();

    // Check for custom panic hook first
    if (custom_panic_hook && ctx) {
        StackFrame* top = ctx->stack_depth > 0 ? &ctx->stack_frames[ctx->stack_depth - 1] : NULL;
        custom_panic_hook(msg, len, top ? top->file_name : NULL, top ? top->line_number : 0);
    }

    // Store panic info for potential recovery
    if (ctx) {
        ctx->panic_state.panic_message = msg;
        ctx->panic_state.panic_message_len = len;
        ctx->panic_state.panic_code = 101;
    }

    // Check if we have a recovery point (catch_unwind)
    if (ctx && ctx->panic_state.has_recovery_point) {
        longjmp(ctx->panic_state.recovery_point, 1);
        // unreachable
    }

    // No recovery - print error and exit
    fprintf(stderr, "\nthread '%s' panicked at ",
            (ctx && ctx->thread_name) ? ctx->thread_name : "main");

    // Print location if available
    if (ctx && ctx->stack_depth > 0) {
        StackFrame* frame = &ctx->stack_frames[ctx->stack_depth - 1];
        if (frame->file_name) {
    ...
    if (msg && len > 0) {
        ...
        fprintf(stderr, "explicit panic");
    }
    fprintf(stderr, "\n");

    // Print stack trace if enabled
    if (ctx && ctx->backtrace_enabled) {
...
void verum_panic_fmt(const char* fmt, ...) {
    char buffer[4096];
    va_list args;
    va_start(args, fmt);
    ...
    if (len > 0) {
        verum_panic(buffer, (size_t)len);
    } else {
        verum_panic("panic formatting failed", 23);
    }
}

// Assert with message
void verum_assert(bool condition, const char* msg, size_t len,
                  const char* file, uint32_t line) {
    if (!condition) {
        verum_push_stack_frame("assert!", file, line, 0);
        verum_panic(msg, len);
    }
}

// Debug assert (only in debug builds - controlled by define)
#ifdef VERUM_DEBUG
void verum_debug_assert(bool condition, const char* msg, size_t len,
                        const char* file, uint32_t line) {
    verum_assert(condition, msg, len, file, line);
}
#else
void verum_debug_assert(bool condition, const char* msg, size_t len,
                        const char* file, uint32_t line) {
    (void)condition; (void)msg; (void)len; (void)file; (void)line;
}
#endif

// Memory utilities
void* verum_memcpy(void* dest, const void* src, size_t n) {
    return memcpy(dest, src, n);
}

void* verum_memset(void* s, int c, size_t n) {
    return memset(s, c, n);
}

int verum_memcmp(const void* s1, const void* s2, size_t n) {
    return memcmp(s1, s2, n);
}

size_t verum_strlen(const char* s) {
    return strlen(s);
}

char* verum_strdup(const char* s) {
    return strdup(s);
}
...
// Format string uses {} for placeholders, args is a packed array of VerumText values
// separated by null terminators
VerumText verum_format_string(VerumText format, VerumText args) {
    // First pass: count placeholders and calculate output size
    size_t placeholder_count = 0;
    size_t output_size = 0;
    size_t last_end = 0;

    for (size_t i = 0; i < format.len; i++) {
        if (i + 1 < format.len && format.ptr[i] == '{' && format.ptr[i + 1] == '}') {
            // Add literal part before placeholder
            output_size += i - last_end;
            placeholder_count++;
            last_end = i + 2;
            i++; // Skip the '}'
        }
    ...
    output_size += format.len - last_end;

    // Parse arguments (null-separated strings with length prefixes)
    // Each arg is: 8-byte length (size_t), then data
    const char* arg_ptr = args.ptr;
    const char* arg_end = args.ptr + args.len;
    VerumText* parsed_args = (VerumText*)malloc(sizeof(VerumText) * (placeholder_count + 1));
    size_t arg_count = 0;

    while (arg_ptr < arg_end && arg_count < placeholder_count) {
        if (arg_ptr + sizeof(size_t) > arg_end) break;
        size_t arg_len;
        memcpy(&arg_len, arg_ptr, sizeof(size_t));
        arg_ptr += sizeof(size_t);

        if (arg_ptr + arg_len > arg_end) break;
        parsed_args[arg_count].ptr = arg_ptr;
        parsed_args[arg_count].len = arg_len;
        output_size += arg_len;
        arg_ptr += arg_len;
        arg_count++;
    }
    ...
    if (!output) {
        free(parsed_args);
        return format; // Fallback to format string on allocation failure
    }
    ...
    for (size_t i = 0; i < format.len; i++) {
        if (i + 1 < format.len && format.ptr[i] == '{' && format.ptr[i + 1] == '}') {
            ...
            size_t literal_len = i - last_end;
            if (literal_len > 0) {
                memcpy(output + out_pos, format.ptr + last_end, literal_len);
                out_pos += literal_len;
            }

            // Insert argument if available
            if (arg_idx < arg_count) {
                memcpy(output + out_pos, parsed_args[arg_idx].ptr, parsed_args[arg_idx].len);
                out_pos += parsed_args[arg_idx].len;
                arg_idx++;
            }
    ...
    // Copy remaining literal part
    size_t remaining = format.len - last_end;
    if (remaining > 0) {
        memcpy(output + out_pos, format.ptr + last_end, remaining);
        out_pos += remaining;
    }
    ...
    VerumText result = {output, out_pos};
    return result;
}

VerumText verum_format_int(int64_t value) {
    char* buf = (char*)malloc(32);
    int len = snprintf(buf, 32, "%lld", (long long)value);
    VerumText result = {buf, (size_t)len};
    return result;
}

VerumText verum_format_float(double value) {
    char* buf = (char*)malloc(64);
    int len = snprintf(buf, 64, "%g", value);
    VerumText result = {buf, (size_t)len};
    return result;
}

VerumText verum_format_bool(bool value) {
    VerumText result;
    if (value) {
        result.ptr = "true";
        result.len = 4;
...
VerumText verum_concat_strings(VerumText a, VerumText b) {
    size_t total = a.len + b.len;
    char* buf = (char*)malloc(total + 1);
    memcpy(buf, a.ptr, a.len);
    memcpy(buf + a.len, b.ptr, b.len);
    buf[total] = '\0';
    return (VerumText){buf, total};
}

// ========================================================================
// String Builtins for Native Codegen
// ========================================================================

// Parse Text to Float
// Returns f64, uses strtod for parsing
double verum_parse_float(const char* ptr, int64_t len) {
    if (!ptr || len <= 0) {
        return 0.0;
    }

    // Create null-terminated copy for strtod
...
// Parse Text to Int
// Returns i64, uses strtoll for parsing
int64_t verum_parse_int(const char* ptr, int64_t len) {
    if (!ptr || len <= 0) {
        return 0;
    }
...
// Returns i32 (Unicode code point), -1 on error
int32_t verum_char_at(const char* ptr, int64_t len, int64_t index) {
    if (!ptr || len <= 0 || index < 0 || index >= len) {
        return -1; // Out of bounds
    }

    // For ASCII/UTF-8, just return byte as unsigned char
    // Full Unicode support would require proper UTF-8 decoding
    return (int32_t)(unsigned char)ptr[index];
}

// Convert Text to List<Char>
// Returns a List struct { ptr: *i32, len: i64, capacity: i64 }
typedef struct {
    int32_t* ptr;
    int64_t len;
    int64_t capacity;
...
CharList verum_text_to_chars(const char* ptr, int64_t len) {
    CharList result;
    ...
    // Allocate array for characters
    int32_t* chars = (int32_t*)malloc(sizeof(int32_t) * len);

    // Convert each byte to i32
    // For full UTF-8 support, this would need proper decoding
    for (int64_t i = 0; i < len; i++) {
        chars[i] = (int32_t)(unsigned char)ptr[i];
    }
...
// List<Text> representation matching Rust Vec layout
typedef struct {
    VerumText* ptr;    // pointer to data
    size_t len;        // number of elements
    size_t cap;        // capacity
} VerumList;

// Create a List<Text> from argc/argv
VerumList verum_create_args_list(int argc, char** argv) {
    VerumList list;
    list.len = (size_t)argc;
    list.cap = (size_t)argc;
    list.ptr = (VerumText*)malloc(sizeof(VerumText) * argc);

    for (int i = 0; i < argc; i++) {
        list.ptr[i].ptr = argv[i];
        list.ptr[i].len = strlen(argv[i]);
    }
...
// ========================================================================
// Context Management (Level 2 DI - Dynamic Contexts)
// Runtime context stack: `provide` pushes, lookup by type_id (~5-30ns via vtable).
// ========================================================================

// Provide a context value (push onto context stack)
void verum_context_provide(uint64_t type_id, void* value, void (*destructor)(void*)) {
    ExecutionContext* ctx = get_or_create_context();
    if (!ctx || ctx->context_count >= MAX_CONTEXT_PROVIDERS) return;

    ContextEntry* entry = &ctx->contexts[ctx->context_count++];
    entry->type_id = type_id;
    entry->value = value;
    entry->destructor = destructor;
}

// Get a context value by type id
void* verum_context_get(uint64_t type_id) {
    ExecutionContext* ctx = get_or_create_context();
    if (!ctx) return NULL;

    // Search from most recent to oldest (stack semantics)
    for (size_t i = ctx->context_count; i > 0; i--) {
        if (ctx->contexts[i - 1].type_id == type_id) {
            return ctx->contexts[i - 1].value;
        }
    }
    return NULL;
}

// Pop context value (cleanup after provide scope ends)
void verum_context_pop(uint64_t type_id) {
    ExecutionContext* ctx = get_or_create_context();
    if (!ctx || ctx->context_count == 0) return;

    // Find and remove the most recent context of this type
    for (size_t i = ctx->context_count; i > 0; i--) {
        if (ctx->contexts[i - 1].type_id == type_id) {
            ContextEntry* entry = &ctx->contexts[i - 1];
            if (entry->destructor && entry->value) {
                entry->destructor(entry->value);
            }
            // Shift remaining entries down
            for (size_t j = i - 1; j < ctx->context_count - 1; j++) {
                ctx->contexts[j] = ctx->contexts[j + 1];
            }
            ctx->context_count--;
            return;
        }
...
// Sets up CBGR allocator, execution context, and panic handler.
// ========================================================================

// Initialize runtime subsystems
static void verum_runtime_init(void) {
    // Check environment variables for configuration
    const char* backtrace_env = getenv("VERUM_PANIC_BACKTRACE");
    if (backtrace_env) {
        if (strcmp(backtrace_env, "0") == 0 || strcmp(backtrace_env, "false") == 0) {
            atomic_store(&panic_backtrace_enabled, false);
        } else if (strcmp(backtrace_env, "1") == 0 || strcmp(backtrace_env, "true") == 0) {
            atomic_store(&panic_backtrace_enabled, true);
        }
    }

    // Initialize thread-local context for main thread
    ExecutionContext* ctx = get_or_create_context();
    if (ctx) {
        ctx->thread_name = "main";
        ctx->execution_tier = 3; // AOT execution
    }
}

// Cleanup runtime subsystems
static void verum_runtime_cleanup(void) {
    // Get CBGR stats for debugging
    uint64_t allocs, deallocs, checks, failed;
    verum_cbgr_get_stats(&allocs, &deallocs, &checks, &failed);

    // Report stats if VERUM_DEBUG is set
    const char* debug_env = getenv("VERUM_DEBUG");
    if (debug_env && strcmp(debug_env, "1") == 0) {
        fprintf(stderr, "\n[CBGR Stats] allocs=%llu deallocs=%llu checks=%llu failed=%llu\n",
                (unsigned long long)allocs, (unsigned long long)deallocs,
                (unsigned long long)checks, (unsigned long long)failed);
    }

    // Check for memory leaks
    if (allocs != deallocs) {
        const char* leak_check = getenv("VERUM_LEAK_CHECK");
        if (leak_check && strcmp(leak_check, "1") == 0) {
            fprintf(stderr, "[CBGR Warning] Possible memory leak: %llu allocations, %llu deallocations\n",
                    (unsigned long long)allocs, (unsigned long long)deallocs);
        }
    }
}
...
// ========================================================================
// Main Entry Point
// Wraps verum_main() with runtime init/shutdown and argc/argv forwarding.
// ========================================================================

// The Verum compiler generates either verum_main() or verum_main_with_args(List<Text>)
// depending on the main function signature in the Verum source code
extern int verum_main(void) __attribute__((weak));
extern int verum_main_with_args(VerumList args) __attribute__((weak));
...
int main(int argc, char** argv) {
    ...
    atexit(verum_runtime_cleanup);

    // Push main function onto stack trace
    verum_push_stack_frame("main", "<entry>", 0, 0);

    int result = 0;

    // Check which variant of main was generated
    if (verum_main_with_args != NULL) {
        // main(args: List<Text>) -> Int
        VerumList args = verum_create_args_list(argc, argv);
        result = verum_main_with_args(args);
        // Clean up allocated memory
        free(args.ptr);
    } else if (verum_main != NULL) {
        // main() -> Int or main() -> ()
        result = verum_main();
    } else {
        fprintf(stderr, "Error: No main function found in Verum program\n");
        result = 1;
    }
...
"#;
    ...
    /// Compile an Objective-C file to object file (macOS Metal GPU runtime).
    #[cfg(target_os = "macos")]
    #[allow(dead_code)] // Used when Metal GPU codegen is enabled
    fn compile_objc_file(&self, source_path: &Path, output_dir: &Path) -> Result<PathBuf> {
        let output_path = output_dir
            .join(source_path.file_stem().unwrap_or_default().to_str().unwrap_or("metal"))
            .with_extension("o");

        let cc = self.detect_c_compiler()?;
        debug!("Compiling Objective-C with {}: {}", cc, source_path.display());
        ...
        cmd.arg("-c")
            ...
            .arg("-O2")
            .arg("-fPIC")
            .arg("-fobjc-arc")        // Automatic Reference Counting
            .arg("-fmodules")         // Clang modules for Metal imports
            .arg("-ffast-math");
        ...
        // Suppress warnings for Metal API usage
        cmd.arg("-Wno-deprecated-declarations");

        let output = cmd.output()?;
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(anyhow::anyhow!("Objective-C compilation failed:\n{}", stderr));
        }
    ...
    pub fn run_mlir_aot(&mut self) -> Result<PathBuf> {
        ...
        let gpu_config = GpuLoweringConfig {
            target: gpu_target,
            opt_level: self.session.options().optimization_level as u8,
        ...
        let mlir_config = MlirConfig::new("gpu_module")
            .with_optimization_level(self.session.options().optimization_level as u8)
