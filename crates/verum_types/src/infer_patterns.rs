...
impl TypeChecker {
        ...
        match &pattern.kind {
            ...
            PatternKind::Variant { path, data } => {
                ...
                if let Type::Generic { name, args } = &expanded_ty {
                    let is_unresolved_assoc = name.as_str().starts_with("::")
                        || (name.as_str().chars().next().map_or(false, |c| c.is_ascii_uppercase())
                            && !args.is_empty()
            ...
            PatternKind::TypeTest { binding, test_type } => {
                ...
                let narrowed_type = if let Some(variant_ty) = variant_resolved {
                    ...
                    match self.ast_to_type(test_type) {
                        Ok(ty) => ty,
                        Err(orig_err) => return Err(orig_err),
                    }
                };
    ...
    pub(crate) fn pattern_has_complex_forms(pattern: &verum_ast::pattern::Pattern) -> bool {
        ...
        match &pattern.kind {
            ...
            PatternKind::And(pats) => pats.iter().any(|p| Self::pattern_has_complex_forms(p)),
            PatternKind::Or(pats) => pats.iter().any(|p| Self::pattern_has_complex_forms(p)),
            PatternKind::Variant { data, .. } => {
                if let verum_common::Maybe::Some(data) = data {
                    match data {
                        verum_ast::pattern::VariantPatternData::Tuple(pats) => {
                            pats.iter().any(|p| Self::pattern_has_complex_forms(p))
                        }
                        verum_ast::pattern::VariantPatternData::Record { fields, .. } => {
                            fields.iter().any(|f| {
                                f.pattern.as_ref().map_or(false, |p| Self::pattern_has_complex_forms(p))
                            })
                        }
                    }
                } else {
