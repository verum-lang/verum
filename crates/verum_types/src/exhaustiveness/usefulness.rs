...
fn is_useful_stream(
    ...
    if cons_covered {
        ...
        let expanded_earlier: Vec<_> = earlier_rows
            .iter()
            .filter_map(|r| expand_stream_row(r))
            .collect();
...
fn matches_nil_pattern(col: &PatternColumn) -> bool {
    match col {
        ...
        PatternColumn::Or(alts) => alts.iter().any(|a| matches_nil_pattern(a)),
        _ => false,
    }
}
...
fn matches_cons_pattern(col: &PatternColumn) -> bool {
    match col {
        ...
        PatternColumn::Or(alts) => alts.iter().any(|a| matches_cons_pattern(a)),
        _ => false,
    }
}
    ...
    match col {
        ...
        PatternColumn::Range {
            ...
            if let super::matrix::LiteralPattern::Int(n) = lit {
                let n = *n as i128;
                let in_start = start.map_or(true, |s| n >= s);
                let in_end = if *inclusive {
                    end.map_or(true, |e| n <= e)
                } else {
                    end.map_or(true, |e| n < e)
                };
                in_start && in_end
...
fn range_covered_by(
    ...
    match col {
        ...
        PatternColumn::Range {
            ...
            let covers_start = s2.map_or(true, |s| start.map_or(false, |st| st >= s));
            let covers_end = e2.map_or(true, |e| {
                end.map_or(false, |en| {
