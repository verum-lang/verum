...
fn find_redundant_patterns(matrix: &CoverageMatrix) -> List<usize> {
    ...
    let rows: Vec<_> = matrix.rows.iter().cloned().collect();
...
fn find_uncovered_numeric(
    ...
    let w = match scrutinee_ty {
        ...
        _ => Witness::int(uncovered_val as i64),
    };
    Ok(List::from_iter([w]))
}
...
fn is_specialized_matrix_exhaustive(
    ...
    if first_ctors.is_infinite {
        let has_col0_wildcard = matrix.rows.iter().any(|row| {
            !row.has_guard
                && row.columns.first().map_or(false, |c| matches!(c, PatternColumn::Wildcard))
        });
        ...
        let wildcard_rows: Vec<PatternRow> = matrix
            ...
            .iter()
            .filter_map(|row| {
                if row.columns.first().map_or(false, |c| matches!(c, PatternColumn::Wildcard)) {
