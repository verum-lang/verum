                ...
                // =====================================================================
                {
                    ...
                    } = check_ty
                    {
                        ...
                        match self.check_refinement_with_evidence(expr, &refinement_type) {
                            ...
                            // An SMT `unknown` on a STATICALLY KNOWN value is
                            // not a pass (T0788).
                            //
                            ...
                            //
                            // `Unknown` on a non-literal is ordinary gradual
                            // verification — the value is not known yet, and
                            // warning there would fire on every refinement in
                            // the stdlib.  `Unknown` on a LITERAL means the
                            // predicate's own shape defeated both the solver
                            // and the syntactic evaluator, and that case used
                            // to be swallowed by a `_ => {}`: hence
                            // `Int{(it, 0).0 != (it, 0).0}` — false for every
                            // value — compiled in silence.
                            //
                            // Gradual verification is deliberate here, so this
                            // is a warning: the constraint is not enforced, and
                            // the reader is told rather than handed a type that
                            ...
                            // Valid, Unknown on a value not yet known, or an
                            // error reaching the solver.
                            _ => {}
                        }
                    }
                }
