    ...
    fn expr_has_self_method_call(&self, expr: &Expr, func_name: &Text) -> bool {
        match &expr.kind {
            ExprKind::MethodCall { receiver, method, args, .. } => {
                ...
                let is_self_call = matches!(&receiver.kind, ExprKind::Path(p) if p.as_ident().map_or(false, |i| i.name.as_str() == "self"))
    ...
    fn expr_has_guarded_recursion(&self, expr: &Expr, func_name: &Text) -> bool {
        match &expr.kind {
            ExprKind::If { then_branch, else_branch, .. } => {
                ...
                (then_calls && !else_calls) || (!then_calls && else_calls) || (then_calls && !else_calls)
            }
            ExprKind::Match { arms, .. } => {
                if arms.len() < 2 {
                    return false;
