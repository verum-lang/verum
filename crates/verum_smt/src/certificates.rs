...
impl CertificateGenerator {
    ...
    pub fn generate_stub(&self, theorem: Theorem) -> Result<Certificate, CertificateError> {
        let content = match self.format {
        ...
        if self.config.include_metadata {
            ...
            cert.metadata
                .properties
                .insert("proof_status".into(), "admitted".into());
        }
        Ok(cert)
    }
...
mod stub_certificate_tests {
    ...
    fn stub_json_includes_admitted_status() {
        ...
        assert!(
            body.contains("\"proof_status\":\"admitted\""),
            "got: {}",
            body
        );
    }
