...
impl CertificateGenerator {
    ...
    /// to the caller (see #285-Followup for that work). Users who
    ...
    /// proof term it constructed.
    pub fn generate_stub(
        &self,
        theorem: Theorem,
    ) -> Result<Certificate, CertificateError> {
        let content = match self.format {
            CertificateFormat::Coq => self.stub_coq(&theorem),
            CertificateFormat::Lean => self.stub_lean(&theorem),
            CertificateFormat::Dedukti => self.stub_dedukti(&theorem),
            CertificateFormat::Metamath => self.stub_metamath(&theorem),
            CertificateFormat::OpenTheory => self.stub_opentheory(&theorem),
            CertificateFormat::Json => self.stub_json(&theorem),
        };
        let mut cert = Certificate::new(self.format, theorem, content);
        if self.config.include_metadata {
            cert.metadata.generator_version = "verum-smt-1.0".into();
            cert.metadata.timestamp = chrono::Utc::now().to_rfc3339().into();
            cert.metadata.properties.insert(
                "proof_status".into(),
                "admitted".into(),
            );
        }
    ...
    fn stub_coq(&self, theorem: &Theorem) -> Text {
        let mut output = String::new();
        if self.config.include_comments {
            output.push_str("(* Generated by Verum SMT (stub — proof admitted) *)\n");
        ...
        output.push_str("Admitted.\n");
        output.into()
    }

    fn stub_lean(&self, theorem: &Theorem) -> Text {
        let mut output = String::new();
        if self.config.include_comments {
            output.push_str("-- Generated by Verum SMT (stub — proof admitted)\n");
            output.push_str(&format!("-- Theorem: {}\n\n", theorem.name));
    ...
    fn stub_dedukti(&self, theorem: &Theorem) -> Text {
        let mut output = String::new();
        if self.config.include_comments {
            output.push_str("(; Generated by Verum SMT (stub) ;)\n");
        }
        output.push_str(&format!(
            "{} : {} .\n",
            theorem.name, theorem.statement
    ...
    fn stub_metamath(&self, theorem: &Theorem) -> Text {
        let mut output = String::new();
        if self.config.include_comments {
            output.push_str("$( Generated by Verum SMT (stub) $)\n");
        }
    ...
    fn stub_opentheory(&self, theorem: &Theorem) -> Text {
        let mut output = String::new();
        if self.config.include_comments {
            output.push_str("# Generated by Verum SMT (stub)\n");
        }
    ...
    fn stub_json(&self, theorem: &Theorem) -> Text {
        format!(
            "{{\"theorem\":\"{}\",\"statement\":\"{}\",\"proof_status\":\"admitted\"}}\n",
            theorem.name.as_str().replace('"', "\\\""),
            theorem.statement.as_str().replace('"', "\\\""),
        ).into()
    }
...
#[cfg(test)]
mod stub_certificate_tests {
    use super::*;

    fn theorem(name: &str, statement: &str) -> Theorem {
        Theorem::new(name.into(), statement.into())
    }

    #[test]
    fn stub_lean_emits_sorry_proof_body() {
        // Pin: Lean stub uses `:= by sorry`, the canonical Lean
        // placeholder that doesn't break checker pipelines.
        let cgen = CertificateGenerator::new(CertificateFormat::Lean);
        let cert = cgen.generate_stub(theorem("test_thm", "True")).unwrap();
        let body = cert.content.as_str();
        assert!(body.contains("theorem test_thm"), "got: {}", body);
        assert!(body.contains(": True"), "got: {}", body);
        assert!(body.contains("sorry"), "got: {}", body);
        assert_eq!(cert.format, CertificateFormat::Lean);
    }

    #[test]
    fn stub_coq_emits_admitted_proof_body() {
        // Pin: Coq stub uses `Admitted.`, Coq's canonical placeholder.
        let cgen = CertificateGenerator::new(CertificateFormat::Coq);
        let cert = cgen.generate_stub(theorem("my_thm", "True")).unwrap();
        let body = cert.content.as_str();
        assert!(body.contains("Theorem my_thm"), "got: {}", body);
        assert!(body.contains(": True"), "got: {}", body);
        assert!(body.contains("Admitted."), "got: {}", body);
    }

    #[test]
    fn stub_certificate_metadata_marks_admitted() {
        // Pin: every stub carries proof_status=admitted in metadata
        // so consumers can distinguish Verum-verified-but-export-
        // stubbed certificates from full proof exports.
        let cgen = CertificateGenerator::new(CertificateFormat::Lean);
        let cert = cgen.generate_stub(theorem("t", "P")).unwrap();
        assert_eq!(
            cert.metadata.properties.get(&Text::from("proof_status"))
                .map(|v| v.as_str()),
            Some("admitted"),
            "stub certificates must mark proof_status=admitted"
        );
    }

    #[test]
    fn stub_dedukti_axiom_form() {
        // Pin: Dedukti stub uses axiom form `name : statement .`
        let cgen = CertificateGenerator::new(CertificateFormat::Dedukti);
        let cert = cgen.generate_stub(theorem("ax_thm", "Bool")).unwrap();
        let body = cert.content.as_str();
        assert!(body.contains("ax_thm : Bool ."), "got: {}", body);
    }

    #[test]
    fn stub_json_includes_admitted_status() {
        // Pin: JSON stub serializes proof_status=admitted inline so
        // pipeline consumers can branch without re-parsing metadata.
        let cgen = CertificateGenerator::new(CertificateFormat::Json);
        let cert = cgen.generate_stub(theorem("j", "stmt")).unwrap();
        let body = cert.content.as_str();
        assert!(body.contains("\"theorem\":\"j\""), "got: {}", body);
        assert!(body.contains("\"proof_status\":\"admitted\""),
            "got: {}", body);
    }

    #[test]
    fn stub_json_escapes_quotes_in_statement() {
        // Pin: JSON serialization escapes `"` so theorem statements
        // containing string literals don't produce malformed JSON.
        let cgen = CertificateGenerator::new(CertificateFormat::Json);
        let cert = cgen.generate_stub(theorem("e", "x = \"hello\"")).unwrap();
        let body = cert.content.as_str();
        // Should contain escaped quote, not raw.
        assert!(body.contains("\\\""), "expected escaped quote; got: {}", body);
    }

    #[test]
    fn stub_extension_matches_format() {
        // Pin: each format's `.extension()` produces the documented
        // file suffix so callers writing certificates to disk get
        // the correct filename.
        assert_eq!(CertificateFormat::Lean.extension(), ".lean");
        assert_eq!(CertificateFormat::Coq.extension(), ".v");
        assert_eq!(CertificateFormat::Dedukti.extension(), ".dk");
        assert_eq!(CertificateFormat::Json.extension(), ".json");
        assert_eq!(CertificateFormat::Metamath.extension(), ".mm");
        assert_eq!(CertificateFormat::OpenTheory.extension(), ".art");
    }
}
