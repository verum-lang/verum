...
fn test_raw_string_literals() {
    // Raw strings r"..." are handled by the fast parser (verum_fast_parser),
    // not the legacy event-based parser. Use regular strings here.
    assert_parses(
        r#"
        fn main() {
            let s = "This is a regular string";
        }
    "#,
    );
}
