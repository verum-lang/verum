...
fn test_declaration_attribute_intrinsic_parses() {
    // @intrinsic is accepted by the parser in expression context
    // (semantic validation rejects it later in the pipeline)
    let (ok, _warnings) = parse_expr_with_warnings("@intrinsic(\"foo\")");
    assert!(ok, "@intrinsic should be parseable (rejected later by semantic analysis)");
}
