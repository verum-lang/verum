...
// =============================================================================
// InductiveRegistry — strict-positivity-checked declarations of inductive types
// =============================================================================
//
// An inductive type is well-formed only when its own name appears
// *strictly positively* in every constructor's argument types. Allowing
// non-positive recursion (e.g. `type Bad = Wrap(Bad -> A)`) admits
// Berardi's paradox and lets the user derive `False`.
//
// The kernel enforces this at registration time via
// `InductiveRegistry::register`: every `RegisteredInductive` is
// validated by `check_strict_positivity` before it is admitted. A
// failure surfaces as `KernelError::PositivityViolation` with a
...
//     the recursive use we are admitting). args may also mention
//     `T`, but the kernel disallows `T` in args of *non-self*
//     inductives — that prevents indirect non-positive recursion via
//     `BadList = Cons(BadList -> A, BadList)` where `Cons`'s first
//     arg is a function whose negative position has `BadList`.
