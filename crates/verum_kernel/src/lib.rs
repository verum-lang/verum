...
/// — the onboarding aid that answers "what tactics did I write?"
/// in a serialisable form. Future work will plug live elaborator state through
/// the same surface.
pub mod proof_view;
pub use proof_view::{
...
/// `check_eps_mu_coherence` with staging.
pub mod eps_mu;
...
/// `K-Round-Trip` kernel rule (/V2) — OC/DC translation round-trip
...
/// (paragraph + theorem number) and is consumed by `K-Round-Trip`
/// to make preprint dependencies explicit at the kernel surface.
/// Future work removes admits as the preprint resolves.
...
/// Native (∞,n)-categorical kernel infrastructure . No mainstream
...
/// `lurie_htt_5_1_4_syn_is_grothendieck` framework axiom; ships an
...
/// equivalence (HTT 3.2.0.1) — algorithmic kernel rules. Gates
...
/// generalised) — algorithmic kernel rule. The decidable
