...
///  — free-variable preservation (): every 2-functor M in
...
///  β-normalisation invariance (): the naturality square
///  commutes up to β-equivalence; if `definitional_eq(M_α, α)`
///  holds the M-action is a β-redex that normalises away — the
///  identity sub-case extends to any term sharing a normal
///  form. When all three gates pass, accepts;
///  full *sufficient* witness construction (σ_α from Code_S
///  morphism + π_α from Perform_{ε_math} naturality through
///  axiom A-3) is the step still tracked under #181.
///  rejects strictly more terms than V2 (every
...
/// sufficient construction will *not* widen this
        ...
        (CoreTerm::EpsilonOf(m_alpha), CoreTerm::AlphaOf(inner_rhs)) => {
            match inner_rhs.as_ref() {
                CoreTerm::EpsilonOf(alpha_rhs) => {
                    // sufficient witness: identity-functor case
                    ...
                    // gate (c): β-normalisation invariance.
                    // Extends the identity sub-case from
                    ...
                    // gate (b): free-variable preservation.
...
pub fn check_eps_mu_coherence_v3_final(
    ...
    // Step 1: run gates. Any failure surfaces directly.
...
mod v3_final_tests {
    ...
    fn v3_final_admits_canonical_naturality_identity_with_empty_audit() {
        // (EpsilonOf(F), AlphaOf(EpsilonOf(F))) — identity sub-case.
        let f = var("F");
        let lhs = eps(f.clone());
    ...
    fn v3_final_records_a_3_for_non_identity_canonical_pair() {
        ...
        // Both F and G are atomic Var with depth 0 — passes
        // gate (a). free_vars(F) = {F}, free_vars(G) = {G} — they DIFFER,
        // so REJECTS (gate b). This means it shouldn't
        ...
        // Not β-equal (no Lam to reduce). admits.
        // routes through the bridge admit.
    ...
    fn v3_final_rejects_depth_mismatch() {
        // rejects on depth mismatch — must
        // surface the same rejection (is strictly stronger,
        // not strictly weaker).
        // ModalBox bumps depth by 1; if lhs has Box and rhs doesn't,
        // depths differ → rejects via gate (a).
        let f = var("F");
    ...
    fn v3_final_rejects_malformed_pair() {
        ...
        // not an EpsilonOf, so rejects shape.
        let lhs = eps(var("F"));
        let rhs = alpha(var("G"));
