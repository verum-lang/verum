...
fn main() {
    ...
    match arg.as_str() {
        "lean" => print!("{}", exporter.emit::<LeanBackend>(&LeanBackend)),
        "coq" => print!("{}", exporter.emit::<CoqBackend>(&CoqBackend)),
        "isabelle" => print!(
            "{}",
            exporter.emit::<IsabelleBackend>(&IsabelleBackend)
        ),
        "agda" => print!("{}", exporter.emit::<AgdaBackend>(&AgdaBackend)),
        _ => {
