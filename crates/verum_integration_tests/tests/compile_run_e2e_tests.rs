...
fn e2e_multiple_return_types() {
    ...
    assert!(
        stdout.contains("true"),
        "Expected true for 4, got: {:?}",
        stdout
    );
    assert!(
        stdout.contains("false"),
        "Expected false for 7, got: {:?}",
        stdout
    );
}
...
fn e2e_tuple_creation() {
    ...
    assert!(
        !stdout.is_empty(),
        "Expected some output for tuple, got empty"
    );
}
...
fn e2e_empty_program_no_output() {
    ...
    assert!(
        ...
        "Expected no output from empty main, got: {:?}",
        stdout
    );
}
