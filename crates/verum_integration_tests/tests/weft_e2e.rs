...
#[test]
fn interpreter_result_combinators_native_dispatch() {
    let dir = tempfile::tempdir().expect("tempdir");
    let body = r#"// Auto-generated by weft_e2e — direct Result.map_err probe.
fn main() {
    let ok:  Result<Int, Text> = Ok(42);
    let err: Result<Int, Text> = Err("hello");

    let id_ok  = ok.map_err(|e| f"prefixed_{e}");
    let mapped = err.map_err(|e| f"prefixed_{e}");
    let plus   = ok.map(|v| v + 100);

    match id_ok {
        Ok(v)  => print(f"OK_IDENTITY:{v}"),
        Err(_) => print("OK_BUG"),
    }
    match mapped {
        Ok(_)  => print("ERR_BUG"),
        Err(e) => print(f"ERR_MAPPED:{e}"),
    }
    match plus {
        Ok(v)  => print(f"MAP_OK:{v}"),
        Err(_) => print("MAP_BUG"),
    }
}
"#;
    let path = write_program(dir.path(), "result_combinators.vr", body);
    let out = run_verum(&["run", path.to_str().unwrap()], dir.path());
    assert!(
        out.status.success(),
        "verum run failed:\nstdout:\n{}\nstderr:\n{}",
    ...
    assert!(
        combined.contains("OK_IDENTITY:42"),
        "Ok.map_err must be identity; got:\n{}",
        combined,
    );
    assert!(
        combined.contains("ERR_MAPPED:prefixed_hello"),
        "Err.map_err must apply closure; got:\n{}",
        combined,
    );
    assert!(
        combined.contains("MAP_OK:142"),
        "Ok.map must apply closure; got:\n{}",
        combined,
    );
}

/// Pinned for task #17 — the OS-level bind probe. Result.map_err
/// dispatch (task #12) is now wired so the interpreter no longer
/// panics with "method not found", but the actual `bind`
/// syscall returns errno 4294967278 (= 0xFFFFFFEE). Investigated
/// separately. Gated behind `WEFT_RUN_BIND_PROBES=1` until #17
/// lands.
#[test]
fn interpreter_bind_pinned_known_failure() {
    if std::env::var_os("WEFT_RUN_BIND_PROBES").is_none() {
        eprintln!("skipped (set WEFT_RUN_BIND_PROBES=1 once task #17 lands)");
        return;
    }
    let dir = tempfile::tempdir().expect("tempdir");
    ...
    assert!(
        ...
        "expected BIND_OK after task #17 lands; got:\n{}",
        combined,
    );
}
