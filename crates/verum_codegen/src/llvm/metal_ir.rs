...
impl<'ctx> MetalIR<'ctx> {
    ...
    fn build_objc_msg_send(
        ...
        // Get selector
        let sel_fn = module
            .get_function("sel_registerName")
            .or_missing_fn("sel_registerName")?;
        let sel_str =
            self.build_global_str(module, builder, selector_name, &format!("sel_{}", name));
        let sel = builder
        ...
        let msg_send = module
            .get_function("objc_msgSend")
            .or_missing_fn("objc_msgSend")?;
        let result = builder
            .build_call(msg_send, &all_args, name)
            .or_llvm_err()?;
    ...
    fn build_objc_msg_send_i64(
        ...
        let result_ptr =
            self.build_objc_msg_send(module, builder, receiver, selector_name, args, name)?;
        // Cast ptr to i64 (ptrtoint)
        Ok(builder
            .build_ptr_to_int(result_ptr, i64_type, &format!("{}_i64", name))
            .or_llvm_err()?)
    }
    ...
    fn build_global_str(
        ...
        let mut vals: Vec<verum_llvm::values::IntValue<'ctx>> = bytes
            .iter()
    ...
    fn emit_objc_runtime_decls(&self, module: &Module<'ctx>) {
        ...
        if module
            .get_function("MTLCreateSystemDefaultDevice")
            .is_none()
        {
    ...
    fn emit_ensure_init(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let init_global = module
            .get_global("__verum_metal_initialized")
            .or_internal("missing global")?;
        let init_val = builder
            .build_load(i64_type, init_global.as_pointer_value(), "init_flag")
            .or_llvm_err()?
            .into_int_value();
        let is_init = builder
            .build_int_compare(
        ...
        builder
            .build_conditional_branch(cas_success, init_body, done)
            .or_llvm_err()?;

        // init_body: call MTLCreateSystemDefaultDevice(), store device, create queue
        builder.position_at_end(init_body);

        let create_device_fn = module
            .get_function("MTLCreateSystemDefaultDevice")
            .or_missing_fn("MTLCreateSystemDefaultDevice")?;
        let device = builder
        ...
        builder
            .build_conditional_branch(device_null, init_fail, store_dev)
            .or_llvm_err()?;

        // init_fail: reset flag to 0, return
        builder.position_at_end(init_fail);
        builder
            .build_store(init_global.as_pointer_value(), i64_type.const_int(0, false))
            .or_llvm_err()?;
        ...
        let dev_global = module
            .get_global("__verum_metal_device")
            .or_internal("missing global")?;
        builder
            .build_store(dev_global.as_pointer_value(), device)
            .or_llvm_err()?;

        // queue = objc_msgSend(device, "newCommandQueue")
        let queue =
            self.build_objc_msg_send(module, &builder, device, "newCommandQueue", &[], "queue")?;
        let queue_global = module
            .get_global("__verum_metal_queue")
            .or_internal("missing global")?;
        builder
            .build_store(queue_global.as_pointer_value(), queue)
            .or_llvm_err()?;
    ...
    fn emit_get_device(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        // ensure_init()
        let init_fn = self.get_or_declare(
            module,
            "verum_metal_ensure_init",
        ...
        let dev_global = module
            ...
            .or_internal("missing global")?;
        let dev = builder
            .build_load(ptr_type, dev_global.as_pointer_value(), "dev")
            .or_llvm_err()?
            .into_pointer_value();
    ...
    fn emit_device_name(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        builder
            .build_conditional_branch(is_null, no_dev, have_dev)
            .or_llvm_err()?;
    ...
    fn emit_max_memory(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        builder.position_at_end(have_dev);
        let result = self.build_objc_msg_send_i64(
            module,
            &builder,
            dev,
    ...
    fn emit_max_threads_per_threadgroup(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let fn_type = i64_type.fn_type(&[], false);
        let func = match self.get_or_declare_new(
            module,
    ...
    fn emit_supports_family(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let family_ptr = builder
            .build_int_to_ptr(family, ptr_type, "fam_ptr")
            .or_llvm_err()?;
        let result_ptr = self.build_objc_msg_send(
            module,
            &builder,
            dev,
            "supportsFamily:",
    ...
    fn emit_alloc(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let both_ok = builder
            .build_and(size_ok, dev_ok, "both_ok")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(both_ok, valid, invalid)
            .or_llvm_err()?;
        ...
        let size_ptr = builder
            .build_int_to_ptr(size, ptr_type, "sz_ptr")
            .or_llvm_err()?;
        let zero_ptr = ptr_type.const_null();
        let buf = self.build_objc_msg_send(
            module,
            &builder,
            dev,
            "newBufferWithLength:options:",
            &[size_ptr.into(), zero_ptr.into()],
            "buf",
        )?;
        ...
        builder
            .build_conditional_branch(buf_null, null_ret, retain_bb)
            .or_llvm_err()?;
        ...
        let retain_fn = module
            .get_function("objc_retain")
            .or_missing_fn("objc_retain")?;
    ...
    fn emit_alloc_with_data(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        builder
            .build_conditional_branch(all_ok, valid, invalid)
            .or_llvm_err()?;
        ...
        let buf = self.build_objc_msg_send(
            ...
            dev,
            "newBufferWithBytes:length:options:",
            &[data_ptr.into(), size_ptr.into(), zero_ptr.into()],
            "buf",
        )?;
    ...
    fn emit_buffer_contents(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let is_zero = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_zero, invalid, valid)
            .or_llvm_err()?;
        ...
        let contents =
            self.build_objc_msg_send(module, &builder, buf_ptr, "contents", &[], "contents")?;
    ...
    fn emit_free(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let is_zero = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_zero, done, valid)
            .or_llvm_err()?;

        builder.position_at_end(valid);
        let ptr = self.handle_to_ptr(&builder, handle, "buf_ptr")?;
        let release_fn = module
            .get_function("objc_release")
            .or_missing_fn("objc_release")?;
        builder
    ...
    fn emit_compile_shader(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let either_null = builder
            .build_or(dev_null, src_null, "either_null")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(either_null, invalid, valid)
            .or_llvm_err()?;
        ...
        let get_class_fn = module
            .get_function("objc_getClass")
            .or_missing_fn("objc_getClass")?;
        ...
        let nsstr_alloc = self.build_objc_msg_send(
            module,
            &builder,
            nsstring_class,
            "alloc",
        ...
        let len_ptr = builder
            .build_int_to_ptr(len, ptr_type, "len_ptr")
            .or_llvm_err()?;
        let encoding = builder
            .build_int_to_ptr(i64_type.const_int(4, false), ptr_type, "enc_ptr")
            .or_llvm_err()?; // NSUTF8StringEncoding = 4
        let nsstr = self.build_objc_msg_send(
            module,
            &builder,
            nsstr_alloc,
            "initWithBytes:length:encoding:",
            &[source.into(), len_ptr.into(), encoding.into()],
            "nsstr",
        )?;

        // MTLCompileOptions *options = [MTLCompileOptions new]
        let mtl_opts_name =
            self.build_global_str(module, &builder, "MTLCompileOptions", "mtl_opts_cls");
        ...
        let lib = self.build_objc_msg_send(
            module,
            &builder,
            dev,
            "newLibraryWithSource:options:error:",
            &[nsstr.into(), options.into(), null_ptr.into()],
            "lib",
        )?;
        ...
        builder
            .build_conditional_branch(lib_null, fail_bb, retain_bb)
            .or_llvm_err()?;
    ...
    fn emit_get_pipeline(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let mtl_func = self.build_objc_msg_send(
            module,
            &builder,
            lib_ptr,
            "newFunctionWithName:",
        ...
        builder
            .build_conditional_branch(func_null, no_func, make_pipeline)
            .or_llvm_err()?;
        ...
        let null_ptr = ptr_type.const_null();
        let pipeline = self.build_objc_msg_send(
            module,
            &builder,
            dev,
            "newComputePipelineStateWithFunction:error:",
            &[mtl_func.into(), null_ptr.into()],
            "pipeline",
        )?;
        ...
        let fail_bb = ctx.append_basic_block(func, "fail");
        builder
            .build_conditional_branch(pipe_null, fail_bb, retain_bb)
            .or_llvm_err()?;
    ...
    fn build_dispatch_common(
        ...
        let queue_global = module
            ...
            .or_internal("missing global")?;
        let queue = builder
            .build_load(ptr_type, queue_global.as_pointer_value(), "queue")
            .or_llvm_err()?
            .into_pointer_value();
        let cmd_buf =
            self.build_objc_msg_send(module, builder, queue, "commandBuffer", &[], "cmd_buf")?;

        // encoder = objc_msgSend(cmdBuf, "computeCommandEncoder")
        let encoder = self.build_objc_msg_send(
            module,
            builder,
            cmd_buf,
        ...
        let elem_ptr = unsafe {
            builder
                .build_in_bounds_gep(i64_type, buffers_ptr, &[i_val], "elem_ptr")
                .or_llvm_err()?
        };
        let buf_handle = builder
            .build_load(i64_type, elem_ptr, "buf_handle")
            .or_llvm_err()?
            .into_int_value();
        ...
        let zero_ptr = ptr_type.const_null();
        let idx_ptr = builder
            .build_int_to_ptr(i_val, ptr_type, "idx_ptr")
            .or_llvm_err()?;
        self.build_objc_msg_send(
            module,
            builder,
            ...
            &[buf_ptr.into(), zero_ptr.into(), idx_ptr.into()],
            "set_buf",
        )?;
        ...
        let one = i64_type.const_int(1, false);
        let grid_x_ptr = builder
            .build_int_to_ptr(grid_dims[0], ptr_type, "gx_ptr")
            .or_llvm_err()?;
        let grid_y_ptr = if grid_dims.len() > 1 {
            builder
                .build_int_to_ptr(grid_dims[1], ptr_type, "gy_ptr")
                .or_llvm_err()?
        } else {
            builder
                .build_int_to_ptr(one, ptr_type, "gy_ptr")
                .or_llvm_err()?
        };
        let grid_z_ptr = builder
            .build_int_to_ptr(one, ptr_type, "gz_ptr")
            .or_llvm_err()?;

        let group_x_ptr = builder
            .build_int_to_ptr(group_dims[0], ptr_type, "tgx_ptr")
            .or_llvm_err()?;
        let group_y_ptr = if group_dims.len() > 1 {
            builder
                .build_int_to_ptr(group_dims[1], ptr_type, "tgy_ptr")
                .or_llvm_err()?
        } else {
            builder
                .build_int_to_ptr(one, ptr_type, "tgy_ptr")
                .or_llvm_err()?
        };
        let group_z_ptr = builder
            .build_int_to_ptr(one, ptr_type, "tgz_ptr")
            .or_llvm_err()?;
    ...
    fn emit_dispatch_1d(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let pipeline_handle = func
            .get_nth_param(0)
            .or_internal("missing param")?
            .into_int_value();
        let buffers_ptr = func
            .get_nth_param(1)
            .or_internal("missing param")?
            .into_int_value();
        let buffer_count = func
            .get_nth_param(2)
            .or_internal("missing param")?
            .into_int_value();
        let grid_size = func
        ...
        let pipe_zero = builder
            ...
            .or_llvm_err()?;
        let bad = builder
            .build_or(queue_null, pipe_zero, "bad")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(bad, done, valid)
            .or_llvm_err()?;
        ...
        let actual_tg = builder
            .build_select(tg_ok, tg_size, default_tg, "actual_tg")
            .or_llvm_err()?
            .into_int_value();
    ...
    fn emit_dispatch_2d(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let gx_ok = builder
            ...
            .or_llvm_err()?;
        let actual_gx = builder
            .build_select(gx_ok, group_x, default_16, "actual_gx")
            .or_llvm_err()?
            .into_int_value();
        let gy_ok = builder
            .build_int_compare(
            ...
            .or_llvm_err()?;
        let actual_gy = builder
            .build_select(gy_ok, group_y, default_16, "actual_gy")
            .or_llvm_err()?
            .into_int_value();
    ...
    fn emit_wait(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let release_fn = module
            ...
            .or_missing_fn("objc_release")?;
        builder
            .build_call(release_fn, &[cb_ptr.into()], "")
            .or_llvm_err()?;
        builder.build_unconditional_branch(done).or_llvm_err()?;
    ...
    fn emit_execution_time_ns(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let gpu_time = builder
            .build_float_sub(end_time, start_time, "gpu_time")
            .or_llvm_err()?;
        // Convert to nanoseconds: * 1e9
        let ns_factor = f64_type.const_float(1e9);
        let ns_float = builder
            .build_float_mul(gpu_time, ns_factor, "ns_float")
            .or_llvm_err()?;
        let ns_i64 = builder
            .build_float_to_signed_int(ns_float, i64_type, "ns_i64")
            .or_llvm_err()?;
    ...
    fn emit_vector_add_f32(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let compile_fn = self.get_or_declare(
            module,
            "verum_metal_compile_shader",
        ...
        let get_pipe_fn = self.get_or_declare(
            module,
            "verum_metal_get_pipeline",
        ...
        let arr_alloca = builder
            .build_array_alloca(i64_type, i64_type.const_int(3, false), "bufs")
            .or_llvm_err()?;
        let idx0 = unsafe {
            builder
                .build_in_bounds_gep(i64_type, arr_alloca, &[i64_type.const_int(0, false)], "p0")
                .or_llvm_err()?
        };
        builder.build_store(idx0, a).or_llvm_err()?;
        let idx1 = unsafe {
            builder
                .build_in_bounds_gep(i64_type, arr_alloca, &[i64_type.const_int(1, false)], "p1")
                .or_llvm_err()?
        };
        builder.build_store(idx1, b).or_llvm_err()?;
        let idx2 = unsafe {
            builder
                .build_in_bounds_gep(i64_type, arr_alloca, &[i64_type.const_int(2, false)], "p2")
                .or_llvm_err()?
        };
        ...
        let dispatch_fn = self.get_or_declare(
            module,
    ...
    fn emit_sgemm(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        let m_trunc = builder
            .build_int_truncate(m_val, i32_type, "m32")
            .or_llvm_err()?;
        ...
        let n_alloca = builder
            .build_alloca(i32_type, "n_val_alloc")
            .or_llvm_err()?;
        let n_trunc = builder
            .build_int_truncate(n_val, i32_type, "n32")
            .or_llvm_err()?;
        ...
        let k_alloca = builder
            .build_alloca(i32_type, "k_val_alloc")
            .or_llvm_err()?;
        let k_trunc = builder
            .build_int_truncate(k_val, i32_type, "k32")
            .or_llvm_err()?;
        ...
        let arr = builder
            .build_array_alloca(i64_type, i64_type.const_int(6, false), "bufs")
            .or_llvm_err()?;
        for (idx, val) in [a_buf, b_buf, c_buf, m_buf_h, n_buf_h, k_buf_h]
            .iter()
            .enumerate()
        {
            let p = unsafe {
    ...
    fn emit_benchmark(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        builder
            .build_conditional_branch(ok, warmup, invalid)
            .or_llvm_err()?;
        ...
        let cmp = builder
            .build_int_compare(IntPredicate::ULT, iter_val, iterations, "cmp")
            .or_llvm_err()?;
        builder
        ...
        // dispatch_async
        let async_fn = self.get_or_declare(
            module,
        ...
        // wait
        let wait_fn = self.get_or_declare(
            module,
            "verum_metal_wait",
            ctx.void_type().fn_type(&[i64_type.into()], false),
        );
        builder
            .build_call(wait_fn, &[cb_handle.into()], "")
            .or_llvm_err()?;

        // execution_time_ns
        let time_fn = self.get_or_declare(
            module,
            "verum_metal_execution_time_ns",
        ...
        let new_total = builder
            ...
            .or_llvm_err()?;
        let next_iter = builder
            .build_int_add(iter_val, i64_type.const_int(1, false), "next_iter")
            .or_llvm_err()?;
        ...
        let avg = builder
            .build_int_signed_div(total_val, iterations, "avg")
            .or_llvm_err()?;
