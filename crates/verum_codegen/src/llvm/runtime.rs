...
impl<'ctx> RuntimeLowering<'ctx> {
    ...
    pub fn emit_misc_ir_functions(&self, module: &Module<'ctx>) -> Result<()> {
        ...
        // The legacy body itself was DELETED under the T0438 census
        // (nine dead duplicate definers, bucket [B]) — see the pointer
        // banner where it lived and the architecture decision in
    ...
    // =========================================================================
    // TCP/UDP networking lives in `platform_ir.rs` — the canonical no-libc
    // path (#96). The LEGACY libc-based `emit_verum_networking_functions`
    // body (disabled at its step! site since #96) was DELETED under the
    // T0438 census: nine dead duplicate `verum_tcp_*`/`verum_udp_*`
    // definers, bucket [B].
    // =========================================================================
