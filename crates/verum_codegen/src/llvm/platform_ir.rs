...
impl<'ctx> PlatformIR<'ctx> {
    ...
    /// verbatim, duplicating `RuntimeLowering::emit_linux_syscall`.
    ...
    fn emit_raw_open3(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let fd_i64 = builder
            .build_int_s_extend(fd_i32, i64_type, "fd64")
            .or_llvm_err()?;
    ...
    fn emit_allocator(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let cur_ptr = builder
            ...
            .into_pointer_value();
        let cur_end = builder
            .build_load(ptr_type, arena_end.as_pointer_value(), "cur_end")
            .or_llvm_err()?
            .into_pointer_value();
        ...
        let ptr_not_null = builder
            .build_int_compare(
                IntPredicate::NE,
                builder
                    .build_ptr_to_int(cur_ptr, i64_type, "cur_i64")
                    .or_llvm_err()?,
                i64_type.const_zero(),
                "not_null",
            )
            .or_llvm_err()?;
        let new_ptr_i64 = builder
            .build_ptr_to_int(new_ptr, i64_type, "new_i64")
            .or_llvm_err()?;
        let end_i64 = builder
            .build_ptr_to_int(cur_end, i64_type, "end_i64")
            .or_llvm_err()?;
        let fits = builder
            .build_int_compare(IntPredicate::ULE, new_ptr_i64, end_i64, "fits")
            .or_llvm_err()?;
        let can_bump = builder
            .build_and(ptr_not_null, fits, "can_bump")
            .or_llvm_err()?;
        builder
        ...
        let block_not_null = builder
            .build_int_compare(
                IntPredicate::NE,
                builder
                    .build_ptr_to_int(new_block, i64_type, "blk_i64")
                    .or_llvm_err()?,
                i64_type.const_zero(),
                "blk_ok",
            )
            .or_llvm_err()?;
        builder
            .build_conditional_branch(block_not_null, new_arena_ok, alloc_fail)
            .or_llvm_err()?;

        // ---- new_arena_ok: set up new arena and retry bump ----
        builder.position_at_end(new_arena_ok);
        builder
            .build_store(arena_base.as_pointer_value(), new_block)
            .or_llvm_err()?;
        builder
        ...
        builder
            .build_store(arena_end.as_pointer_value(), new_end)
            .or_llvm_err()?;
    ...
    fn emit_verum_os_write(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        if target_is_windows(module) {
            ...
            let fd_i32 = builder
                .build_int_truncate(fd.into_int_value(), i32_type, "fd32")
                .or_llvm_err()?;
            ...
            let handle_ptr = match handle {
                BasicValueEnum::PointerValue(p) => p,
                _ => {
                    return Err(super::error::LlvmLoweringError::internal(
                        "GetStdHandle did not return a pointer".to_string(),
                    ));
                }
            };
            let is_null = builder.build_is_null(handle_ptr, "h_null").or_llvm_err()?;
            ...
            builder
                .build_return(Some(&count.into_int_value()))
                .or_llvm_err()?;
            ...
            builder
                .build_store(written_ptr, i32_type.const_zero())
                .or_llvm_err()?;

            let count_i32 = builder
                .build_int_truncate(count.into_int_value(), i32_type, "cnt32")
                .or_llvm_err()?;
            ...
            let written_val = builder
                .build_load(i32_type, written_ptr, "wr")
                .or_llvm_err()?;
            let result = builder
                .build_int_z_extend(written_val.into_int_value(), i64_type, "wr64")
                .or_llvm_err()?;
            builder.build_return(Some(&result)).or_llvm_err()?;
        } else {
    ...
    fn emit_verum_os_exit(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        if target_is_windows(module) {
            ...
            let code_u32 = builder
                .build_int_cast(code, i32_type, "code32")
                .or_llvm_err()?;
            builder
                .build_call(exit_process, &[code_u32.into()], "")
                .or_llvm_err()?;
        } else {
            let exit_fn = module
            ...
            let code64 = builder
                .build_int_s_extend(code, i64_type, "c64")
                .or_llvm_err()?;
            builder
                .build_call(exit_fn, &[code64.into()], "")
                .or_llvm_err()?;
        }
    ...
    fn emit_main_entry(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        if let Some(store_fn) = module.get_function("verum_store_args") {
            builder
                .build_call(store_fn, &[argc.into(), argv.into()], "")
                .or_llvm_err()?;
        }
        // Initialize runtime
        ...
        if let Some(vm) = verum_main {
            builder
                .build_unconditional_branch(call_main)
                .or_llvm_err()?;

            builder.position_at_end(call_main);
            let result = builder
                .build_call(vm, &[], "result")
                .or_llvm_err()?
                .try_as_basic_value()
                .basic()
            ...
            let ret_val = match result {
                BasicValueEnum::IntValue(i) => {
                    if i.get_type().get_bit_width() > 32 {
                        builder
                            .build_int_truncate(i, i32_type, "ret32")
                            .or_llvm_err()?
                    } else if i.get_type().get_bit_width() < 32 {
                        builder
                            .build_int_z_extend(i, i32_type, "ret32")
                            .or_llvm_err()?
                    } else {
                        i
                    }
        ...
        if let Some(main_fn) = module.get_function("main") {
            ...
            let exit_code = match result {
                BasicValueEnum::IntValue(i) => {
                    if i.get_type().get_bit_width() != 32 {
                        builder
                            .build_int_truncate(i, i32_type, "ec32")
                            .or_llvm_err()?
                    } else {
                        i
                    }
            ...
            builder
                .build_call(exit_process, &[exit_code.into()], "")
                .or_llvm_err()?;
        }
    ...
    pub fn emit_runtime_globals(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        if module
            ...
            .is_none()
        {
            let g = module.add_global(i64_type, None, "__verum_runtime_async_worker_threads");
            g.set_initializer(
                &i64_type.const_int(self.runtime_bridge.async_worker_threads as u64, false),
            );
            g.set_linkage(verum_llvm::module::Linkage::Internal);
        }
        if module
            ...
            .is_none()
        {
            let g = module.add_global(i64_type, None, "__verum_runtime_task_stack_size");
            g.set_initializer(&i64_type.const_int(self.runtime_bridge.task_stack_size, false));
            g.set_linkage(verum_llvm::module::Linkage::Internal);
        }
        Ok(())
    }
    ...
    pub fn emit_runtime_bridge_getters(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        let ctx = self.context;
        let i64_type = ctx.i64_type();
        let fn_type = i64_type.fn_type(&[], false);
    ...
    pub fn emit_get_argc(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        if let Some(argc_global) = module.get_global("__verum_argc") {
            let val = builder
                ...
                .or_llvm_err()?;
            let ext = builder
                .build_int_z_extend(val.into_int_value(), i64_type, "argc64")
                .or_llvm_err()?;
    ...
    pub fn emit_get_argv(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        if let Some(argv_global) = module.get_global("__verum_argv") {
            ...
            let elem_ptr = unsafe {
                builder
                    .build_in_bounds_gep(ptr_type, argv, &[index], "arg_ptr")
                    .or_llvm_err()?
            };
            let arg = builder
                .build_load(ptr_type, elem_ptr, "arg")
                .or_llvm_err()?;
    ...
    pub fn emit_stack_frame_stubs(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let push_type = void_type.fn_type(
            &[
                ptr_type.into(),
                ptr_type.into(),
                i32_type.into(),
    ...
    pub fn emit_sync_primitives(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        if init_fn.count_basic_blocks() == 0 {
            ...
            let m = init_fn
                ...
                .into_pointer_value();
            builder
                .build_store(m, i32_type.const_zero())
                .or_llvm_err()?;
            builder.build_return(None).or_llvm_err()?;
        }
        ...
        if trylock_fn.count_basic_blocks() == 0 {
            ...
            let cas = builder
                ...
                .or_llvm_err()?;
            let success = builder
                .build_extract_value(cas, 1, "ok")
                .or_llvm_err()?
                .into_int_value();
            let result = builder
                .build_int_z_extend(success, i64_type, "result")
                .or_llvm_err()?;
            builder.build_return(Some(&result)).or_llvm_err()?;
        }
        ...
        if cond_init_fn.count_basic_blocks() == 0 {
            ...
            let cv = cond_init_fn
                ...
                .into_pointer_value();
            builder
                .build_store(cv, i32_type.const_zero())
                .or_llvm_err()?;
            builder.build_return(None).or_llvm_err()?;
        }
    ...
    fn emit_mutex_lock(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let cas = builder
            ...
            .or_llvm_err()?;
        let cas_ok = builder
            .build_extract_value(cas, 1, "fast_ok")
            .or_llvm_err()?
            .into_int_value();
        builder
            .build_conditional_branch(cas_ok, locked, contended)
            .or_llvm_err()?;
        ...
        let _prev = builder
            .build_atomicrmw(
                verum_llvm::AtomicRMWBinOp::Xchg,
                m,
                two,
                verum_llvm::AtomicOrdering::Acquire,
            )
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(spin_loop)
            .or_llvm_err()?;
        ...
        let exchanged = builder
            ...
            .or_llvm_err()?;
        let got_lock = builder
            .build_int_compare(IntPredicate::EQ, exchanged, zero, "got")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(got_lock, locked, spin_loop)
            .or_llvm_err()?;
    ...
    fn emit_mutex_unlock(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let prev = builder
            ...
            .or_llvm_err()?;
        let was_two = builder
            .build_int_compare(IntPredicate::EQ, prev, two, "was2")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(was_two, was_contended, done)
            .or_llvm_err()?;
        ...
        builder
            .build_call(
                futex_wake,
                &[m_as_i64.into(), i64_type.const_int(1, false).into()],
                "",
            )
            .or_llvm_err()?;
    ...
    fn emit_cond_signal(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_call(
                futex_wake,
                &[cv_i64.into(), i64_type.const_int(1, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder.build_return(None).or_llvm_err()?;
    ...
    fn emit_cond_broadcast(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_call(
                futex_wake,
                &[cv_i64.into(), i64_type.const_int(0x7FFFFFFF, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder.build_return(None).or_llvm_err()?;
    ...
    fn emit_threading(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let decls: &[(&str, verum_llvm::types::FunctionType<'ctx>)] = &[
            ...
            (
                "verum_thread_is_done",
                i64_type.fn_type(&[i64_type.into()], false),
            ),
            (
                "verum_thread_get_result",
                i64_type.fn_type(&[i64_type.into()], false),
            ),
        ];
    ...
    fn emit_cbgr_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        // ThinRef layout at ptr: { user_ptr: ptr(8), generation: i32(4), epoch_caps: i32(4) }
        {
            ...
            if func.count_basic_blocks() > 0 { /* already emitted */
                ...
                let user_ptr = builder
                    .build_load(i64_type, ref_ptr, "user_ptr")
                    .or_llvm_err()?
                    .into_int_value();
                ...
                let generation = builder
                    .build_load(i32_type, gen_ptr, "generation")
                    .or_llvm_err()?
                    .into_int_value();
                ...
                let epoch_caps = builder
                    .build_load(i32_type, caps_ptr, "epoch_caps")
                    .or_llvm_err()?
                    .into_int_value();
                ...
                let gen_i64 = builder
                    .build_int_z_extend(generation, i64_type, "gen_i64")
                    .or_llvm_err()?;
        ...
        // verum_cbgr_check_write(ref: ptr) -> i1: validate ref AND check write capability
        {
            ...
            if func.count_basic_blocks() > 0 { /* already emitted */
                ...
                let caps_shifted = builder
                    ...
                    .or_llvm_err()?;
                let caps_i16 = builder
                    .build_int_truncate(caps_shifted, i16_type, "caps_i16")
                    .or_llvm_err()?;

                // Check write capability (bit 1 of caps)
                let write_bit = builder
                    .build_and(caps_i16, i16_type.const_int(0x02, false), "write_bit")
                    .or_llvm_err()?;
                let has_write = builder
                    .build_int_compare(
                        IntPredicate::NE,
                        write_bit,
                        i16_type.const_int(0, false),
                        "has_write",
                    )
                    .or_llvm_err()?;
            ...
            .basic_value_or("validate_ref: expected return value")?
                ...
                let result = builder
                    .build_and(ref_valid, has_write, "valid_and_writable")
                    .or_llvm_err()?;
                builder.build_return(Some(&result)).or_llvm_err()?;
            }
        }
        ...
        // verum_cbgr_epoch_begin(): increment global epoch with overflow detection
        {
            ...
            if func.count_basic_blocks() > 0 { /* already emitted */
                ...
                // Load current epoch
                let current = builder
                    .build_load(i64_type, epoch_global.as_pointer_value(), "cur_epoch")
                    .or_llvm_err()?
                    .into_int_value();
                ...
                let is_overflow = builder
                    .build_int_compare(
                        ...
                        current,
                        max_epoch,
                        "epoch_overflow_check",
                    )
                    .or_llvm_err()?;

                builder
                    .build_conditional_branch(is_overflow, overflow_bb, ok_bb)
                    .or_llvm_err()?;
                ...
                let incremented = builder
                    .build_int_add(current, i64_type.const_int(1, false), "new_epoch")
                    .or_llvm_err()?;
                builder
                    .build_store(epoch_global.as_pointer_value(), incremented)
                    .or_llvm_err()?;
        ...
        // Returns (actual_gen == expected_gen) && (actual_epoch == expected_epoch)
        {
            ...
            if func.count_basic_blocks() > 0 { /* already emitted */
                ...
                let user_ptr = func
                    ...
                    .or_internal("missing param 0")?
                    .into_int_value();
                let expected_packed = func
                    .get_nth_param(1)
                    .or_internal("missing param 1")?
                    .into_int_value();

                // Extract expected generation (low 32 bits)
                let expected_gen = builder
                    .build_int_truncate(expected_packed, i32_type, "expected_gen")
                    .or_llvm_err()?;
                ...
                let shifted = builder
                    ...
                    .or_llvm_err()?;
                let expected_epoch = builder
                    .build_int_truncate(shifted, i16_type, "expected_epoch")
                    .or_llvm_err()?;
                ...
                let actual_epoch = builder
                    .build_load(i16_type, epoch_ptr, "actual_epoch")
                    .or_llvm_err()?
                    .into_int_value();

                // Compare: gen_ok = (actual_gen == expected_gen)
                let gen_ok = builder
                    .build_int_compare(IntPredicate::EQ, actual_gen, expected_gen, "gen_ok")
                    .or_llvm_err()?;

                // Compare: epoch_ok = (actual_epoch == expected_epoch)
                let epoch_ok = builder
                    .build_int_compare(IntPredicate::EQ, actual_epoch, expected_epoch, "epoch_ok")
                    .or_llvm_err()?;
        ...
        // ---- verum_shared_inc_ref(ptr: i64) -> void ----
        {
            ...
            if func.count_basic_blocks() > 0 { /* already emitted */
                ...
                // if (ptr == 0) return
                let is_null = builder
                    .build_int_compare(IntPredicate::EQ, ptr_val, i64_type.const_zero(), "is_null")
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(is_null, ret_bb, do_inc)
                    .or_llvm_err()?;
        ...
        //  Returns true (1) if ref_count reached 0, false (0) otherwise.
        {
            ...
            if func.count_basic_blocks() > 0 { /* already emitted */
                ...
                let is_null = builder
                    ...
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(is_null, null_ret, do_dec)
                    .or_llvm_err()?;
                ...
                let reached_zero = builder
                    ...
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(reached_zero, was_one, not_zero)
                    .or_llvm_err()?;
    ...
    fn emit_file_io(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        // verum_file_read_all is defined UAF-safe by the runtime pass
        // (verum_text_from_cstr COPIES); the former platform copy
        // dealloc'd the buffer verum_text_alloc ALIASES — a
        // use-after-free + double-free one emission-order flip from
        // live, removed like the T0436 siblings above (T0437).
    ...
    fn emit_file_open(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let is_write = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_write, mode_write, mode_append)
            .or_llvm_err()?;
        ...
        builder
            .build_store(
                flags_alloca,
                i32_type.const_int(o_wronly_creat_trunc as u64, false),
            )
            .or_llvm_err()?;
        ...
        let is_append = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_append, mode_read, mode_read)
            .or_llvm_err()?;
    ...
    fn emit_file_read_text(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let text_alloc_fn = self.get_or_declare_fn(
            module,
            "verum_text_alloc",
            i64_type.fn_type(&[ptr_type.into(), i64_type.into(), i64_type.into()], false),
        );
        let text_from_cstr_fn = self.get_or_declare_fn(
            module,
        ...
        let is_positive = builder
            .build_int_compare(IntPredicate::SGT, max_len, i64_type.const_zero(), "pos")
            .or_llvm_err()?;
        let buf_size = builder
            .build_select(is_positive, max_len, i64_type.const_int(4096, false), "bs")
            .or_llvm_err()?
            .into_int_value();
        ...
        builder
            .build_conditional_branch(buf_null, read_fail, read_ok)
            .or_llvm_err()?;
        ...
        builder
            .build_conditional_branch(read_positive, read_done, read_fail_dealloc)
            .or_llvm_err()?;
        ...
        builder
            ...
            .or_llvm_err()?;
        let empty_str2 = builder
            .build_global_string_ptr("", "empty2")
            .or_llvm_err()?;
        let empty_text2 = builder
            .build_call(
                text_from_cstr_fn,
                &[empty_str2.as_pointer_value().into()],
                "et2",
            )
            .or_llvm_err()?
            .basic_value_or("expected basic value")?;
        ...
        let in_bounds = builder
            .build_int_compare(IntPredicate::SLT, n, buf_size, "ib")
            .or_llvm_err()?;
        let null_idx = builder
            .build_select(
                in_bounds,
                n,
                builder
                    .build_int_sub(buf_size, i64_type.const_int(1, false), "bm1")
                    .or_llvm_err()?,
                "ni",
            )
            .or_llvm_err()?
        ...
        let null_ptr = unsafe {
            builder
                .build_gep(i8_type, buf, &[null_idx], "np")
                .or_llvm_err()?
        };
        builder
    ...
    fn emit_file_write_text(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let text_get_ptr_fn = self.get_or_declare_fn(
            module,
            "verum_text_get_ptr",
            ptr_type.fn_type(&[i64_type.into()], false),
        );
        let strlen_fn = self.get_or_declare_fn(
            module,
            "strlen",
            i64_type.fn_type(&[ptr_type.into()], false),
        );
        ...
        builder
            .build_conditional_branch(ptr_null, ret_zero, do_write)
            .or_llvm_err()?;
        ...
        let is_ok = builder
            ...
            .or_llvm_err()?;
        let result = builder
            .build_select(is_ok, n, i64_type.const_all_ones(), "r")
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_file_write_all(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let fd_good = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(fd_good, fd_ok, ret_fail)
            .or_llvm_err()?;
        ...
        builder
            ...
            .or_llvm_err()?;
        let is_ok = builder
            .build_int_compare(IntPredicate::SGE, n, i64_type.const_zero(), "wok")
            .or_llvm_err()?;
        let result = builder
            .build_select(is_ok, n, i64_type.const_all_ones(), "r")
            .or_llvm_err()?;
    ...
    fn ensure_networking_syscalls(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let syscalls: &[SocketSyscall] = &[
            ...
            SocketSyscall {
                ...
                sys_x86_64: 44,
                sys_aarch64: 206,
            }, // SYS_sendto with NULL addr
            SocketSyscall {
                name: "recv",
                sys_x86_64: 45,
                sys_aarch64: 207,
            }, // SYS_recvfrom with NULL addr
            SocketSyscall {
                name: "sendto",
                sys_x86_64: 44,
                sys_aarch64: 206,
    ...
    fn emit_libc_free_socket_wrapper(
        ...
        if target_is_linux(module) {
            ...
            let sys_num = if target_is_aarch64(module) {
                sys_aarch64
            } else {
                sys_x86_64
            };
    ...
    fn build_sockaddr_in(
        ...
        if super::target_triple::target_is_darwin(module) {
            ...
            let len_ptr = unsafe {
                builder
                    .build_gep(i8_type, buf, &[i8_type.const_zero()], "len_p")
                    .or_llvm_err()?
            };
            builder
                .build_store(len_ptr, i8_type.const_int(16, false))
                .or_llvm_err()?;
            // SAFETY: GEP into the stack-allocated 16-byte sockaddr_in buffer at byte 1 (sin_family field)
            let fam_ptr = unsafe {
                builder
                    .build_gep(i8_type, buf, &[i8_type.const_int(1, false)], "fam_p")
                    .or_llvm_err()?
            };
            let fam8 = builder
            ...
            let fam_ptr = unsafe {
                builder
                    .build_gep(i8_type, buf, &[i8_type.const_zero()], "fam_p")
                    .or_llvm_err()?
            };
            let fam_ptr16 = fam_ptr; // ptr to i16
            let fam16 = builder
                .build_int_truncate(family, i16_type, "fam16")
                .or_llvm_err()?;
        ...
        let port_ptr = unsafe {
            builder
                .build_gep(i8_type, buf, &[i8_type.const_int(2, false)], "port_p")
                .or_llvm_err()?
        };
        let port16 = builder
            .build_int_truncate(port_be, i16_type, "port16")
            .or_llvm_err()?;
        ...
        let addr_ptr = unsafe {
            builder
                .build_gep(i8_type, buf, &[i8_type.const_int(4, false)], "addr_p")
                .or_llvm_err()?
        };
    ...
    fn emit_tcp_connect(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        while ret_fail.get_instructions().count() > 0 {
            ret_fail
                ...
                .or_internal("no last instruction")?
    ...
    fn emit_tcp_listen(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_conditional_branch(listen_good, ret_ok, ret_fail)
            .or_llvm_err()?;
    ...
    fn emit_tcp_accept(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let client_addr = builder
            .build_alloca(i8_type.array_type(16), "caddr")
            .or_llvm_err()?;
        let client_len = builder.build_alloca(i32_type, "clen").or_llvm_err()?;
        builder
            .build_store(client_len, i32_type.const_int(16, false))
            .or_llvm_err()?;
    ...
    fn emit_tcp_send_text(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_conditional_branch(ptr_null, ret_zero, do_send)
            .or_llvm_err()?;
    ...
    fn emit_tcp_recv_text(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let is_pos = builder
            ...
            .or_llvm_err()?;
        let buf_size = builder
            .build_select(is_pos, max_len, i64_type.const_int(4096, false), "bs")
            .or_llvm_err()?
            .into_int_value();
        let alloc_size = builder
            .build_int_add(buf_size, i64_type.const_int(1, false), "as")
            .or_llvm_err()?;
        ...
        let n_pos = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(n_pos, recv_ok, ret_empty)
            .or_llvm_err()?;
    ...
    fn emit_udp_bind(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let ret_ok = ctx.append_basic_block(func, "ret_ok");
        builder
            .build_conditional_branch(bind_good, ret_ok, ret_fail)
            .or_llvm_err()?;
    ...
    fn emit_process_wait(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let status_val = builder
            ...
            .or_llvm_err()?
            .into_int_value();
        let status64 = builder
            .build_int_s_extend(status_val, i64_type, "s64")
            .or_llvm_err()?;
        let ret = builder
            .build_select(failed, i64_type.const_all_ones(), status64, "ret")
            .or_llvm_err()?;
    ...
    fn emit_fd_read_all_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_conditional_branch(buf_null, ret_zero, read_loop)
            .or_llvm_err()?;
        ...
        let need_grow = builder
            .build_int_compare(IntPredicate::UGE, cur_len, cur_cap, "ng")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(need_grow, grow_buf, do_read)
            .or_llvm_err()?;

        // grow_buf
        builder.position_at_end(grow_buf);
        let new_cap = builder
            .build_int_mul(cur_cap, i64_type.const_int(2, false), "nc")
            .or_llvm_err()?;
        let new_buf = builder
        ...
        builder
            .build_call(
                memcpy_fn,
                &[new_buf.into(), cur_buf.into(), cur_len.into()],
                "",
            )
            .or_llvm_err()?;
        builder
            .build_call(dealloc_fn, &[cur_buf.into(), cur_cap.into()], "")
            .or_llvm_err()?;
        ...
        let buf_offset = unsafe {
            builder
                .build_gep(i8_type, rd_buf_val, &[cur_len], "bo")
                .or_llvm_err()?
        };
        let remaining = builder
            .build_int_sub(rd_cap_val, cur_len, "rem")
            .or_llvm_err()?;
        let fd32 = fd; // i64 Verum ABI — no truncation needed
        let n = builder
            .build_call(
                read_fn,
                &[fd32.into(), buf_offset.into(), remaining.into()],
                "n",
            )
            .or_llvm_err()?
            .basic_value_or("expected basic value")?
        ...
        let is_eof = builder
            .build_int_compare(IntPredicate::SLE, n, i64_type.const_zero(), "eof")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_eof, read_eof, read_loop)
            .or_llvm_err()?;
        ...
        let hdr_1 = unsafe {
            builder
                .build_gep(i64_type, hdr, &[i64_type.const_int(1, false)], "h1")
                .or_llvm_err()?
        };
        builder.build_store(hdr_1, rd_cap_val).or_llvm_err()?;
        // Store buf ptr as i64 at hdr[2]
        let buf_as_i64 = builder
            .build_ptr_to_int(rd_buf_val, i64_type, "bi64")
            .or_llvm_err()?;
        // SAFETY: GEP into the 24-byte [len, cap, buf_ptr] byte-array header at slot 2 (data pointer)
        let hdr_2 = unsafe {
            builder
                .build_gep(i64_type, hdr, &[i64_type.const_int(2, false)], "h2")
                .or_llvm_err()?
        };
        builder.build_store(hdr_2, buf_as_i64).or_llvm_err()?;
        let hdr_as_i64 = builder
            .build_ptr_to_int(hdr, i64_type, "hi64")
            .or_llvm_err()?;
    ...
    fn emit_fd_close_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let fd = func
            ...
            .or_internal("missing first param")?
            .into_int_value();
        let fd_valid = builder
            .build_int_compare(IntPredicate::SGE, fd, i64_type.const_zero(), "ok")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(fd_valid, do_close, ret)
            .or_llvm_err()?;
    ...
    fn emit_socket_set_nonblocking(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let o_nonblock: u64 = if target_is_linux(module) {
            0x800
        } else {
            0x0004
        };
        let builder = ctx.create_builder();
        let entry = ctx.append_basic_block(func, "entry");
        let getfl_ok = ctx.append_basic_block(func, "getfl_ok");
        let ret_fail = ctx.append_basic_block(func, "ret_fail");
        builder.position_at_end(entry);
        let fd = func
            .get_first_param()
        ...
        let ok = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(ok, getfl_ok, ret_fail)
            .or_llvm_err()?;
        builder.position_at_end(ret_fail);
        builder
            .build_return(Some(&i64_type.const_all_ones()))
    ...
    fn emit_setsockopt_bool_helper(
        &self,
        module: &Module<'ctx>,
        func_name: &str,
        sol_level: u64,
        ...
        let fd = func
            ...
            .or_internal("missing first param")?
            .into_int_value();
        let va = builder.build_alloca(i32_type, "val").or_llvm_err()?;
        builder
    ...
    fn emit_socket_set_nodelay(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        self.emit_setsockopt_bool_helper(module, "verum_socket_set_nodelay", 6, 1)?;
        Ok(())
    }
    ...
    fn emit_socket_get_error(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let builder = ctx.create_builder();
        let entry = ctx.append_basic_block(func, "entry");
        let gso_ok = ctx.append_basic_block(func, "gso_ok");
        let ret_fail = ctx.append_basic_block(func, "ret_fail");
        builder.position_at_end(entry);
        let fd = func
            .get_first_param()
            ...
            .into_int_value();
        let va = builder.build_alloca(i32_type, "val").or_llvm_err()?;
        let la = builder.build_alloca(i32_type, "len").or_llvm_err()?;
        builder
            .build_store(va, i32_type.const_zero())
            .or_llvm_err()?;
        builder
        ...
        let ok = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(ok, gso_ok, ret_fail)
            .or_llvm_err()?;
        builder.position_at_end(gso_ok);
        let v = builder
            .build_load(i32_type, va, "v")
            .or_llvm_err()?
            .into_int_value();
        let v64 = builder
            .build_int_z_extend(v, i64_type, "v64")
            .or_llvm_err()?;
        builder.build_return(Some(&v64)).or_llvm_err()?;
        builder.position_at_end(ret_fail);
        builder
            .build_return(Some(&i64_type.const_all_ones()))
            .or_llvm_err()?;
    ...
    fn emit_socket_connect_nonblocking(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let func = self.get_or_declare_fn(
            module,
            "verum_socket_connect_nonblocking",
            i64_type.fn_type(&[i64_type.into(), ptr_type.into(), i64_type.into()], false),
        );
        if func.count_basic_blocks() > 0 {
            return Ok(());
        ...
        let builder = ctx.create_builder();
        let entry = ctx.append_basic_block(func, "entry");
        let check_errno = ctx.append_basic_block(func, "check_errno");
        let ret_ok = ctx.append_basic_block(func, "ret_ok");
        let ret_inp = ctx.append_basic_block(func, "ret_inprogress");
        let ret_fail = ctx.append_basic_block(func, "ret_fail");
        builder.position_at_end(entry);
        let fd = func
            .get_nth_param(0)
        ...
        let cok = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(cok, ret_ok, check_errno)
            .or_llvm_err()?;
        builder.position_at_end(ret_ok);
        builder
            .build_return(Some(&i64_type.const_zero()))
        ...
        let efn = if target_is_linux(module) {
            "__errno_location"
        } else {
            "__error"
        };
        ...
        let ev = builder
            ...
            .into_int_value();
        let ev64 = builder
            .build_int_z_extend(ev, i64_type, "ev64")
            .or_llvm_err()?;
        let ip = builder
            .build_int_compare(
                IntPredicate::EQ,
                ev64,
                i64_type.const_int(einprogress, false),
                "ip",
            )
            .or_llvm_err()?;
        builder
            .build_conditional_branch(ip, ret_inp, ret_fail)
            .or_llvm_err()?;
        builder.position_at_end(ret_inp);
        builder
            .build_return(Some(&i64_type.const_int(1, false)))
            .or_llvm_err()?;
        builder.position_at_end(ret_fail);
    ...
    fn emit_socket_accept_nonblocking(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let func = self.get_or_declare_fn(
            module,
            "verum_socket_accept_nonblocking",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        if func.count_basic_blocks() > 0 {
            return Ok(());
    ...
    fn emit_chan_new(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let cap_p = unsafe {
            builder
                .build_gep(i8_type, hdr_ptr, &[i64_type.const_int(16, false)], "cap_p")
                .or_llvm_err()?
        };
        ...
        let data_sz = builder
            .build_int_mul(cap, i64_type.const_int(8, false), "dsz")
            .or_llvm_err()?;
        ...
        let data_i64 = builder
            .build_ptr_to_int(data_ptr, i64_type, "di64")
            .or_llvm_err()?;
        // SAFETY: GEP into the 64-byte channel header at offset 56 (data ring buffer pointer)
        let dp_p = unsafe {
            builder
                .build_gep(i8_type, hdr_ptr, &[i64_type.const_int(56, false)], "dp_p")
                .or_llvm_err()?
        };
        ...
        let result = builder
            .build_ptr_to_int(hdr_ptr, i64_type, "result")
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_chan_send(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let cond_wait = module
            .get_function("verum_cond_wait")
            .or_missing_fn("verum_cond_wait")?;
        let cond_signal = module
            .get_function("verum_cond_signal")
            .or_missing_fn("verum_cond_signal")?;
        ...
        let not_empty_p = unsafe {
            builder
                .build_gep(i8_type, hdr, &[i64_type.const_int(4, false)], "ne_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP into the channel header at offset 8 (not_full condvar)
        let not_full_p = unsafe {
            builder
                .build_gep(i8_type, hdr, &[i64_type.const_int(8, false)], "nf_p")
                .or_llvm_err()?
        };
        ...
        let tail_p = unsafe {
            builder
                .build_gep(i8_type, hdr, &[i64_type.const_int(40, false)], "tail_p")
                .or_llvm_err()?
        };
        ...
        let data_p = unsafe {
            builder
                .build_gep(i8_type, hdr, &[i64_type.const_int(56, false)], "data_p")
                .or_llvm_err()?
        };
        ...
        builder
            ...
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(wait_loop)
            .or_llvm_err()?;
        ...
        let closed = builder
            ...
            .or_llvm_err()?
            .into_int_value();
        let not_closed = builder
            .build_int_compare(IntPredicate::EQ, closed, i64_type.const_zero(), "nc")
            .or_llvm_err()?;
        let need_wait = builder.build_and(full, not_closed, "nw").or_llvm_err()?;
        builder
            .build_conditional_branch(need_wait, check_closed, do_send)
            .or_llvm_err()?;

        // check_closed: wait, then re-check (but first check if actually need to wait vs closed)
        builder.position_at_end(check_closed);
        builder
            .build_call(cond_wait, &[not_full_p.into(), mtx_p.into()], "")
            .or_llvm_err()?;
        // After wait, check closed again
        let closed2 = builder
            .build_load(i64_type, closed_p, "cl2")
            .or_llvm_err()?
            .into_int_value();
        let is_closed2 = builder
            .build_int_compare(IntPredicate::NE, closed2, i64_type.const_zero(), "ic2")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_closed2, ret_closed, wait_loop)
            .or_llvm_err()?;

        // do_send: check closed one more time, then store value
        builder.position_at_end(do_send);
        let closed3 = builder
            .build_load(i64_type, closed_p, "cl3")
            .or_llvm_err()?
            .into_int_value();
        let is_closed3 = builder
            .build_int_compare(IntPredicate::NE, closed3, i64_type.const_zero(), "ic3")
            .or_llvm_err()?;
        let do_store = ctx.append_basic_block(func, "do_store");
        builder
            .build_conditional_branch(is_closed3, ret_closed, do_store)
            .or_llvm_err()?;

        builder.position_at_end(do_store);
        // data[tail] = value
        let dp_i64 = builder
            .build_load(i64_type, data_p, "dp_i64")
            .or_llvm_err()?
            .into_int_value();
        let dp = builder
            .build_int_to_ptr(dp_i64, ptr_type, "dp")
            .or_llvm_err()?;
        let tail = builder
            .build_load(i64_type, tail_p, "tail")
            .or_llvm_err()?
            .into_int_value();
        let tail_off = builder
            .build_int_mul(tail, i64_type.const_int(8, false), "toff")
            .or_llvm_err()?;
        // SAFETY: GEP into the channel data ring buffer at slot[tail]; tail < cap, within the allocated buffer
        let slot = unsafe {
            builder
                .build_gep(i8_type, dp, &[tail_off], "slot")
                .or_llvm_err()?
        };
        ...
        let cap2 = builder
            .build_load(i64_type, cap_p, "cap2")
            .or_llvm_err()?
            .into_int_value();
        let tail_inc = builder
            .build_int_add(tail, i64_type.const_int(1, false), "ti")
            .or_llvm_err()?;
        let new_tail = builder
            .build_int_unsigned_rem(tail_inc, cap2, "nt")
            .or_llvm_err()?;
        builder.build_store(tail_p, new_tail).or_llvm_err()?;

        // len++
        let len2 = builder
            .build_load(i64_type, len_p, "len2")
            .or_llvm_err()?
            .into_int_value();
        let new_len = builder
            .build_int_add(len2, i64_type.const_int(1, false), "nl")
            .or_llvm_err()?;
        builder.build_store(len_p, new_len).or_llvm_err()?;

        // signal not_empty, unlock, return 0
        builder
            .build_call(cond_signal, &[not_empty_p.into()], "")
            .or_llvm_err()?;
        builder
    ...
    fn emit_chan_recv(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let head_p = unsafe {
            builder
                .build_gep(i8_type, hdr, &[i64_type.const_int(32, false)], "head_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP into the channel header at offset 48 (closed flag)
        let closed_p = unsafe {
        ...
        let len = builder
            ...
            .into_int_value();
        let empty = builder
            .build_int_compare(IntPredicate::EQ, len, i64_type.const_zero(), "empty")
            .or_llvm_err()?;
        let closed = builder
            .build_load(i64_type, closed_p, "closed")
            .or_llvm_err()?
        ...
        builder
            .build_conditional_branch(need_wait, do_wait, check_empty_closed)
            .or_llvm_err()?;

        // do_wait
        builder.position_at_end(do_wait);
        builder
            .build_call(cond_wait, &[not_empty_p.into(), mtx_p.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(wait_loop)
            .or_llvm_err()?;
        ...
        let len2 = builder
            ...
            .into_int_value();
        let empty2 = builder
            .build_int_compare(IntPredicate::EQ, len2, i64_type.const_zero(), "emp2")
            .or_llvm_err()?;
        let closed2 = builder
            .build_load(i64_type, closed_p, "cl2")
            .or_llvm_err()?
        ...
        builder
            .build_conditional_branch(empty_and_closed, ret_empty_closed, do_recv)
            .or_llvm_err()?;
        ...
        let dp = builder
            ...
            .or_llvm_err()?;
        let head = builder
            .build_load(i64_type, head_p, "head")
            .or_llvm_err()?
            .into_int_value();
        let head_off = builder
            .build_int_mul(head, i64_type.const_int(8, false), "hoff")
            .or_llvm_err()?;
        // SAFETY: GEP into the channel data ring buffer at slot[head]; head < cap, within the allocated buffer
        let slot = unsafe {
            builder
                .build_gep(i8_type, dp, &[head_off], "slot")
                .or_llvm_err()?
        };
        let val = builder
            .build_load(i64_type, slot, "val")
            .or_llvm_err()?
            .into_int_value();
        ...
        let head_inc = builder
            .build_int_add(head, i64_type.const_int(1, false), "hi")
            .or_llvm_err()?;
        let new_head = builder
            .build_int_unsigned_rem(head_inc, cap, "nh")
            .or_llvm_err()?;
        builder.build_store(head_p, new_head).or_llvm_err()?;

        let len3 = builder
            .build_load(i64_type, len_p, "len3")
            .or_llvm_err()?
            .into_int_value();
        let new_len = builder
            .build_int_sub(len3, i64_type.const_int(1, false), "nl")
            .or_llvm_err()?;
        builder.build_store(len_p, new_len).or_llvm_err()?;

        // Signal not_full, *ok_out = 1, unlock, return value
        builder
            .build_call(cond_signal, &[not_full_p.into()], "")
            .or_llvm_err()?;
        builder
            .build_store(ok_out, i64_type.const_int(1, false))
            .or_llvm_err()?;
        builder
            .build_call(mutex_unlock, &[mtx_p.into()], "")
            .or_llvm_err()?;
        ...
        builder
            .build_store(ok_out, i64_type.const_zero())
            .or_llvm_err()?;
        builder
            .build_call(mutex_unlock, &[mtx_p.into()], "")
            .or_llvm_err()?;
    ...
    fn emit_chan_try_send(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let is_closed = builder
            .build_int_compare(IntPredicate::NE, closed, i64_type.const_zero(), "ic")
            .or_llvm_err()?;
        let cant_send = builder.build_or(full, is_closed, "cant").or_llvm_err()?;
        builder
            .build_conditional_branch(cant_send, ret_fail, do_send)
            .or_llvm_err()?;
    ...
    fn emit_chan_try_recv(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let empty = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(empty, ret_empty, do_recv)
            .or_llvm_err()?;
        ...
        let len2 = builder
            ...
            .into_int_value();
        let new_len = builder
            .build_int_sub(len2, i64_type.const_int(1, false), "nl")
            .or_llvm_err()?;
        builder.build_store(len_p, new_len).or_llvm_err()?;
    ...
    fn emit_chan_close(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let mutex_unlock = module
            .get_function("verum_mutex_unlock")
            .or_missing_fn("verum_mutex_unlock")?;
        let cond_broadcast = module
            .get_function("verum_cond_broadcast")
            .or_missing_fn("verum_cond_broadcast")?;
        ...
        builder
            ...
            .or_llvm_err()?;
        builder
            .build_store(closed_p, i64_type.const_int(1, false))
            .or_llvm_err()?;
        builder
            .build_call(cond_broadcast, &[not_empty_p.into()], "")
            .or_llvm_err()?;
        builder
            .build_call(cond_broadcast, &[not_full_p.into()], "")
            .or_llvm_err()?;
        builder
            .build_call(mutex_unlock, &[mtx_p.into()], "")
            .or_llvm_err()?;
    ...
    fn emit_nursery_new(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let max_p = unsafe {
            builder
                .build_gep(i8_type, nurs_ptr, &[i64_type.const_int(16, false)], "max_p")
                .or_llvm_err()?
        };
        ...
        let has_max = builder
            .build_int_compare(IntPredicate::SGT, max, i64_type.const_zero(), "hm")
            .or_llvm_err()?;
        let arr_sz: verum_llvm::values::IntValue = builder
            .build_select(has_max, max, default_sz, "asz")
            .or_llvm_err()?
            .into_int_value();
        let handles_sz = builder
            .build_int_mul(arr_sz, i64_type.const_int(8, false), "hsz")
            .or_llvm_err()?;
        ...
        let hdl_i64 = builder
            .build_ptr_to_int(handles_ptr, i64_type, "hi64")
            .or_llvm_err()?;
        builder.build_store(nurs_ptr, hdl_i64).or_llvm_err()?;

        let result = builder
            .build_ptr_to_int(nurs_ptr, i64_type, "result")
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_nursery_spawn(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let pool_global_submit = self.get_or_declare_fn(
            module,
            "verum_pool_global_submit",
            i64_type.fn_type(&[i64_type.into(), i64_type.into()], false),
        );
        let mutex_lock = module
            .get_function("verum_mutex_lock")
        ...
        let arg = func
            ...
            .into_int_value();
        let nurs = builder
            .build_int_to_ptr(nurs_i64, ptr_type, "nurs")
            .or_llvm_err()?;
        ...
        let cnt_p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(8, false)], "cnt_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP into the nursery struct at offset 16 (max_threads)
        let max_p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(16, false)], "max_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP into the nursery struct at offset 48 (mutex)
        let mtx_p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(48, false)], "mtx_p")
                .or_llvm_err()?
        };
        ...
        builder
            ...
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(check_max)
            .or_llvm_err()?;

        // check_max: if max > 0 && count >= max → return -1
        builder.position_at_end(check_max);
        let max_val = builder
            .build_load(i64_type, max_p, "max")
            .or_llvm_err()?
            .into_int_value();
        let cnt = builder
            .build_load(i64_type, cnt_p, "cnt")
            .or_llvm_err()?
            .into_int_value();
        let has_limit = builder
            .build_int_compare(IntPredicate::SGT, max_val, i64_type.const_zero(), "hl")
            .or_llvm_err()?;
        let at_limit = builder
            .build_int_compare(IntPredicate::SGE, cnt, max_val, "al")
            .or_llvm_err()?;
        let blocked = builder
            .build_and(has_limit, at_limit, "blk")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(blocked, ret_limit, do_spawn)
            .or_llvm_err()?;
        ...
        let hdl_arr_i64 = builder
            .build_load(i64_type, nurs, "hdl_i64")
            .or_llvm_err()?
            .into_int_value();
        let hdl_arr = builder
            .build_int_to_ptr(hdl_arr_i64, ptr_type, "hdl_arr")
            .or_llvm_err()?;
        let off = builder
            .build_int_mul(cnt, i64_type.const_int(8, false), "off")
            .or_llvm_err()?;
        // SAFETY: GEP into the nursery handles array at slot[count]; count < max (checked above), within allocated buffer
        let slot = unsafe {
            builder
                .build_gep(i8_type, hdl_arr, &[off], "slot")
                .or_llvm_err()?
        };
        ...
        // Increment count
        let new_cnt = builder
            .build_int_add(cnt, i64_type.const_int(1, false), "nc")
            .or_llvm_err()?;
    ...
    fn emit_nursery_await_all(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let pool_await_fn = self.get_or_declare_fn(
            module,
            "verum_pool_await",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        ...
        let timeout_p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(24, false)], "to_p")
                .or_llvm_err()?
        };
        let timeout_ms = builder
            .build_load(i64_type, timeout_p, "to_ms")
            .or_llvm_err()?
            .into_int_value();
        // SAFETY: GEP at a fixed offset within a known struct layout; the pointer is valid from prior allocation
        let err_beh_p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(32, false)], "eb_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP at a fixed offset within a known struct layout; the pointer is valid from prior allocation
        let cancel_p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(40, false)], "cf_p")
                .or_llvm_err()?
        };
        ...
        builder
            ...
            .or_llvm_err()?;
        let start_sec = builder
            .build_load(i64_type, ts_alloca, "ss")
            .or_llvm_err()?
            .into_int_value();
        // SAFETY: GEP into the stack-allocated timespec {sec, nsec} at slot 1 (nanoseconds)
        let start_ns_p = unsafe {
            builder
                .build_gep(i64_type, ts_alloca, &[i64_type.const_int(1, false)], "snp")
                .or_llvm_err()?
        };
        let start_ns = builder
            .build_load(i64_type, start_ns_p, "sn")
            .or_llvm_err()?
            .into_int_value();
        ...
        builder
            .build_store(err_alloca, i64_type.const_zero())
            .or_llvm_err()?;
        ...
        let cmp = builder
            .build_int_compare(IntPredicate::SLT, i_val, cnt, "cmp")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(cmp, check_cancel, ret_ok)
            .or_llvm_err()?;

        // check_cancel: if cancel_flag set, skip remaining
        builder.position_at_end(check_cancel);
        let cf = builder
            .build_load(i64_type, cancel_p, "cf")
            .or_llvm_err()?
            .into_int_value();
        let is_cancelled = builder
            .build_int_compare(IntPredicate::NE, cf, i64_type.const_zero(), "cancelled")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_cancelled, ret_cancel, do_await)
            .or_llvm_err()?;

        // do_await: check if timeout is set
        builder.position_at_end(do_await);
        let has_timeout = builder
            .build_int_compare(IntPredicate::SGT, timeout_ms, i64_type.const_zero(), "ht")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(has_timeout, check_timeout_bb, timed_poll)
            .or_llvm_err()?;

        // check_timeout: check elapsed time before blocking await
        builder.position_at_end(check_timeout_bb);
        let ts2 = builder
            .build_alloca(i64_type.array_type(2), "ts2")
            .or_llvm_err()?;
        builder
            .build_call(clock_gettime_fn, &[clock_id.into(), ts2.into()], "")
            .or_llvm_err()?;
        let now_sec = builder
            .build_load(i64_type, ts2, "ns")
            .or_llvm_err()?
            .into_int_value();
        // SAFETY: GEP at a fixed offset within a known struct layout; the pointer is valid from prior allocation
        let now_ns_p = unsafe {
            builder
                .build_gep(i64_type, ts2, &[i64_type.const_int(1, false)], "nnp")
                .or_llvm_err()?
        };
        let now_ns = builder
            .build_load(i64_type, now_ns_p, "nn")
            .or_llvm_err()?
            .into_int_value();
        // elapsed_ms = (now_sec - start_sec) * 1000 + (now_ns - start_ns) / 1_000_000
        let ds = builder
            .build_int_sub(now_sec, start_sec, "ds")
            .or_llvm_err()?;
        let ds_ms = builder
            .build_int_mul(ds, i64_type.const_int(1000, false), "dsms")
            .or_llvm_err()?;
        let dn = builder
            .build_int_sub(now_ns, start_ns, "dn")
            .or_llvm_err()?;
        let dn_ms = builder
            .build_int_signed_rem(dn, i64_type.const_int(1_000_000_000, false), "dnr")
            .or_llvm_err()?;
        let dn_ms_div = builder
            .build_int_signed_div(dn_ms, i64_type.const_int(1_000_000, false), "dnms")
            .or_llvm_err()?;
        let elapsed_ms = builder
            .build_int_add(ds_ms, dn_ms_div, "elapsed")
            .or_llvm_err()?;
        let timed_out = builder
            .build_int_compare(IntPredicate::SGE, elapsed_ms, timeout_ms, "tout")
            .or_llvm_err()?;
        // If timed out: set cancel_flag and return 1
        let timeout_cancel = ctx.append_basic_block(func, "timeout_cancel");
        builder
            .build_conditional_branch(timed_out, timeout_cancel, timed_poll)
            .or_llvm_err()?;

        builder.position_at_end(timeout_cancel);
        builder
            .build_store(cancel_p, i64_type.const_int(1, false))
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(ret_timeout)
            .or_llvm_err()?;

        // timed_poll: do a blocking pool_await (pool already uses adaptive spin+yield)
        builder.position_at_end(timed_poll);
        let off = builder
            .build_int_mul(i_val, i64_type.const_int(8, false), "off")
            .or_llvm_err()?;
        // SAFETY: GEP at a fixed offset within a known struct layout; the pointer is valid from prior allocation
        let slot = unsafe {
            builder
        ...
        let is_error = builder
            .build_int_compare(IntPredicate::SLT, result, i64_type.const_zero(), "is_err")
            .or_llvm_err()?;
        let error_path = ctx.append_basic_block(func, "error_path");
        builder
            .build_conditional_branch(is_error, error_path, next_iter)
            .or_llvm_err()?;

        // error_path: apply error behavior policy
        builder.position_at_end(error_path);
        let err_beh = builder
            .build_load(i64_type, err_beh_p, "eb")
            .or_llvm_err()?
            .into_int_value();
        // Accumulate error
        let cur_err = builder
            .build_load(i64_type, err_alloca, "ce")
            .or_llvm_err()?
            .into_int_value();
        let new_err = builder
            .build_int_add(cur_err, i64_type.const_int(1, false), "ne")
            .or_llvm_err()?;
        builder.build_store(err_alloca, new_err).or_llvm_err()?;
        // fail_fast (2): return immediately
        let is_fail_fast = builder
            .build_int_compare(
                IntPredicate::EQ,
                err_beh,
                i64_type.const_int(2, false),
        ...
        builder
            .build_conditional_branch(is_fail_fast, ret_err_bb, cancel_all_bb)
            .or_llvm_err()?;
        ...
        let is_cancel_all = builder
            .build_int_compare(IntPredicate::EQ, err_beh, i64_type.const_zero(), "ca")
            .or_llvm_err()?;
        let do_cancel_bb = ctx.append_basic_block(func, "do_cancel");
        builder
            .build_conditional_branch(is_cancel_all, do_cancel_bb, next_iter)
            .or_llvm_err()?;
        ...
        builder
            ...
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(next_iter)
            .or_llvm_err()?;
        ...
        let ni = builder
            .build_int_add(i_val, i64_type.const_int(1, false), "ni")
            .or_llvm_err()?;
        ...
        builder.position_at_end(ret_ok);
        let final_err = builder
            .build_load(i64_type, err_alloca, "fe")
            .or_llvm_err()?
            .into_int_value();
        let has_err = builder
            .build_int_compare(IntPredicate::SGT, final_err, i64_type.const_zero(), "he")
            .or_llvm_err()?;
        let neg_err = builder.build_int_neg(final_err, "neg_err").or_llvm_err()?;
        let ret_val: verum_llvm::values::IntValue = builder
            .build_select(has_err, neg_err, i64_type.const_zero(), "rv")
            .or_llvm_err()?
            .into_int_value();
        builder.build_return(Some(&ret_val)).or_llvm_err()?;
    ...
    fn emit_nursery_set_timeout(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let t = func
            ...
            .into_int_value();
        let nurs = builder
            .build_int_to_ptr(n, ptr_type, "nurs")
            .or_llvm_err()?;
        // SAFETY: GEP into the nursery struct at offset 24 (timeout_ms field)
        let p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(24, false)], "p")
                .or_llvm_err()?
        };
    ...
    fn emit_nursery_set_max_tasks(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(16, false)], "p")
                .or_llvm_err()?
        };
    ...
    fn emit_nursery_set_error_behavior(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let func = self.get_or_declare_fn(
            module,
            "verum_nursery_set_error_behavior",
            void_type.fn_type(&[i64_type.into(), i64_type.into()], false),
        );
        if func.count_basic_blocks() > 0 {
            return Ok(());
        ...
        let p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(32, false)], "p")
                .or_llvm_err()?
        };
    ...
    fn emit_nursery_cancel(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let p = unsafe {
            builder
                .build_gep(i8_type, nurs, &[i64_type.const_int(40, false)], "p")
                .or_llvm_err()?
        };
        builder
            .build_store(p, i64_type.const_int(1, false))
            .or_llvm_err()?;
        builder.build_return(None).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_waitgroup_new(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let ptr_type = ctx.ptr_type(AddressSpace::default());
        let func =
            self.get_or_declare_fn(module, "verum_waitgroup_new", i64_type.fn_type(&[], false));
        if func.count_basic_blocks() > 0 {
            return Ok(());
        }
        ...
        let result = builder
            .build_ptr_to_int(wg_ptr, i64_type, "result")
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_waitgroup_add(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let delta = func
            ...
            .into_int_value();
        let wg = builder
            .build_int_to_ptr(wg_i64, ptr_type, "wg")
            .or_llvm_err()?;
        ...
        let mtx_p = unsafe {
            builder
                .build_gep(i8_type, wg, &[i64_type.const_int(8, false)], "mtx_p")
                .or_llvm_err()?
        };
        builder
            .build_call(mutex_lock, &[mtx_p.into()], "")
    ...
    fn emit_waitgroup_done(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let cv_p = unsafe {
            builder
                .build_gep(i8_type, wg, &[i64_type.const_int(12, false)], "cv_p")
                .or_llvm_err()?
        };
        ...
        let cnt = builder
            ...
            .into_int_value();
        let new_cnt = builder
            .build_int_sub(cnt, i64_type.const_int(1, false), "nc")
            .or_llvm_err()?;
        builder.build_store(wg, new_cnt).or_llvm_err()?;

        // If counter == 0, broadcast
        let is_zero = builder
            .build_int_compare(IntPredicate::EQ, new_cnt, i64_type.const_zero(), "iz")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_zero, bcast, done)
            .or_llvm_err()?;

        builder.position_at_end(bcast);
        builder
            .build_call(cond_broadcast, &[cv_p.into()], "")
            .or_llvm_err()?;
        builder.build_unconditional_branch(done).or_llvm_err()?;
    ...
    fn emit_waitgroup_wait(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let cnt = builder
            ...
            .into_int_value();
        let gt = builder
            .build_int_compare(IntPredicate::SGT, cnt, i64_type.const_zero(), "gt")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(gt, do_wait, done)
            .or_llvm_err()?;

        builder.position_at_end(do_wait);
        builder
            .build_call(cond_wait, &[cv_p.into(), mtx_p.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(wait_loop)
            .or_llvm_err()?;
    ...
    fn emit_waitgroup_destroy(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_call(
                dealloc_fn,
                &[wg.into(), i64_type.const_int(32, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder.build_return(None).or_llvm_err()?;
    ...
    fn emit_select_channels(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_call(
                clock_gettime_fn,
                &[i64_type.const_zero().into(), ts_alloca.into()],
                "",
            )
            .or_llvm_err()?;
        let start_sec_p = ts_alloca;
        let start_sec = builder
            .build_load(i64_type, start_sec_p, "ss")
            .or_llvm_err()?
            .into_int_value();
        // SAFETY: GEP at a fixed offset within a known struct layout; the pointer is valid from prior allocation
        let start_ns_p = unsafe {
        ...
        builder
            .build_unconditional_branch(poll_loop)
            .or_llvm_err()?;
        ...
        builder
            .build_unconditional_branch(check_chan)
            .or_llvm_err()?;
        ...
        let cmp = builder
            .build_int_compare(IntPredicate::SLT, i, n, "cmp")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(cmp, chan_next, poll_yield)
            .or_llvm_err()?;

        // chan_next: load channel ptr, lock, check len, unlock
        builder.position_at_end(chan_next);
        let off = builder
            .build_int_mul(i, i64_type.const_int(8, false), "off")
            .or_llvm_err()?;
        // SAFETY: GEP into the channel header at a fixed offset; the channel object layout is defined by emit_channels_ir
        let slot = unsafe {
            builder
                .build_gep(i8_type, ptrs, &[off], "slot")
                .or_llvm_err()?
        };
        let chan_i64 = builder
            .build_load(i64_type, slot, "chan_i64")
            .or_llvm_err()?
            .into_int_value();
        let hdr = builder
            .build_int_to_ptr(chan_i64, ptr_type, "hdr")
        ...
        let has_data = builder
            .build_int_compare(IntPredicate::SGT, len, i64_type.const_zero(), "has")
            .or_llvm_err()?;
        let ni = builder
        ...
        builder
            .build_conditional_branch(has_data, chan_ready, check_chan)
            .or_llvm_err()?;
        ...
        let round_next = builder
            .build_int_add(round, i64_type.const_int(1, false), "rn")
            .or_llvm_err()?;
        let past_spin = builder
            .build_int_compare(
                IntPredicate::SGE,
                round,
                i64_type.const_int(32, false),
        ...
        builder
            .build_call(
                clock_gettime_fn,
                &[i64_type.const_zero().into(), ts2.into()],
                "",
            )
            .or_llvm_err()?;
        let now_sec = builder
        ...
        let exceeded = builder
            .build_int_compare(IntPredicate::SGE, elapsed_ns, timeout, "exc")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(exceeded, ret_timeout, continue_bb)
            .or_llvm_err()?;
    ...
    fn emit_io_engine_new(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let result = builder
            .build_ptr_to_int(eng_ptr, i64_type, "result")
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
        Ok(())
    }
    ...
    fn build_kevent_on_stack<'a>(
        ...
        let kev = builder
            .build_alloca(i8_type.array_type(32), "kev")
            .or_llvm_err()?;
        builder
            .build_memset(kev, 1, i8_type.const_zero(), i64_type.const_int(32, false))
            .or_llvm_err()?;
        ...
        let filter_p = unsafe {
            builder
                .build_gep(i8_type, kev, &[i64_type.const_int(8, false)], "filt_p")
                .or_llvm_err()?
        };
        ...
        let flags_p = unsafe {
            builder
                .build_gep(i8_type, kev, &[i64_type.const_int(10, false)], "flags_p")
                .or_llvm_err()?
        };
    ...
    fn emit_io_submit(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let eng_ptr = builder
            .build_int_to_ptr(engine, ptr_type, "eng_ptr")
            .or_llvm_err()?;
        let kq_fd = builder
            .build_load(i64_type, eng_ptr, "kq_fd")
            .or_llvm_err()?
            .into_int_value();

        // Check if events & 2 (WRITE) — otherwise default to READ
        let is_write = builder
            .build_and(events, i64_type.const_int(2, false), "is_w")
            .or_llvm_err()?;
        let is_write_flag = builder
            .build_int_compare(IntPredicate::NE, is_write, i64_type.const_zero(), "wf")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_write_flag, do_write, do_read)
            .or_llvm_err()?;
        ...
        let i16_type = ctx.i16_type();
        let kev_r = builder
            .build_alloca(i8_type.array_type(32), "kev_r")
            .or_llvm_err()?;
        builder
            .build_memset(
                kev_r,
        ...
        let filt_p_r = unsafe {
            builder
                .build_gep(i8_type, kev_r, &[i64_type.const_int(8, false)], "fp_r")
                .or_llvm_err()?
        };
        builder
            .build_store(filt_p_r, i16_type.const_int((-1i16 as u16) as u64, false))
            .or_llvm_err()?; // EVFILT_READ = -1
        // SAFETY: GEP into the 32-byte kevent struct at byte 10 (flags field) for EV_ADD|EV_ENABLE
        let flags_p_r = unsafe {
            builder
                .build_gep(i8_type, kev_r, &[i64_type.const_int(10, false)], "flp_r")
                .or_llvm_err()?
        };
        builder
        ...
        builder.position_at_end(do_write);
        let kev_w = builder
            .build_alloca(i8_type.array_type(32), "kev_w")
            .or_llvm_err()?;
        builder
            .build_memset(
                kev_w,
        ...
        let filt_p_w = unsafe {
            builder
                .build_gep(i8_type, kev_w, &[i64_type.const_int(8, false)], "fp_w")
                .or_llvm_err()?
        };
        builder
            .build_store(filt_p_w, i16_type.const_int((-2i16 as u16) as u64, false))
            .or_llvm_err()?; // EVFILT_WRITE = -2
        // SAFETY: GEP into the 32-byte kevent struct at byte 10 (flags field) for EV_ADD|EV_ENABLE
        let flags_p_w = unsafe {
            builder
                .build_gep(i8_type, kev_w, &[i64_type.const_int(10, false)], "flp_w")
                .or_llvm_err()?
        };
        builder
    ...
    fn emit_io_poll(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let block_forever = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(block_forever, no_timeout, with_timeout)
            .or_llvm_err()?;
        ...
        let ts = builder
            .build_alloca(i8_type.array_type(16), "ts")
            .or_llvm_err()?;
        builder
            .build_memset(ts, 1, i8_type.const_zero(), i64_type.const_int(16, false))
            .or_llvm_err()?;
        let billion = i64_type.const_int(1_000_000_000, false);
        let secs = builder
            .build_int_signed_div(timeout_ns, billion, "secs")
            .or_llvm_err()?;
        let nsecs = builder
            .build_int_signed_rem(timeout_ns, billion, "nsecs")
            .or_llvm_err()?;
        // offset 0: tv_sec (i64)
        ...
        let ns_p = unsafe {
            builder
                .build_gep(i8_type, ts, &[i64_type.const_int(8, false)], "ns_p")
                .or_llvm_err()?
        };
    ...
    fn emit_io_remove(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let filt_p = unsafe {
            builder
                .build_gep(i8_type, kev, &[i64_type.const_int(8, false)], "fp")
                .or_llvm_err()?
        };
        builder
            .build_store(filt_p, i16_type.const_int((-1i16 as u16) as u64, false))
            .or_llvm_err()?;
        // SAFETY: GEP into the 32-byte kevent struct at byte 10 (flags field) for EV_DELETE
        let flags_p = unsafe {
            builder
                .build_gep(i8_type, kev, &[i64_type.const_int(10, false)], "flp")
                .or_llvm_err()?
        };
        builder
        ...
        let ok = builder
            ...
            .or_llvm_err()?;
        let res = builder
            .build_select(ok, i64_type.const_zero(), i64_type.const_all_ones(), "res")
            .or_llvm_err()?;
    ...
    fn emit_io_modify(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let submit_fn = module
            .get_function("verum_io_submit")
            .or_missing_fn("verum_io_submit")?;
    ...
    fn emit_io_engine_destroy(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let kq_fd = builder
            ...
            .into_int_value();
        builder
            .build_call(close_fn, &[kq_fd.into()], "")
            .or_llvm_err()?;
        ...
        builder
            .build_call(
                dealloc_fn,
                &[eng_ptr.into(), i64_type.const_int(8, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder.build_return(None).or_llvm_err()?;
    ...
    fn emit_io_submit_both(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let kevs = builder
            .build_alloca(i8_type.array_type(64), "kevs")
            .or_llvm_err()?;
        builder
            .build_memset(kevs, 1, i8_type.const_zero(), i64_type.const_int(64, false))
            .or_llvm_err()?;
        ...
        let filt0 = unsafe {
            builder
                .build_gep(i8_type, kevs, &[i64_type.const_int(8, false)], "f0")
                .or_llvm_err()?
        };
        builder
            .build_store(filt0, i16_type.const_int((-1i16 as u16) as u64, false))
            .or_llvm_err()?;
        // SAFETY: GEP into kevent[0] at byte 10 (flags field) in the 64-byte two-kevent array
        let flags0 = unsafe {
            builder
                .build_gep(i8_type, kevs, &[i64_type.const_int(10, false)], "fl0")
                .or_llvm_err()?
        };
        builder
            .build_store(flags0, i16_type.const_int(5, false))
            .or_llvm_err()?;
        ...
        let kev1 = unsafe {
            builder
                .build_gep(i8_type, kevs, &[i64_type.const_int(32, false)], "kev1")
                .or_llvm_err()?
        };
        ...
        let filt1 = unsafe {
            builder
                .build_gep(i8_type, kevs, &[i64_type.const_int(40, false)], "f1")
                .or_llvm_err()?
        };
        builder
            .build_store(filt1, i16_type.const_int((-2i16 as u16) as u64, false))
            .or_llvm_err()?;
        // SAFETY: GEP into kevent[1] at byte 42 (flags field of second kevent)
        let flags1 = unsafe {
            builder
                .build_gep(i8_type, kevs, &[i64_type.const_int(42, false)], "fl1")
                .or_llvm_err()?
        };
        builder
    ...
    fn emit_pool_worker_entry(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let pool_ptr = builder
            .build_int_to_ptr(pool_i64, ptr_type, "pool_ptr")
            .or_llvm_err()?;
        ...
        let mtx_p = unsafe {
            builder
                .build_gep(i8_type, pool_ptr, &[i64_type.const_int(40, false)], "mtx_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP into the pool struct at offset 44 (condvar)
        let cv_p = unsafe {
            builder
                .build_gep(i8_type, pool_ptr, &[i64_type.const_int(44, false)], "cv_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP into the pool struct at offset 32 (queue length)
        let len_p = unsafe {
            builder
                .build_gep(i8_type, pool_ptr, &[i64_type.const_int(32, false)], "len_p")
                .or_llvm_err()?
        };
        ...
        let cap_p = unsafe {
            builder
                .build_gep(i8_type, pool_ptr, &[i64_type.const_int(8, false)], "cap_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP into the pool struct at offset 64 (shutdown flag)
        let shutdown_p = unsafe {
            builder
                .build_gep(i8_type, pool_ptr, &[i64_type.const_int(64, false)], "sd_p")
                .or_llvm_err()?
        };
        // SAFETY: GEP into the pool struct at offset 72 (not_full condvar)
        let not_full_cv_p = unsafe {
            builder
                .build_gep(
                    i8_type,
                    pool_ptr,
                    &[i64_type.const_int(72, false)],
                    "nf_cv_p",
                )
                .or_llvm_err()?
        };
        ...
        let len = builder
            ...
            .or_llvm_err()?
            .into_int_value();
        let has_task = builder
            .build_int_compare(IntPredicate::SGT, len, i64_type.const_zero(), "has_task")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(has_task, dequeue_bb, wait_bb)
            .or_llvm_err()?;

        // wait: if shutdown, exit; else cond_wait then check again
        builder.position_at_end(wait_bb);
        let is_shutdown = builder
            .build_int_compare(IntPredicate::NE, sd, i64_type.const_zero(), "is_sd")
            .or_llvm_err()?;
        let wait_body = ctx.append_basic_block(func, "wait_body");
        builder
            .build_conditional_branch(is_shutdown, exit_bb, wait_body)
            .or_llvm_err()?;
        ...
        let queue_i64 = builder
            .build_load(i64_type, pool_ptr, "qi64")
            .or_llvm_err()?
            .into_int_value();
        let queue_ptr = builder
            .build_int_to_ptr(queue_i64, ptr_type, "queue_ptr")
            .or_llvm_err()?;
        let head = builder
            .build_load(i64_type, head_p, "head")
            .or_llvm_err()?
        ...
        let task_off = builder
            .build_int_mul(head, i64_type.const_int(32, false), "toff")
            .or_llvm_err()?;
        // SAFETY: GEP to access a struct field at a fixed offset; the struct was allocated with sufficient size for all fields
        let task_p = unsafe {
            builder
                .build_gep(i8_type, queue_ptr, &[task_off], "task_p")
                .or_llvm_err()?
        };

        // Load task fields: func_ptr(+0), arg(+8), result_ptr(+16), done_ptr(+24)
        let func_ptr_val = builder
            .build_load(i64_type, task_p, "func_ptr")
            .or_llvm_err()?
            .into_int_value();
        // SAFETY: GEP into the 32-byte task struct at offset 8 (arg field)
        let arg_p = unsafe {
            builder
                .build_gep(i8_type, task_p, &[i64_type.const_int(8, false)], "arg_p")
                .or_llvm_err()?
        };
        let arg_val = builder
            .build_load(i64_type, arg_p, "arg")
            .or_llvm_err()?
            .into_int_value();
        // SAFETY: GEP into the 32-byte task struct at offset 16 (result_ptr field)
        let rp_p = unsafe {
            builder
                .build_gep(i8_type, task_p, &[i64_type.const_int(16, false)], "rp_p")
                .or_llvm_err()?
        };
        let result_ptr_val = builder
            .build_load(i64_type, rp_p, "result_ptr")
            .or_llvm_err()?
            .into_int_value();
        // SAFETY: GEP into the 32-byte task struct at offset 24 (done_ptr field)
        let dp_p = unsafe {
            builder
                .build_gep(i8_type, task_p, &[i64_type.const_int(24, false)], "dp_p")
                .or_llvm_err()?
        };
        let done_ptr_val = builder
            .build_load(i64_type, dp_p, "done_ptr")
            .or_llvm_err()?
            .into_int_value();
        ...
        // len--
        let cur_len = builder
            .build_load(i64_type, len_p, "cl")
            .or_llvm_err()?
            .into_int_value();
        let new_len = builder
            .build_int_sub(cur_len, i64_type.const_int(1, false), "nl")
            .or_llvm_err()?;
        builder.build_store(len_p, new_len).or_llvm_err()?;

        // Signal not_full condvar (unblocks submitters waiting on full queue)
        builder
            .build_call(cond_signal, &[not_full_cv_p.into()], "")
            .or_llvm_err()?;
        ...
        let result_p = builder
            .build_int_to_ptr(result_ptr_val, ptr_type, "rp")
            .or_llvm_err()?;
        builder.build_store(result_p, result).or_llvm_err()?;

        // Set done flag at done_ptr (atomic store release)
        let done_p_ptr = builder
            .build_int_to_ptr(done_ptr_val, ptr_type, "dp")
            .or_llvm_err()?;
        let store = builder
            .build_store(done_p_ptr, i64_type.const_int(1, false))
            .or_llvm_err()?;
        store
            .set_atomic_ordering(verum_llvm::AtomicOrdering::Release)
            .or_llvm_err()?;
    ...
    fn emit_pool_create(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let spawn_fn = self.get_or_declare_fn(
            module,
            "verum_thread_spawn",
            i64_type.fn_type(&[i64_type.into(), i64_type.into()], false),
        );
        // pool_worker_entry is a C function — declare as extern
        let worker_entry_fn = self.get_or_declare_fn(
            module,
            "pool_worker_entry",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        ...
        let queue_i64 = builder
            .build_ptr_to_int(queue_ptr, i64_type, "qi64")
            .or_llvm_err()?;
        ...
        builder
            .build_store(cap_p, i64_type.const_int(1024, false))
            .or_llvm_err()?;
        ...
        let threads_sz = builder
            .build_int_mul(num_workers, i64_type.const_int(8, false), "tsz")
            .or_llvm_err()?;
        ...
        let threads_i64 = builder
            .build_ptr_to_int(threads_ptr, i64_type, "ti64")
            .or_llvm_err()?;
        // SAFETY: GEP into the pool struct at offset 48 (thread handles array pointer)
        let tp_p = unsafe {
            builder
                .build_gep(i8_type, pool_ptr, &[i64_type.const_int(48, false)], "tp_p")
                .or_llvm_err()?
        };
        ...
        let nw_p = unsafe {
            builder
                .build_gep(i8_type, pool_ptr, &[i64_type.const_int(56, false)], "nw_p")
                .or_llvm_err()?
        };
        ...
        let pool_i64 = builder
            .build_ptr_to_int(pool_ptr, i64_type, "pi64")
            .or_llvm_err()?;

        // Get worker_entry as i64 function pointer
        let worker_ptr = builder
            .build_ptr_to_int(
                worker_entry_fn.as_global_value().as_pointer_value(),
                i64_type,
                "wptr",
            )
            .or_llvm_err()?;
        ...
        builder
            .build_unconditional_branch(spawn_loop)
            .or_llvm_err()?;
        ...
        let i_val = i_phi.as_basic_value().into_int_value();
        let done = builder
            .build_int_compare(IntPredicate::SGE, i_val, num_workers, "done")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(done, spawn_done, {
        ...
        let slot = unsafe {
            builder
                .build_gep(i8_type, threads_ptr, &[off], "slot")
                .or_llvm_err()?
        };
    ...
    fn emit_pool_submit(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let handle_i64 = builder
            .build_ptr_to_int(handle_ptr, i64_type, "hi64")
            .or_llvm_err()?;
        ...
        let done_i64 = builder
            .build_ptr_to_int(done_p, i64_type, "di64")
            .or_llvm_err()?;
        ...
        builder.position_at_end(wait_loop);
        let wl_len = builder
            .build_load(i64_type, len_p, "wl_len")
            .or_llvm_err()?
            .into_int_value();
        let wl_cap = builder
            .build_load(i64_type, cap_p, "wl_cap")
            .or_llvm_err()?
            .into_int_value();
        let is_full = builder
            .build_int_compare(IntPredicate::UGE, wl_len, wl_cap, "is_full")
            .or_llvm_err()?;
        let wait_body = ctx.append_basic_block(func, "wait_body");
        builder
            .build_conditional_branch(is_full, wait_body, do_submit)
            .or_llvm_err()?;

        builder.position_at_end(wait_body);
        builder
            .build_call(cond_wait, &[nf_cv_p.into(), mtx_p.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(wait_loop)
            .or_llvm_err()?;
        ...
        // Load queue fields
        let queue_p = builder
            .build_load(i64_type, pool_ptr, "queue_i64")
            .or_llvm_err()?
            .into_int_value();
        let queue_ptr_v = builder
            .build_int_to_ptr(queue_p, ptr_type, "queue_ptr")
            .or_llvm_err()?;
        ...
        let task_off = builder
            .build_int_mul(tail, i64_type.const_int(32, false), "toff")
            .or_llvm_err()?;
        // SAFETY: GEP into the task queue ring buffer at task[tail]; tail < cap, within allocated buffer
        let task_p = unsafe {
            builder
                .build_gep(i8_type, queue_ptr_v, &[task_off], "task_p")
                .or_llvm_err()?
        };
        // task.func_ptr at +0
        ...
        let tail_inc = builder
            ...
            .or_llvm_err()?;
        let new_tail = builder
            .build_int_unsigned_rem(tail_inc, cap, "nt")
            .or_llvm_err()?;
        builder.build_store(tail_p, new_tail).or_llvm_err()?;
        ...
        let len = builder
            ...
            .into_int_value();
        let new_len = builder
            .build_int_add(len, i64_type.const_int(1, false), "nl")
            .or_llvm_err()?;
        builder.build_store(len_p, new_len).or_llvm_err()?;

        // Signal not_empty condvar (wake workers), unlock
        builder
            .build_call(cond_signal, &[cv_p.into()], "")
            .or_llvm_err()?;
        builder
            .build_call(mutex_unlock, &[mtx_p.into()], "")
            .or_llvm_err()?;
    ...
    fn emit_pool_await(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let handle_i64 = func
            ...
            .into_int_value();
        let handle_ptr = builder
            .build_int_to_ptr(handle_i64, ptr_type, "handle_ptr")
            .or_llvm_err()?;
        ...
        builder
            .build_unconditional_branch(spin_phase)
            .or_llvm_err()?;
        ...
        done_load
            ...
            .or_internal("expected instruction")?
            .set_atomic_ordering(verum_llvm::AtomicOrdering::Acquire)
            .or_llvm_err()?;
        let done_val = done_load.into_int_value();
        let is_done = builder
            .build_int_compare(IntPredicate::NE, done_val, i64_type.const_zero(), "is_done")
            .or_llvm_err()?;

        let spin_cont = ctx.append_basic_block(func, "spin_cont");
        builder
            .build_conditional_branch(is_done, done_bb, spin_cont)
            .or_llvm_err()?;

        builder.position_at_end(spin_cont);
        let sc_next = builder
            .build_int_add(sc_val, i64_type.const_int(1, false), "scn")
            .or_llvm_err()?;
        ...
        let exhausted = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(exhausted, yield_phase, spin_phase)
            .or_llvm_err()?;
        ...
        let is_done2 = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_done2, done_bb, yield_phase)
            .or_llvm_err()?;

        // done: load result from offset 0, dealloc, return
        builder.position_at_end(done_bb);
        let result = builder
            .build_load(i64_type, handle_ptr, "result")
            .or_llvm_err()?
            .into_int_value();
        builder
            .build_call(
                dealloc_fn,
                &[handle_ptr.into(), i64_type.const_int(16, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
    ...
    fn emit_pool_destroy(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let mutex_unlock = module
            .get_function("verum_mutex_unlock")
            .or_missing_fn("verum_mutex_unlock")?;
        let join_fn = self.get_or_declare_fn(
            module,
            "verum_thread_join",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        ...
        builder
            .build_store(shutdown_p, i64_type.const_int(1, false))
            .or_llvm_err()?;
        ...
        let threads_i64 = builder
            .build_load(i64_type, tp_p, "ti64")
            .or_llvm_err()?
            .into_int_value();
        let threads_ptr = builder
            .build_int_to_ptr(threads_i64, ptr_type, "threads_ptr")
            .or_llvm_err()?;

        builder
            .build_unconditional_branch(join_loop)
            .or_llvm_err()?;
        ...
        builder
            .build_conditional_branch(done, join_done, join_body)
            .or_llvm_err()?;
        ...
        let tid = builder
            .build_load(i64_type, slot, "tid")
            .or_llvm_err()?
            .into_int_value();
        builder
            .build_call(join_fn, &[tid.into()], "")
            .or_llvm_err()?;
        let next_i = builder
            .build_int_add(i_val, i64_type.const_int(1, false), "ni")
            .or_llvm_err()?;
        ...
        let queue_i64 = builder
            ...
            .into_int_value();
        let queue_ptr = builder
            .build_int_to_ptr(queue_i64, ptr_type, "qp")
            .or_llvm_err()?;
        builder
            .build_call(
                dealloc_fn,
                &[queue_ptr.into(), i64_type.const_int(32768, false).into()],
                "",
            )
            .or_llvm_err()?;
        // Dealloc threads
        let tsz = builder
            ...
            .or_llvm_err()?;
        builder
            .build_call(dealloc_fn, &[threads_ptr.into(), tsz.into()], "")
            .or_llvm_err()?;
        // Dealloc pool (80 bytes: includes not_full condvar at offset 72)
        builder
            .build_call(
                dealloc_fn,
                &[pool_ptr.into(), i64_type.const_int(80, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder.build_return(None).or_llvm_err()?;
    ...
    fn emit_pool_global(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let create_fn = self.get_or_declare_fn(
            module,
            "verum_pool_create",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        ...
        let cur = builder
            .build_load(i64_type, gp, "cur")
            .or_llvm_err()?
            .into_int_value();
        let is_null = builder
            .build_int_compare(IntPredicate::EQ, cur, i64_type.const_zero(), "is_null")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_null, create_bb, done_bb)
            .or_llvm_err()?;
        ...
        let num_cpus = {
            ...
            // Clamp: min 2, max 64
            let min_2 = builder
                .build_int_compare(IntPredicate::SLT, ncpu, i64_type.const_int(2, false), "lt2")
                .or_llvm_err()?;
            let clamped_low: verum_llvm::values::IntValue = builder
                .build_select(min_2, i64_type.const_int(2, false), ncpu, "cl")
                .or_llvm_err()?
                .into_int_value();
            let max_64 = builder
                .build_int_compare(
                ...
                .or_llvm_err()?;
            let result: verum_llvm::values::IntValue = builder
                .build_select(max_64, i64_type.const_int(64, false), clamped_low, "ch")
                .or_llvm_err()?
                .into_int_value();
            result
        };
        ...
        builder
            .build_conditional_branch(success, cas_ok, cas_fail)
            .or_llvm_err()?;
        ...
        let destroy_fn = self.get_or_declare_fn(
            module,
            "verum_pool_destroy",
            ctx.void_type().fn_type(&[i64_type.into()], false),
        );
        builder
            .build_call(destroy_fn, &[new_pool.into()], "")
            .or_llvm_err()?;
        builder.build_unconditional_branch(done_bb).or_llvm_err()?;
    ...
    fn emit_pool_global_submit(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let global_fn = module
            .get_function("verum_pool_global")
            .or_missing_fn("verum_pool_global")?;
        let submit_fn = module
            .get_function("verum_pool_submit")
            .or_missing_fn("verum_pool_submit")?;
        ...
        let handle = builder
            .build_call(
                submit_fn,
                &[pool.into(), func_arg.into(), arg.into()],
                "handle",
            )
            .or_llvm_err()?
            .basic_value_or("expected basic value")?
    ...
    fn emit_async_accept(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let submit_fn = self.get_or_declare_fn(
            module,
            "verum_io_submit",
            i64_type.fn_type(&[i64_type.into(), i64_type.into(), i64_type.into()], false),
        );
        let poll_fn = self.get_or_declare_fn(
            module,
            "verum_io_poll",
            i64_type.fn_type(
                &[
        ...
        let accept_fn = self.get_or_declare_fn(
            module,
            "accept",
    ...
    fn emit_gen_create(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let gen_mtx_init_fn = self.get_or_declare_fn(
            module,
            "gen_mtx_init",
            ctx.void_type().fn_type(&[ptr_type.into()], false),
        );
        let gen_cv_init_fn = self.get_or_declare_fn(
            module,
            "gen_cv_init",
            ctx.void_type().fn_type(&[ptr_type.into()], false),
        );
        ...
        let gen_thread_entry_fn = self.get_or_declare_fn(
            module,
            "gen_thread_entry",
        ...
        let nargs_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(8, false)], "nargs_p")
                .or_llvm_err()?
        };
        ...
        let mutex_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(40, false)], "mtx_p")
                .or_llvm_err()?
        };
        builder
            .build_call(gen_mtx_init_fn, &[mutex_p.into()], "")
            .or_llvm_err()?;
        // SAFETY: GEP at a known offset within an allocated object; the pointer is valid and the offset does not exceed the allocation size
        let cv_caller_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(44, false)], "cv_c_p")
                .or_llvm_err()?
        };
        builder
            .build_call(gen_cv_init_fn, &[cv_caller_p.into()], "")
            .or_llvm_err()?;
        // SAFETY: GEP into the generator struct at offset 48 (cv_gen condvar)
        let cv_gen_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(48, false)], "cv_g_p")
                .or_llvm_err()?
        };
        builder
            .build_call(gen_cv_init_fn, &[cv_gen_p.into()], "")
            .or_llvm_err()?;

        // Copy args if num_args > 0
        let has_args = builder
            .build_int_compare(
                IntPredicate::SGT,
                num_args,
                i64_type.const_zero(),
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(has_args, copy_args, no_args)
            .or_llvm_err()?;

        // copy_args: alloc num_args*8, memcpy, store at offset 16 (args ptr)
        builder.position_at_end(copy_args);
        let args_sz = builder
            .build_int_mul(num_args, i64_type.const_int(8, false), "asz")
            .or_llvm_err()?;
        ...
        builder
            .build_call(
                memcpy_fn,
                &[args_copy.into(), args.into(), args_sz.into()],
                "",
            )
            .or_llvm_err()?;
        let args_i64 = builder
            .build_ptr_to_int(args_copy, i64_type, "ai64")
            .or_llvm_err()?;
        // SAFETY: GEP into the generator struct at offset 16 (args_ptr field) to store copied arguments
        let argsp_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(16, false)], "argsp")
                .or_llvm_err()?
        };
        ...
        let tid = builder
            .build_load(i64_type, tid_alloca, "tid")
            .or_llvm_err()?;
        // SAFETY: GEP into the generator struct at offset 56 (thread handle)
        let th_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(56, false)], "th_p")
                .or_llvm_err()?
        };
    ...
    fn emit_gen_next(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let gen_lock_fn = self.get_or_declare_fn(
            module,
            "gen_mtx_lock",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        let gen_unlock_fn = self.get_or_declare_fn(
            module,
            "gen_mtx_unlock",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        let gen_signal_fn = self.get_or_declare_fn(
            module,
            "gen_cv_signal",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        let gen_wait_fn = self.get_or_declare_fn(
            module,
            "gen_cv_wait",
            void_type.fn_type(&[ptr_type.into(), ptr_type.into()], false),
        );
        ...
        let handle = func
            ...
            .into_int_value();
        let gen_ptr = builder
            .build_int_to_ptr(handle, ptr_type, "gen")
            .or_llvm_err()?;
        ...
        let mutex_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(40, false)], "mtx")
                .or_llvm_err()?
        };
        builder
            .build_call(gen_lock_fn, &[mutex_p.into()], "")
            .or_llvm_err()?;
        ...
        let status_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(32, false)], "st_p")
                .or_llvm_err()?
        };
        builder
            .build_store(status_p, i64_type.const_int(1, false))
            .or_llvm_err()?;
        ...
        let cv_gen_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(48, false)], "cvg")
                .or_llvm_err()?
        };
        builder
            .build_call(gen_signal_fn, &[cv_gen_p.into()], "")
            .or_llvm_err()?;
        ...
        let cv_caller_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(44, false)], "cvc")
                .or_llvm_err()?
        };
        builder
            .build_unconditional_branch(wait_loop)
        ...
        builder.position_at_end(wait_loop);
        builder
            .build_call(gen_wait_fn, &[cv_caller_p.into(), mutex_p.into()], "")
            .or_llvm_err()?;
        let status = builder
            .build_load(i64_type, status_p, "st")
            .or_llvm_err()?
            .into_int_value();
        let still_running = builder
            .build_int_compare(
                IntPredicate::EQ,
                status,
                i64_type.const_int(1, false),
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(still_running, wait_loop, done)
            .or_llvm_err()?;
        ...
        let final_status = builder
            .build_load(i64_type, status_p, "fst")
            .or_llvm_err()?
            .into_int_value();
        ...
        builder
            .build_conditional_branch(is_completed, completed_bb, yield_bb)
            .or_llvm_err()?;
        ...
        let value_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(24, false)], "val_p")
                .or_llvm_err()?
        };
        let value = builder
            .build_load(i64_type, value_p, "val")
            .or_llvm_err()?
            .into_int_value();
        builder
            .build_call(gen_unlock_fn, &[mutex_p.into()], "")
            .or_llvm_err()?;
    ...
    fn emit_gen_has_next(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let not_completed = builder
            .build_int_compare(IntPredicate::NE, status, i64_type.const_int(3, false), "nc")
            .or_llvm_err()?;
        let result = builder
            .build_int_z_extend(not_completed, i64_type, "res")
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_gen_next_maybe(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let has_next_fn = self.get_or_declare_fn(
            module,
            "verum_gen_has_next",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        let next_fn = self.get_or_declare_fn(
            module,
            "verum_gen_next",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        ...
        let has = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(has, has_bb, none_bb)
            .or_llvm_err()?;
        ...
        let gen_ptr = builder
            .build_int_to_ptr(handle, ptr_type, "gp")
            .or_llvm_err()?;
        // SAFETY: GEP into the generator struct at offset 32 (status field) to check completion
        let status_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(32, false)], "sp2")
                .or_llvm_err()?
        };
        let st = builder
            .build_load(i64_type, status_p, "st2")
            .or_llvm_err()?
            .into_int_value();
        let completed = builder
            .build_int_compare(IntPredicate::EQ, st, i64_type.const_int(3, false), "cmp")
            .or_llvm_err()?;
        let some_bb = ctx.append_basic_block(func, "some");
        builder
            .build_conditional_branch(completed, none_bb, some_bb)
            .or_llvm_err()?;

        builder.position_at_end(some_bb);
        builder
            .build_store(out_tag, i64_type.const_int(1, false))
            .or_llvm_err()?;
        ...
        builder
            .build_store(out_tag, i64_type.const_zero())
            .or_llvm_err()?;
        builder.build_return(None).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_gen_close(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_store(status_p, i64_type.const_int(3, false))
            .or_llvm_err()?;
        ...
        let th = builder
            .build_load(ptr_type, th_p, "th")
            .or_llvm_err()?
            .into_pointer_value();
        builder
            .build_call(
                pthread_join_fn,
                &[th.into(), ptr_type.const_null().into()],
                "",
            )
            .or_llvm_err()?;
        ...
        let argsp_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(24, false)], "argsp")
                .or_llvm_err()?
        };
        let args_i64 = builder
            .build_load(i64_type, argsp_p, "args_i64")
            .or_llvm_err()?
            .into_int_value();
        let has_args = builder
            .build_int_compare(IntPredicate::NE, args_i64, i64_type.const_zero(), "ha")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(has_args, dealloc_args, finish)
            .or_llvm_err()?;

        // dealloc_args: free args array
        builder.position_at_end(dealloc_args);
        let args_ptr = builder
            .build_int_to_ptr(args_i64, ptr_type, "aptr")
            .or_llvm_err()?;
        ...
        let nargs_p = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(16, false)], "nap")
                .or_llvm_err()?
        };
        let nargs = builder
            .build_load(i64_type, nargs_p, "na")
            .or_llvm_err()?
            .into_int_value();
        let args_sz = builder
            .build_int_mul(nargs, i64_type.const_int(8, false), "asz")
            .or_llvm_err()?;
        builder
            .build_call(dealloc_fn, &[args_ptr.into(), args_sz.into()], "")
            .or_llvm_err()?;
        ...
        builder
            .build_call(
                dealloc_fn,
                &[gen_ptr.into(), i64_type.const_int(112, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder.build_return(None).or_llvm_err()?;
    ...
    fn emit_thread_spawn_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let mutex_init_fn = self.get_or_declare_fn(
            module,
            "verum_mutex_init",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        let cond_init_fn = self.get_or_declare_fn(
            module,
            "verum_cond_init",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        ...
        let trampoline_fn = self.get_or_declare_fn(
            module,
            "verum_thread_entry_darwin",
            ptr_type.fn_type(&[ptr_type.into()], false),
        );
        ...
        let func_p = unsafe {
            builder
                .build_gep(i8_type, th_ptr, &[i64_type.const_int(24, false)], "func_p")
                .or_llvm_err()?
        };
        ...
        let arg_p = unsafe {
            builder
                .build_gep(i8_type, th_ptr, &[i64_type.const_int(32, false)], "arg_p")
                .or_llvm_err()?
        };
        builder.build_store(arg_p, arg).or_llvm_err()?;
        ...
        let mutex_p = unsafe {
            builder
                .build_gep(i8_type, th_ptr, &[i64_type.const_int(16, false)], "mtx_p")
                .or_llvm_err()?
        };
        builder
            .build_call(mutex_init_fn, &[mutex_p.into()], "")
            .or_llvm_err()?;
        ...
        let cv_p = unsafe {
            builder
                .build_gep(i8_type, th_ptr, &[i64_type.const_int(20, false)], "cv_p")
                .or_llvm_err()?
        };
        builder
            .build_call(cond_init_fn, &[cv_p.into()], "")
            .or_llvm_err()?;
        ...
        let result = builder
            .build_ptr_to_int(th_ptr, i64_type, "res")
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_thread_join_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let mutex_lock_fn = self.get_or_declare_fn(
            module,
            "verum_mutex_lock",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        let mutex_unlock_fn = self.get_or_declare_fn(
            module,
            "verum_mutex_unlock",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        let cond_wait_fn = self.get_or_declare_fn(
            module,
            "verum_cond_wait",
            void_type.fn_type(&[ptr_type.into(), ptr_type.into()], false),
        );
        ...
        let th_i64 = func
            ...
            .into_int_value();
        let th_ptr = builder
            .build_int_to_ptr(th_i64, ptr_type, "th")
            .or_llvm_err()?;
        ...
        let mutex_p = unsafe {
            builder
                .build_gep(i8_type, th_ptr, &[i64_type.const_int(16, false)], "mtx")
                .or_llvm_err()?
        };
        builder
            .build_call(mutex_lock_fn, &[mutex_p.into()], "")
            .or_llvm_err()?;
        ...
        let cv_p = unsafe {
            builder
                .build_gep(i8_type, th_ptr, &[i64_type.const_int(20, false)], "cv")
                .or_llvm_err()?
        };
        ...
        let i32_type = ctx.i32_type();
        let done_val = builder
            .build_load(i32_type, done_p, "dv")
            .or_llvm_err()?
            .into_int_value();
        ...
        let is_done = builder
            .build_int_compare(IntPredicate::NE, done_val, i32_type.const_zero(), "id")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_done, done_bb, wait_bb)
            .or_llvm_err()?;

        // wait: cond_wait, then go back to check
        builder.position_at_end(wait_bb);
        builder
            .build_call(cond_wait_fn, &[cv_p.into(), mutex_p.into()], "")
            .or_llvm_err()?;
        builder.build_unconditional_branch(check_bb).or_llvm_err()?;
        ...
        builder.position_at_end(done_bb);
        builder
            .build_call(mutex_unlock_fn, &[mutex_p.into()], "")
            .or_llvm_err()?;
        ...
        let result_p = unsafe {
            builder
                .build_gep(i8_type, th_ptr, &[i64_type.const_int(8, false)], "res_p")
                .or_llvm_err()?
        };
        let result = builder
            .build_load(i64_type, result_p, "res")
            .or_llvm_err()?
            .into_int_value();
        ...
        let tid_ptr = builder
            .build_int_to_ptr(tid, ptr_type, "tid_ptr")
            .or_llvm_err()?;
        builder
            .build_call(
                pthread_join_fn,
                &[tid_ptr.into(), ptr_type.const_null().into()],
                "",
            )
            .or_llvm_err()?;
    ...
    fn emit_thread_is_done_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let i32_type = ctx.i32_type();
        let done_val = builder
            .build_load(i32_type, th_ptr, "dv")
            .or_llvm_err()?
            .into_int_value();
        let done_i64 = builder
            .build_int_z_extend(done_val, i64_type, "d64")
            .or_llvm_err()?;
    ...
    fn emit_verum_process_spawn(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let cap_stderr = func
            ...
            .or_internal("missing param 4")?
            .into_int_value();
        let out_stdout_fd = func
            .get_nth_param(5)
            .or_internal("missing param 5")?
            .into_pointer_value();
        let out_stderr_fd = func
            .get_nth_param(6)
            .or_internal("missing param 6")?
            .into_pointer_value();
        ...
        let stdout_pipe = builder
            .build_alloca(pipe_arr_type, "stdout_pipe")
            .or_llvm_err()?;
        let stderr_pipe = builder
            .build_alloca(pipe_arr_type, "stderr_pipe")
            .or_llvm_err()?;
        builder
            .build_store(stdout_pipe, pipe_arr_type.const_zero())
            .or_llvm_err()?;
        builder
            .build_store(stderr_pipe, pipe_arr_type.const_zero())
            .or_llvm_err()?;

        // Store -1 as default output fds
        builder
            .build_store(out_stdout_fd, i64_type.const_all_ones())
            .or_llvm_err()?;
        builder
            .build_store(out_stderr_fd, i64_type.const_all_ones())
            .or_llvm_err()?;

        let want_stdout = builder
            .build_int_compare(IntPredicate::NE, cap_stdout, i64_type.const_zero(), "ws")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(want_stdout, setup_stdout, after_stdout)
            .or_llvm_err()?;

        // setup_stdout: pipe(stdout_pipe)
        builder.position_at_end(setup_stdout);
        builder
            .build_call(pipe_fn, &[stdout_pipe.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(after_stdout)
            .or_llvm_err()?;

        builder.position_at_end(after_stdout);
        let want_stderr = builder
            .build_int_compare(IntPredicate::NE, cap_stderr, i64_type.const_zero(), "we")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(want_stderr, setup_stderr, after_stderr)
            .or_llvm_err()?;

        // setup_stderr: pipe(stderr_pipe)
        builder.position_at_end(setup_stderr);
        builder
            .build_call(pipe_fn, &[stderr_pipe.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(after_stderr)
            .or_llvm_err()?;
        ...
        let is_error = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_error, fork_fail, child_block)
            .or_llvm_err()?;
        ...
        let is_child = builder
            .build_int_compare(IntPredicate::EQ, pid, i64_type.const_zero(), "is_child")
            .or_llvm_err()?;
        let child_exec = ctx.append_basic_block(func, "child_exec");
        builder
            .build_conditional_branch(is_child, child_exec, parent_block)
            .or_llvm_err()?;
        ...
        builder
            .build_conditional_branch(want_stdout, child_stdout_cap, child_after_stdout)
            .or_llvm_err()?;

        builder.position_at_end(child_stdout_cap);
        let so_read = builder
            .build_struct_gep(pipe_arr_type, stdout_pipe, 0, "so_r")
            .or_llvm_err()?;
        let so_read_fd: verum_llvm::values::BasicValueEnum = builder
            .build_int_s_extend(
                builder
                    .build_load(i32_type, so_read, "so_rf32")
                    .or_llvm_err()?
                    .into_int_value(),
                i64_type,
                "so_rf",
            ...
            .into();
        builder
            .build_call(close_fn, &[so_read_fd.into()], "")
            .or_llvm_err()?;
        let so_write = builder
            .build_struct_gep(pipe_arr_type, stdout_pipe, 1, "so_w")
            .or_llvm_err()?;
        let so_write_fd: verum_llvm::values::BasicValueEnum = builder
            .build_int_s_extend(
                builder
                    .build_load(i32_type, so_write, "so_wf32")
                    .or_llvm_err()?
                    .into_int_value(),
                i64_type,
                "so_wf",
        ...
        builder
            .build_call(
                dup2_fn,
                &[so_write_fd.into(), i64_type.const_int(1, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder
            .build_call(close_fn, &[so_write_fd.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(child_after_stdout)
            .or_llvm_err()?;
        ...
        builder
            .build_conditional_branch(want_stderr, child_stderr_cap, child_after_stderr)
            .or_llvm_err()?;

        builder.position_at_end(child_stderr_cap);
        let se_read = builder
            .build_struct_gep(pipe_arr_type, stderr_pipe, 0, "se_r")
            .or_llvm_err()?;
        let se_read_fd: verum_llvm::values::BasicValueEnum = builder
            .build_int_s_extend(
                builder
                    .build_load(i32_type, se_read, "se_rf32")
                    .or_llvm_err()?
                    .into_int_value(),
                i64_type,
                "se_rf",
            ...
            .into();
        builder
            .build_call(close_fn, &[se_read_fd.into()], "")
            .or_llvm_err()?;
        let se_write = builder
            .build_struct_gep(pipe_arr_type, stderr_pipe, 1, "se_w")
            .or_llvm_err()?;
        let se_write_fd: verum_llvm::values::BasicValueEnum = builder
            .build_int_s_extend(
                builder
                    .build_load(i32_type, se_write, "se_wf32")
                    .or_llvm_err()?
                    .into_int_value(),
                i64_type,
                "se_wf",
        ...
        builder
            .build_call(
                dup2_fn,
                &[se_write_fd.into(), i64_type.const_int(2, false).into()],
                "",
            )
            .or_llvm_err()?;
        builder
            .build_call(close_fn, &[se_write_fd.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(child_after_stderr)
            .or_llvm_err()?;

        builder.position_at_end(child_after_stderr);
        // execvp(program, argv)
        builder
            .build_call(execvp_fn, &[program.into(), argv.into()], "")
            .or_llvm_err()?;
        // If execvp returns, _exit(127)
        builder
            .build_call(exit_fn, &[i64_type.const_int(127, false).into()], "")
            .or_llvm_err()?;
        builder.build_unreachable().or_llvm_err()?;
        ...
        builder
            .build_conditional_branch(want_stdout, par_stdout_cap, par_after_stdout)
            .or_llvm_err()?;

        builder.position_at_end(par_stdout_cap);
        let pso_write = builder
            .build_struct_gep(pipe_arr_type, stdout_pipe, 1, "pso_w")
            .or_llvm_err()?;
        let pso_write_fd: verum_llvm::values::BasicValueEnum = builder
            .build_int_s_extend(
                builder
                    .build_load(i32_type, pso_write, "pso_wf32")
                    .or_llvm_err()?
                    .into_int_value(),
                i64_type,
                "pso_wf",
            ...
            .into();
        builder
            .build_call(close_fn, &[pso_write_fd.into()], "")
            .or_llvm_err()?;
        let pso_read = builder
            .build_struct_gep(pipe_arr_type, stdout_pipe, 0, "pso_r")
            .or_llvm_err()?;
        let pso_read_fd: verum_llvm::values::BasicValueEnum = builder
            .build_int_s_extend(
                builder
                    .build_load(i32_type, pso_read, "pso_rf32")
                    .or_llvm_err()?
                    .into_int_value(),
                i64_type,
                "pso_rf",
            ...
            .into();
        builder
            .build_store(out_stdout_fd, pso_read_fd)
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(par_after_stdout)
            .or_llvm_err()?;
        ...
        builder
            .build_conditional_branch(want_stderr, par_stderr_cap, par_after_stderr)
            .or_llvm_err()?;

        builder.position_at_end(par_stderr_cap);
        let pse_write = builder
            .build_struct_gep(pipe_arr_type, stderr_pipe, 1, "pse_w")
            .or_llvm_err()?;
        let pse_write_fd: verum_llvm::values::BasicValueEnum = builder
            .build_int_s_extend(
                builder
                    .build_load(i32_type, pse_write, "pse_wf32")
                    .or_llvm_err()?
                    .into_int_value(),
                i64_type,
                "pse_wf",
            ...
            .into();
        builder
            .build_call(close_fn, &[pse_write_fd.into()], "")
            .or_llvm_err()?;
        let pse_read = builder
            .build_struct_gep(pipe_arr_type, stderr_pipe, 0, "pse_r")
            .or_llvm_err()?;
        let pse_read_fd: verum_llvm::values::BasicValueEnum = builder
            .build_int_s_extend(
                builder
                    .build_load(i32_type, pse_read, "pse_rf32")
                    .or_llvm_err()?
                    .into_int_value(),
                i64_type,
                "pse_rf",
            ...
            .into();
        builder
            .build_store(out_stderr_fd, pse_read_fd)
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(par_after_stderr)
            .or_llvm_err()?;
    ...
    fn emit_verum_process_run(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let fd_read_all_fn = self.get_or_declare_fn(
            module,
            "verum_fd_read_all",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        let wait_fn = self.get_or_declare_fn(
            module,
            "verum_process_wait",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        let fd_close_fn = self.get_or_declare_fn(
            module,
            "verum_fd_close",
            ctx.void_type().fn_type(&[i64_type.into()], false),
        );
        ...
        builder
            .build_store(stdout_fd_alloca, i64_type.const_all_ones())
            .or_llvm_err()?;
        builder
            .build_store(stderr_fd_alloca, i64_type.const_all_ones())
            .or_llvm_err()?;
        ...
        let pid = builder
            .build_call(
                ...
                &[
                    ...
                    one.into(),
                    one.into(),
                    stdout_fd_alloca.into(),
                    stderr_fd_alloca.into(),
                ],
                "pid",
            )
            .or_llvm_err()?
        ...
        let pid_ok = builder
            .build_int_compare(IntPredicate::SGE, pid, i64_type.const_zero(), "ok")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(pid_ok, spawn_ok, spawn_fail)
            .or_llvm_err()?;
        ...
        let stdout_fd = builder
            .build_load(i64_type, stdout_fd_alloca, "sofd")
            .or_llvm_err()?
            .into_int_value();
        ...
        builder
            .build_call(fd_close_fn, &[stdout_fd.into()], "")
            .or_llvm_err()?;

        // Read stderr
        let stderr_fd = builder
            .build_load(i64_type, stderr_fd_alloca, "sefd")
            .or_llvm_err()?
            .into_int_value();
        ...
        builder
            .build_call(fd_close_fn, &[stderr_fd.into()], "")
            .or_llvm_err()?;
    ...
    fn emit_verum_process_spawn_cmd(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let list_ptr = builder
            .build_int_to_ptr(args_list, ptr_type, "list_ptr")
            .or_llvm_err()?;
        ...
        let len_ptr = unsafe {
            builder
                .build_gep(i8_type, list_ptr, &[i64_type.const_int(32, false)], "len_p")
                .or_llvm_err()?
        };
        let argc = builder
            .build_load(i64_type, len_ptr, "argc")
            .or_llvm_err()?
            .into_int_value();
        ...
        let backing_ptr_ptr = unsafe {
            builder
                .build_gep(i8_type, list_ptr, &[i64_type.const_int(24, false)], "bp_p")
                .or_llvm_err()?
        };
        let backing_i64 = builder
            .build_load(i64_type, backing_ptr_ptr, "bp_i64")
            .or_llvm_err()?
            .into_int_value();
        let backing = builder
            .build_int_to_ptr(backing_i64, ptr_type, "backing")
            .or_llvm_err()?;

        // Allocate argv array: (argc + 2) pointers — [program, arg0, arg1, ..., NULL]
        let argc_plus2 = builder
            .build_int_add(argc, i64_type.const_int(2, false), "ap2")
            .or_llvm_err()?;
        let argv_size = builder
            .build_int_mul(argc_plus2, i64_type.const_int(8, false), "avs")
            .or_llvm_err()?;
        ...
        let has_args = builder
            .build_int_compare(IntPredicate::SGT, argc, i64_type.const_zero(), "ha")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(has_args, build_argv_loop, build_done)
            .or_llvm_err()?;
        ...
        let elem_ptr = unsafe {
            builder
                .build_gep(i64_type, backing, &[i_val], "elem_p")
                .or_llvm_err()?
        };
        let text_val = builder
            .build_load(i64_type, elem_ptr, "tv")
            .or_llvm_err()?
            .into_int_value();
        let cstr = builder
        ...
        let argv_slot = unsafe {
            builder
                .build_gep(ptr_type, argv_buf, &[idx_plus1], "avs")
                .or_llvm_err()?
        };
        ...
        let next_i = builder
            ...
            .or_llvm_err()?;
        let done = builder
            .build_int_compare(IntPredicate::UGE, next_i, argc, "done")
            .or_llvm_err()?;
        i_phi.add_incoming(&[(&next_i, build_argv_loop)]);
        builder
            .build_conditional_branch(done, build_done, build_argv_loop)
            .or_llvm_err()?;

        // build_done: set argv[argc+1] = null, call spawn
        builder.position_at_end(build_done);
        let null_idx = builder
            .build_int_add(argc, i64_type.const_int(1, false), "ni2")
            .or_llvm_err()?;
        // SAFETY: GEP to write the null terminator at argv[argc+1]; array allocated for argc+2 pointers
        let null_slot = unsafe {
            builder
                .build_gep(ptr_type, argv_buf, &[null_idx], "ns")
                .or_llvm_err()?
        };
        builder
            .build_store(null_slot, ptr_type.const_null())
            .or_llvm_err()?;
        ...
        builder
            .build_store(out_stdout_alloca, i64_type.const_all_ones())
            .or_llvm_err()?;
        builder
            .build_store(out_stderr_alloca, i64_type.const_all_ones())
            .or_llvm_err()?;

        let total_argc = builder
            .build_int_add(argc, i64_type.const_int(1, false), "ta")
            .or_llvm_err()?;
        let pid = builder
            .build_call(
                spawn_fn,
                &[
                    ...
                    cap_stdout.into(),
                    cap_stderr.into(),
                    out_stdout_alloca.into(),
                    out_stderr_alloca.into(),
                ],
                "pid",
            )
            .or_llvm_err()?
    ...
    fn emit_verum_process_exec(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let run_fn = self.get_or_declare_fn(
            module,
            "verum_process_run",
            i64_type.fn_type(
                &[
        ...
        let list_ptr = builder
            .build_int_to_ptr(args_list, ptr_type, "lp")
            .or_llvm_err()?;
        // SAFETY: GEP into the 48-byte list header at offset 32 (len field) to read the argv list length
        let len_ptr = unsafe {
            builder
        ...
        let backing_i64 = builder
            .build_load(i64_type, bp_ptr, "bi64")
            .or_llvm_err()?
            .into_int_value();
        let backing = builder
            .build_int_to_ptr(backing_i64, ptr_type, "backing")
        ...
        let has_args = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(has_args, build_loop, build_done)
            .or_llvm_err()?;
        ...
        let elem_ptr = unsafe {
            builder
                .build_gep(i64_type, backing, &[i_val], "ep")
                .or_llvm_err()?
        };
        let text_val = builder
            .build_load(i64_type, elem_ptr, "tv")
        ...
        let slot = unsafe {
            builder
                .build_gep(ptr_type, argv_buf, &[ip1], "sl")
                .or_llvm_err()?
        };
        ...
        builder
            .build_conditional_branch(done, build_done, build_loop)
            .or_llvm_err()?;

        builder.position_at_end(build_done);
        // Set null terminator
        let null_idx = builder
            .build_int_add(argc, i64_type.const_int(1, false), "nidx")
            .or_llvm_err()?;
        ...
        builder
            .build_store(status_alloca, i64_type.const_zero())
            .or_llvm_err()?;
        builder
            .build_store(stdout_alloca, i64_type.const_zero())
            .or_llvm_err()?;
        builder
            .build_store(stderr_alloca, i64_type.const_zero())
            .or_llvm_err()?;
        ...
        let ok = builder
            .build_int_compare(IntPredicate::EQ, rc, i64_type.const_zero(), "ok")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(ok, run_ok, run_fail)
            .or_llvm_err()?;
        ...
        let raw_status = builder
            .build_load(i64_type, status_alloca, "rs")
            .or_llvm_err()?
            .into_int_value();
        let shifted = builder
            .build_right_shift(raw_status, i64_type.const_int(8, false), false, "sh")
            .or_llvm_err()?;
        let masked = builder
            .build_and(shifted, i64_type.const_int(0xFF, false), "wes")
            .or_llvm_err()?;
        ...
        // verum_futex_wait(addr: i64, expected: i64, timeout_ns: i64) -> i64
        {
            ...
            if func.count_basic_blocks() > 0 { /* already emitted */
                ...
                let has_to = builder
                    ...
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(has_to, has_timeout, no_timeout)
                    .or_llvm_err()?;

                // has_timeout: convert ns to microseconds
                builder.position_at_end(has_timeout);
                let timeout_us = builder
                ...
                let to_i32 = builder
                    .build_int_truncate(to_val, i32_type, "to32")
                    .or_llvm_err()?;
        ...
        // verum_futex_wake(addr: i64, count: i64) -> i64
        {
            ...
            if func.count_basic_blocks() > 0 { /* already emitted */
                ...
                let is_all = builder
                    ...
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(is_all, wake_all, wake_one)
                    .or_llvm_err()?;

                builder.position_at_end(wake_all);
                let flag_all = builder
                    .build_or(ulf_wake_all, ul_compare_and_wait, "flag_all")
                    .or_llvm_err()?;
    ...
    fn emit_context_system_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let void_type = ctx.void_type();
        let ptr_type = ctx.ptr_type(AddressSpace::default());
        let get_ctx_fn = self.get_or_declare_fn(
            module,
            "get_or_create_context",
            ptr_type.fn_type(&[], false),
        );
        let get_fn = self.get_or_declare_fn(
            module,
            "verum_ctx_get",
        ...
        if get_fn.count_basic_blocks() == 0 {
            let builder = ctx.create_builder();
            let entry = ctx.append_basic_block(get_fn, "entry");
            let search_loop = ctx.append_basic_block(get_fn, "search_loop");
            let check_match = ctx.append_basic_block(get_fn, "check_match");
            let found = ctx.append_basic_block(get_fn, "found");
            let not_found = ctx.append_basic_block(get_fn, "not_found");
            builder.position_at_end(entry);
            let tid32 = get_fn
                .get_first_param()
                .or_internal("missing first param")?
                .into_int_value();
            let tid = builder
                .build_int_z_extend(tid32, i64_type, "tid")
                .or_llvm_err()?;
            let ec = builder
            ...
            let cnt_p = unsafe {
                builder
                    .build_gep(i8_type, ec, &[i64_type.const_int(2048, false)], "cnt_p")
                    .or_llvm_err()?
            };
            let cnt = builder
                .build_load(i64_type, cnt_p, "cnt")
            ...
            let cb = unsafe {
                builder
                    .build_gep(i8_type, ec, &[i64_type.const_int(2056, false)], "cb")
                    .or_llvm_err()?
            };
            let si = builder
                .build_int_sub(cnt, i64_type.const_int(1, false), "si")
                .or_llvm_err()?;
            let has = builder
                .build_int_compare(IntPredicate::SGT, cnt, i64_type.const_zero(), "has")
                .or_llvm_err()?;
            builder
                .build_conditional_branch(has, search_loop, not_found)
                .or_llvm_err()?;
            builder.position_at_end(search_loop);
            let ip = builder.build_phi(i64_type, "idx").or_llvm_err()?;
            ip.add_incoming(&[(&si, entry)]);
            let idx = ip.as_basic_value().into_int_value();
            let eo = builder
                .build_int_mul(idx, i64_type.const_int(16, false), "eo")
                .or_llvm_err()?;
            // SAFETY: GEP at a fixed offset within a known struct layout; the pointer is valid from prior allocation
            let ep = unsafe { builder.build_gep(i8_type, cb, &[eo], "ep").or_llvm_err()? };
            let st = builder
                .build_load(i64_type, ep, "st")
                .or_llvm_err()?
                .into_int_value();
            let m = builder
                .build_int_compare(IntPredicate::EQ, st, tid, "m")
                .or_llvm_err()?;
            builder
                .build_conditional_branch(m, found, check_match)
                .or_llvm_err()?;
            ...
            let vo = builder
                .build_int_add(eo, i64_type.const_int(8, false), "vo")
                .or_llvm_err()?;
            let vp = unsafe { builder.build_gep(i8_type, cb, &[vo], "vp").or_llvm_err()? };
            let val = builder.build_load(i64_type, vp, "val").or_llvm_err()?;
            builder.build_return(Some(&val)).or_llvm_err()?;
            builder.position_at_end(check_match);
            let ni = builder
                .build_int_sub(idx, i64_type.const_int(1, false), "ni")
                .or_llvm_err()?;
            let cs = builder
                .build_int_compare(IntPredicate::SGE, ni, i64_type.const_zero(), "cs")
                .or_llvm_err()?;
            ip.add_incoming(&[(&ni, check_match)]);
            builder
                .build_conditional_branch(cs, search_loop, not_found)
                .or_llvm_err()?;
        ...
        let prov_fn = self.get_or_declare_fn(
            module,
            "verum_ctx_provide",
            void_type.fn_type(&[i32_type.into(), i64_type.into(), i64_type.into()], false),
        );
        if prov_fn.count_basic_blocks() == 0 {
            let builder = ctx.create_builder();
            let entry = ctx.append_basic_block(prov_fn, "entry");
            builder.position_at_end(entry);
            let tid32 = prov_fn
                .get_nth_param(0)
                .or_internal("missing param 0")?
            ...
            let eo = builder
                .build_int_mul(cnt, i64_type.const_int(16, false), "eo")
                .or_llvm_err()?;
            let tp = unsafe { builder.build_gep(i8_type, cb, &[eo], "tp").or_llvm_err()? };
            builder.build_store(tp, tid).or_llvm_err()?;
            // SAFETY: GEP into the context binding entry at offset 8 to store the value
            let vo = builder
                .build_int_add(eo, i64_type.const_int(8, false), "vo")
                .or_llvm_err()?;
            let vp = unsafe { builder.build_gep(i8_type, cb, &[vo], "vp").or_llvm_err()? };
            builder.build_store(vp, value).or_llvm_err()?;
            let nc = builder
                .build_int_add(cnt, i64_type.const_int(1, false), "nc")
                .or_llvm_err()?;
            builder.build_store(cnt_p, nc).or_llvm_err()?;
            builder.build_return(None).or_llvm_err()?;
        }
        let end_fn = self.get_or_declare_fn(
            module,
            "verum_ctx_end",
            void_type.fn_type(&[i64_type.into()], false),
        );
        if end_fn.count_basic_blocks() == 0 {
            ...
            let nc = builder
                ...
                .or_llvm_err()?;
            let fc = builder
                .build_select(has, nc, i64_type.const_zero(), "fc")
                .or_llvm_err()?;
            builder.build_store(cnt_p, fc).or_llvm_err()?;
            builder.build_return(None).or_llvm_err()?;
        }
        Ok(())
    }
    ...
    fn emit_tls_and_context_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let pthread_key_create_fn = self.get_or_declare_fn(
            module,
            "pthread_key_create",
            i64_type.fn_type(&[ptr_type.into(), ptr_type.into()], false),
        );
        ...
        let pthread_getspecific_fn = self.get_or_declare_fn(
            module,
            "pthread_getspecific",
            i64_type.fn_type(&[i64_type.into()], false),
        );
        let pthread_setspecific_fn = self.get_or_declare_fn(
            module,
            "pthread_setspecific",
            i64_type.fn_type(&[i64_type.into(), ptr_type.into()], false),
        );
        ...
        // 1. tls_init()
        {
            ...
            if func.count_basic_blocks() > 0 { /* skip */
                ...
                builder.position_at_end(entry);
                let init_g = module
                    .get_global("__verum_tls_initialized")
                    .or_internal("missing __verum_tls_initialized global")?;
                let inited = builder
                    .build_load(i64_type, init_g.as_pointer_value(), "inited")
                    .or_llvm_err()?
                    .into_int_value();
                let is_init = builder
                    .build_int_compare(IntPredicate::NE, inited, i64_type.const_zero(), "ii")
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(is_init, done, do_init)
                    .or_llvm_err()?;
                builder.position_at_end(do_init);
                let key_g = module
                    .get_global("__verum_tls_key")
                    .or_internal("missing __verum_tls_key global")?;
                builder
                    .build_call(
                        pthread_key_create_fn,
                        &[
                            key_g.as_pointer_value().into(),
                            ptr_type.const_null().into(),
                        ],
                        "",
                    )
                    .or_llvm_err()?;
                builder
                    .build_store(init_g.as_pointer_value(), i64_type.const_int(1, false))
                    .or_llvm_err()?;
                builder.build_unconditional_branch(done).or_llvm_err()?;
                builder.position_at_end(done);
                builder.build_return(None).or_llvm_err()?;
        ...
        // 2. get_or_create_context() -> ptr
        {
            ...
            if func.count_basic_blocks() > 0 { /* skip */
                ...
                let key_g = module
                    ...
                    .or_internal("missing __verum_tls_key global")?;
                let key_i64 = builder
                    .build_load(i64_type, key_g.as_pointer_value(), "key_i64")
                    .or_llvm_err()?
                    .into_int_value();
                ...
                let key: verum_llvm::values::BasicValueEnum = if getspec_param_type.is_int_type()
                    && getspec_param_type.into_int_type().get_bit_width() == 32
                {
                    builder
                        .build_int_truncate(key_i64, ctx.i32_type(), "key32")
                        .or_llvm_err()?
                        .into()
                } else {
                    key_i64.into()
            ...
            .basic_value_or("expected basic value")?;
                ...
                let existing = match existing_val {
                    ...
                    verum_llvm::values::BasicValueEnum::IntValue(i) => builder
                        .build_int_to_ptr(i, ptr_type, "ctx_ptr")
                        .or_llvm_err()?,
                    _ => ptr_type.const_null(),
                };
                let is_null = builder.build_is_null(existing, "is_null").or_llvm_err()?;
                builder
                    .build_conditional_branch(is_null, create_ctx, have_ctx)
                    .or_llvm_err()?;
                ...
                let new_ctx = builder
                    .build_call(
                        alloc_fn,
                        &[i64_type.const_int(Self::EC_SIZE, false).into()],
                        "new_ctx",
                    )
                    .or_llvm_err()?
            .basic_value_or("expected basic value")?
                ...
                let tier_ptr = unsafe {
                    builder
                        .build_gep(
                            i8_type,
                            new_ctx,
                            &[i64_type.const_int(Self::EC_EXECUTION_TIER, false)],
                            "tier_p",
                        )
                        .or_llvm_err()?
                };
                builder
                    .build_store(tier_ptr, i8_type.const_int(3, false))
                    .or_llvm_err()?;
                // Store via pthread_setspecific — adapt key to match VBC-declared param type
                let key2_i64 = builder
                    .build_load(i64_type, key_g.as_pointer_value(), "key2_i64")
                    .or_llvm_err()?
                    .into_int_value();
                let setspec_param_type = pthread_setspecific_fn.get_type().get_param_types()[0];
                let key2: verum_llvm::values::BasicValueEnum = if setspec_param_type.is_int_type()
                    && setspec_param_type.into_int_type().get_bit_width() == 32
                {
                    builder
                        .build_int_truncate(key2_i64, ctx.i32_type(), "key2_32")
                        .or_llvm_err()?
                        .into()
                } else {
                    key2_i64.into()
                ...
                builder
                    .build_call(pthread_setspecific_fn, &[key2.into(), new_ctx.into()], "")
                    .or_llvm_err()?;
        ...
        // 3. verum_tls_get(slot: i64) -> i64 (override with EC-based impl)
        {
            ...
            let base_offset = builder
                .build_int_add(
                    slot_offset,
                    i64_type.const_int(Self::EC_TLS_SLOTS, false),
                    "bo",
                )
                .or_llvm_err()?;
            // SAFETY: GEP into a struct or object at a fixed slot offset; the object was allocated with the expected layout
            let tls_ptr = unsafe {
                builder
                    .build_gep(i8_type, ec, &[base_offset], "tls_p")
                    .or_llvm_err()?
            };
    ...
    fn emit_stack_frame_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        // 5. verum_push_stack_frame(name: ptr, file: ptr, line: i32, col: i32)
        {
            ...
            let depth_ptr = unsafe {
                builder
                    .build_gep(
                        i8_type,
                        ec,
                        &[i64_type.const_int(Self::EC_STACK_DEPTH, false)],
                        "dp",
                    )
                    .or_llvm_err()?
            };
            let depth = builder
                .build_load(i64_type, depth_ptr, "depth")
                .or_llvm_err()?
                .into_int_value();
            let max_depth = i64_type.const_int(256, false);
            let in_bounds = builder
                .build_int_compare(IntPredicate::ULT, depth, max_depth, "ib")
                .or_llvm_err()?;
            builder
                .build_conditional_branch(in_bounds, do_push, done)
                .or_llvm_err()?;
            builder.position_at_end(do_push);
            // frame_offset = EC_STACK_FRAMES + depth * 24
            let frame_off = builder
                .build_int_mul(
                    depth,
                    i64_type.const_int(Self::EC_STACK_FRAME_SIZE, false),
                    "fo",
                )
                .or_llvm_err()?;
            let frame_base = builder
                .build_int_add(
                    frame_off,
                    i64_type.const_int(Self::EC_STACK_FRAMES, false),
                    "fb",
                )
                .or_llvm_err()?;
            ...
            let frame_ptr = unsafe {
                builder
                    .build_gep(i8_type, ec, &[frame_base], "fp")
                    .or_llvm_err()?
            };
            ...
            let fp8 = unsafe {
                builder
                    .build_gep(i8_type, frame_ptr, &[i64_type.const_int(8, false)], "fp8")
                    .or_llvm_err()?
            };
            ...
            let fp16 = unsafe {
                builder
                    .build_gep(i8_type, frame_ptr, &[i64_type.const_int(16, false)], "fp16")
                    .or_llvm_err()?
            };
            ...
            let fp20 = unsafe {
                builder
                    .build_gep(i8_type, frame_ptr, &[i64_type.const_int(20, false)], "fp20")
                    .or_llvm_err()?
            };
            builder.build_store(fp20, col).or_llvm_err()?;
            // Increment depth
            let new_depth = builder
                .build_int_add(depth, i64_type.const_int(1, false), "nd")
                .or_llvm_err()?;
        ...
        // 6. verum_pop_stack_frame()
        {
            ...
            let depth = builder
                ...
                .into_int_value();
            let is_pos = builder
                .build_int_compare(IntPredicate::SGT, depth, i64_type.const_zero(), "ip")
                .or_llvm_err()?;
            let new_depth = builder
                .build_int_sub(depth, i64_type.const_int(1, false), "nd")
                .or_llvm_err()?;
            let final_depth = builder
                .build_select(is_pos, new_depth, i64_type.const_zero(), "fd")
                .or_llvm_err()?;
    ...
    fn emit_panic_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        builder
            .build_call(
                ...
                &[
                    ...
                    i64_type.const_int(7, false).into(),
                ],
                "",
            )
            .or_llvm_err()?;
        // Write message
        let has_msg = builder
            .build_int_compare(IntPredicate::SGT, len, i64_type.const_zero(), "hm")
            .or_llvm_err()?;
        let write_len = builder
            .build_select(has_msg, len, i64_type.const_zero(), "wl")
            .or_llvm_err()?;
        builder
            .build_call(
                write_fn,
                &[stderr_fd.into(), msg.into(), write_len.into()],
                "",
            )
            .or_llvm_err()?;
        // Write newline
        ...
        // Returns pointer to jmp_buf for setjmp. Entry: { jmp_buf(192), value(i64), active(i32), defer_depth(i64) } = 208 bytes
        {
            ...
            .basic_value_or("expected basic value")?
                ...
                let cnt_ptr = unsafe {
                    builder
                        .build_gep(
                            i8_type,
                            ec,
                            &[i64_type.const_int(Self::EC_EXCEPTION_HANDLER_COUNT, false)],
                            "cnt_p",
                        )
                        .or_llvm_err()?
                };
                let cnt = builder
                    .build_load(i64_type, cnt_ptr, "cnt")
                    .or_llvm_err()?
                    .into_int_value();
                // Compute entry ptr = ec + EC_EXCEPTION_HANDLERS + cnt * 208
                let entry_off = builder
                    .build_int_mul(
                        cnt,
                        i64_type.const_int(Self::EC_EXCEPTION_ENTRY_SIZE, false),
                        "eo",
                    )
                    .or_llvm_err()?;
                let base_off = builder
                    .build_int_add(
                        entry_off,
                        i64_type.const_int(Self::EC_EXCEPTION_HANDLERS, false),
                        "bo",
                    )
                    .or_llvm_err()?;
                ...
                let entry_ptr = unsafe {
                    builder
                        .build_gep(i8_type, ec, &[base_off], "ep")
                        .or_llvm_err()?
                };
                ...
                let defer_cnt_ptr = unsafe {
                    builder
                        .build_gep(
                            i8_type,
                            ec,
                            &[i64_type.const_int(Self::EC_DEFER_COUNT, false)],
                            "dcp",
                        )
                        .or_llvm_err()?
                };
                let defer_depth = builder
                    .build_load(i64_type, defer_cnt_ptr, "dd")
                    .or_llvm_err()?;
                let dd_off = builder
                    .build_int_add(base_off, i64_type.const_int(200, false), "ddo")
                    .or_llvm_err()?;
                // SAFETY: GEP into exception handler entry at offset 200 (defer_depth field) within the entry selected by cnt
                let dd_ptr = unsafe {
                    builder
                        .build_gep(i8_type, ec, &[dd_off], "ddp")
                        .or_llvm_err()?
                };
        ...
        // 9. verum_exception_pop()
        {
            ...
            .basic_value_or("expected basic value")?
                ...
                let cnt = builder
                    ...
                    .into_int_value();
                let is_pos = builder
                    .build_int_compare(IntPredicate::SGT, cnt, i64_type.const_zero(), "ip")
                    .or_llvm_err()?;
                let new_cnt = builder
                    .build_int_sub(cnt, i64_type.const_int(1, false), "nc")
                    .or_llvm_err()?;
                let final_cnt = builder
                    .build_select(is_pos, new_cnt, i64_type.const_zero(), "fc")
                    .or_llvm_err()?;
        ...
        // 10. verum_exception_throw(value: i64) — simplified: store value, longjmp
        {
            ...
            .basic_value_or("expected basic value")?
                ...
                let cnt = builder
                    ...
                    .or_llvm_err()?
                    .into_int_value();
                let has_handler = builder
                    .build_int_compare(IntPredicate::SGT, cnt, i64_type.const_zero(), "hh")
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(has_handler, do_throw, no_handler)
                    .or_llvm_err()?;
                ...
                builder
                    .build_call(defer_run_fn, &[saved_depth.into()], "")
                    .or_llvm_err()?;
                // Store exception value at entry offset 192
                let val_off = builder
                    .build_int_add(base_off, i64_type.const_int(192, false), "vo")
                    .or_llvm_err()?;
                // SAFETY: GEP into exception handler entry to store the exception value at offset 192
                let val_ptr = unsafe {
                    builder
                        .build_gep(i8_type, ec, &[val_off], "vp")
                        .or_llvm_err()?
                };
        ...
        // 11. verum_exception_get() -> i64
        {
            ...
            .basic_value_or("expected basic value")?
                ...
                let val_off = builder
                    .build_int_add(
                        entry_off,
                        i64_type.const_int(Self::EC_EXCEPTION_HANDLERS + 192, false),
                        "vo",
                    )
                    .or_llvm_err()?;
                // SAFETY: GEP at a known offset within an allocated object; the pointer is valid and the offset does not exceed the allocation size
        ...
        // 14. verum_defer_run_to(target: i64)
        {
            ...
            .basic_value_or("expected basic value")?
                ...
                let cnt = builder
                    ...
                    .into_int_value();
                let above = builder
                    .build_int_compare(IntPredicate::UGT, cnt, target, "above")
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(above, loop_body, done)
                    .or_llvm_err()?;
                ...
                let cleanup_fn_ptr = builder
                    .build_load(ptr_type, fn_p, "cfn")
                    .or_llvm_err()?
                    .into_pointer_value();
                let arg_off = builder
                    .build_int_add(fn_off, i64_type.const_int(8, false), "ao")
                ...
                builder
                    .build_indirect_call(cleanup_ty, cleanup_fn_ptr, &[arg_val.into()], "")
                    .or_llvm_err()?;
                builder
                    .build_unconditional_branch(loop_check)
                    .or_llvm_err()?;
    ...
    fn emit_context_provide_pop_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        // 16. verum_context_provide(type_id: i64, value: ptr, destructor: ptr)
        {
            ...
            .basic_value_or("expected basic value")?
                ...
                let cnt_ptr = unsafe {
                    builder
                        .build_gep(
                            i8_type,
                            ec,
                            &[i64_type.const_int(Self::EC_CONTEXT_COUNT, false)],
                            "cnt_p",
                        )
                        .or_llvm_err()?
                };
                ...
                let entry_off = builder
                    .build_int_mul(
                        cnt,
                        i64_type.const_int(Self::EC_CONTEXT_ENTRY_SIZE, false),
                        "eo",
                    )
                    .or_llvm_err()?;
                let base_off = builder
                    .build_int_add(
                        entry_off,
                        i64_type.const_int(Self::EC_CONTEXTS, false),
                        "bo",
                    )
                    .or_llvm_err()?;
                ...
                let tid_ptr = unsafe {
                    builder
                        .build_gep(i8_type, ec, &[base_off], "tp")
                        .or_llvm_err()?
                };
                builder.build_store(tid_ptr, type_id).or_llvm_err()?;
                // Store value at offset 8
                let val_off = builder
                    .build_int_add(base_off, i64_type.const_int(8, false), "vo")
                    .or_llvm_err()?;
                // SAFETY: GEP into the context binding entry at offset 8 to store the value
                let val_ptr = unsafe {
                    builder
                ...
                let dtor_off = builder
                    .build_int_add(base_off, i64_type.const_int(16, false), "do")
                    .or_llvm_err()?;
                // SAFETY: GEP into the context binding entry at offset 16 to store the destructor pointer
                let dtor_ptr = unsafe {
                    builder
                        .build_gep(i8_type, ec, &[dtor_off], "dp")
                        .or_llvm_err()?
                };
        ...
        // 17. verum_context_pop(type_id: i64)
        {
            ...
            .basic_value_or("expected basic value")?
                ...
                let tid_ptr = unsafe {
                    builder
                        .build_gep(i8_type, ec, &[tid_off], "tp")
                        .or_llvm_err()?
                };
                let stored_tid = builder
                    .build_load(i64_type, tid_ptr, "st")
                    .or_llvm_err()?
                    .into_int_value();
                let matches = builder
                    .build_int_compare(IntPredicate::EQ, stored_tid, type_id, "m")
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(matches, found, check_dtor)
                    .or_llvm_err()?;

                builder.position_at_end(check_dtor);
                let prev_idx = builder
                    .build_int_sub(idx, i64_type.const_int(1, false), "pi")
                    .or_llvm_err()?;
                let still_valid = builder
                    .build_int_compare(IntPredicate::SGE, prev_idx, i64_type.const_zero(), "sv")
                    .or_llvm_err()?;
                idx_phi.add_incoming(&[(&prev_idx, check_dtor)]);
                builder
                    .build_conditional_branch(still_valid, search_loop, not_found)
                    .or_llvm_err()?;

                builder.position_at_end(found);
                // Load destructor at offset 16
                let dtor_off = builder
                    .build_int_add(tid_off, i64_type.const_int(16, false), "do_off")
                    .or_llvm_err()?;
                ...
                let dtor = builder
                    .build_load(ptr_type, dtor_ptr, "dtor")
                    .or_llvm_err()?
                    .into_pointer_value();
                let dtor_null = builder.build_is_null(dtor, "dn").or_llvm_err()?;
                // Load value at offset 8
                let val_off = builder
                    .build_int_add(tid_off, i64_type.const_int(8, false), "val_off")
                    .or_llvm_err()?;
                ...
                builder
                    .build_conditional_branch(dtor_null, shift_loop, call_dtor)
                    .or_llvm_err()?;

                builder.position_at_end(call_dtor);
                let dtor_ty = void_type.fn_type(&[ptr_type.into()], false);
                builder
                    .build_indirect_call(dtor_ty, dtor, &[val.into()], "")
                    .or_llvm_err()?;
                builder
                ...
                let last_idx = builder
                    .build_int_sub(cnt, i64_type.const_int(1, false), "li")
                    .or_llvm_err()?;
                let need_shift = builder
                    .build_int_compare(IntPredicate::ULT, idx, last_idx, "ns")
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(need_shift, shift_body, done)
                    .or_llvm_err()?;
    ...
    fn emit_create_args_list_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let arr_sz = builder
            .build_int_mul(argc, i64_type.const_int(8, false), "asz")
            .or_llvm_err()?;
        let arr_ptr = builder
        ...
        let ptr_field = unsafe {
            builder
                .build_gep(i8_type, list_ptr, &[i64_type.const_int(24, false)], "pf")
                .or_llvm_err()?
        };
        let arr_as_i64 = builder
            .build_ptr_to_int(arr_ptr, i64_type, "ai")
            .or_llvm_err()?;
        ...
        let len_field = unsafe {
            builder
                .build_gep(i8_type, list_ptr, &[i64_type.const_int(32, false)], "lf")
                .or_llvm_err()?
        };
        ...
        let cap_field = unsafe {
            builder
                .build_gep(i8_type, list_ptr, &[i64_type.const_int(40, false)], "cf")
                .or_llvm_err()?
        };
        ...
        let i_val = i_phi.as_basic_value().into_int_value();
        let cond = builder
            .build_int_compare(IntPredicate::ULT, i_val, argc, "cond")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(cond, loop_body, done)
            .or_llvm_err()?;

        builder.position_at_end(loop_body);
        // Load argv[i]
        // SAFETY: GEP into the argv pointer array at index i; i < argc is checked by the loop condition
        let argv_elem = unsafe {
            builder
                .build_gep(ptr_type, argv, &[i_val], "ae")
                .or_llvm_err()?
        };
        let cstr = builder
            .build_load(ptr_type, argv_elem, "cs")
            .or_llvm_err()?;
        let text = builder
        ...
        let arr_elem = unsafe {
            builder
                .build_gep(i64_type, arr_ptr, &[i_val], "arri")
                .or_llvm_err()?
        };
        ...
        builder.position_at_end(done);
        let result = builder
            .build_ptr_to_int(list_ptr, i64_type, "result")
            .or_llvm_err()?;
        builder.build_return(Some(&result)).or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_i64_to_str_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let is_zero = builder
            .build_int_compare(IntPredicate::EQ, value, i64_type.const_zero(), "iz")
            .or_llvm_err()?;
        let zero_block = ctx.append_basic_block(func, "zero");
        let nonzero = ctx.append_basic_block(func, "nonzero");
        builder
            .build_conditional_branch(is_zero, zero_block, nonzero)
            .or_llvm_err()?;

        builder.position_at_end(zero_block);
        builder
            .build_store(buf, i8_type.const_int(b'0' as u64, false))
            .or_llvm_err()?;
        // SAFETY: GEP into the caller-provided buffer at offset 1 to write the null terminator after '0'
        let buf1 = unsafe {
            builder
                .build_gep(i8_type, buf, &[i64_type.const_int(1, false)], "b1")
                .or_llvm_err()?
        };
        builder
            .build_store(buf1, i8_type.const_zero())
            .or_llvm_err()?;
        builder
            .build_return(Some(&i64_type.const_int(1, false)))
            .or_llvm_err()?;
        ...
        let neg = builder
            .build_int_compare(IntPredicate::SLT, value, i64_type.const_zero(), "neg")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(neg, is_neg, digit_loop)
            .or_llvm_err()?;
        ...
        builder
            .build_store(pos_alloca, i64_type.const_zero())
            .or_llvm_err()?;
        ...
        builder
            .build_store(buf, i8_type.const_int(b'-' as u64, false))
            .or_llvm_err()?;
        builder
            .build_store(pos_alloca, i64_type.const_int(1, false))
            .or_llvm_err()?;
        let neg_val = builder
            .build_int_sub(i64_type.const_zero(), value, "nv")
            .or_llvm_err()?;
        ...
        let _pos = builder
            ...
            .or_llvm_err()?
            .into_int_value();
        let digit_done = builder
            .build_int_compare(IntPredicate::EQ, abs_v, i64_type.const_zero(), "dd")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(digit_done, reverse_loop, {
        ...
        let abs_v2 = builder
            .build_load(i64_type, abs_alloca, "av2")
            .or_llvm_err()?
            .into_int_value();
        let pos2 = builder
            .build_load(i64_type, pos_alloca, "p2")
            .or_llvm_err()?
            .into_int_value();
        let digit = builder
            .build_int_unsigned_rem(abs_v2, i64_type.const_int(10, false), "d")
            .or_llvm_err()?;
        let ch = builder
            .build_int_add(digit, i64_type.const_int(b'0' as u64, false), "ch")
            .or_llvm_err()?;
        let ch8 = builder
            .build_int_truncate(ch, i8_type, "ch8")
            .or_llvm_err()?;
        // SAFETY: GEP into the caller-provided buffer at buf[pos] to store a digit character; pos < bufsize
        let bp = unsafe {
            builder
                .build_gep(i8_type, buf, &[pos2], "bp")
                .or_llvm_err()?
        };
        builder.build_store(bp, ch8).or_llvm_err()?;
        let new_abs = builder
            .build_int_unsigned_div(abs_v2, i64_type.const_int(10, false), "na")
            .or_llvm_err()?;
        builder.build_store(abs_alloca, new_abs).or_llvm_err()?;
        let new_pos = builder
            .build_int_add(pos2, i64_type.const_int(1, false), "np")
            .or_llvm_err()?;
        ...
        let final_pos = builder
            .build_load(i64_type, pos_alloca, "fp")
            .or_llvm_err()?
            .into_int_value();
        // Null-terminate
        ...
        let null_p = unsafe {
            builder
                .build_gep(i8_type, buf, &[final_pos], "np")
                .or_llvm_err()?
        };
        builder
            .build_store(null_p, i8_type.const_zero())
            .or_llvm_err()?;
        ...
        let start = builder
            ...
            .into_int_value();
        let end = builder
            .build_int_sub(final_pos, i64_type.const_int(1, false), "end")
            .or_llvm_err()?;
        ...
        builder
            .build_unconditional_branch(rev_check)
            .or_llvm_err()?;

        builder.position_at_end(rev_check);
        let l = builder
            .build_load(i64_type, left_a, "l")
            .or_llvm_err()?
            .into_int_value();
        let r = builder
            .build_load(i64_type, right_a, "r")
            .or_llvm_err()?
            .into_int_value();
        let cont = builder
            .build_int_compare(IntPredicate::ULT, l, r, "cont")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(cont, rev_body, ret_block)
            .or_llvm_err()?;

        builder.position_at_end(rev_body);
        let l2 = builder
            .build_load(i64_type, left_a, "l2")
            .or_llvm_err()?
            .into_int_value();
        let r2 = builder
            .build_load(i64_type, right_a, "r2")
            .or_llvm_err()?
            .into_int_value();
        ...
        let nl = builder
            .build_int_add(l2, i64_type.const_int(1, false), "nl")
            .or_llvm_err()?;
        let nr = builder
            .build_int_sub(r2, i64_type.const_int(1, false), "nr")
            .or_llvm_err()?;
        ...
        let final_len = builder
            .build_load(i64_type, pos_alloca, "fl")
            .or_llvm_err()?;
    ...
    fn emit_write_helpers_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        // 21. write_stdout(s: ptr, len: i64)
        {
            ...
            if func.count_basic_blocks() > 0 { /* skip */
                ...
                builder
                    .build_call(
                        write_fn,
                        &[i64_type.const_int(1, false).into(), s.into(), len.into()],
                        "",
                    )
                    .or_llvm_err()?;
                builder.build_return(None).or_llvm_err()?;
        ...
        // 22. write_stderr(s: ptr, len: i64)
        {
            ...
            if func.count_basic_blocks() > 0 { /* skip */
                ...
                builder
                    .build_call(
                        write_fn,
                        &[i64_type.const_int(2, false).into(), s.into(), len.into()],
                        "",
                    )
                    .or_llvm_err()?;
                builder.build_return(None).or_llvm_err()?;
        ...
        // 23. write_stderr_str(s: ptr) — call strlen then write_stderr
        {
            ...
            .basic_value_or("expected basic value")?;
                ...
                builder
                    .build_call(write_stderr_fn, &[s.into(), len.into()], "")
                    .or_llvm_err()?;
                builder.build_return(None).or_llvm_err()?;
            }
        }
        ...
        // 24. write_stderr_int(value: i64) — call verum_i64_to_str then write_stderr
        {
            ...
            if func.count_basic_blocks() > 0 { /* skip */
                ...
                let i64_to_str_fn = self.get_or_declare_fn(
                    module,
                    "verum_i64_to_str",
                    i64_type.fn_type(&[i64_type.into(), ptr_type.into(), i64_type.into()], false),
                );
                ...
                let buf = builder
                    .build_alloca(i8_type.array_type(32), "buf")
                    .or_llvm_err()?;
                let len = builder
                    .build_call(
                        i64_to_str_fn,
            ...
            .basic_value_or("expected basic value")?;
                let write_stderr_fn = module
                    ...
                    .or_missing_fn("write_stderr")?;
                builder
                    .build_call(write_stderr_fn, &[buf.into(), len.into()], "")
                    .or_llvm_err()?;
                builder.build_return(None).or_llvm_err()?;
            }
        }
    ...
    fn emit_gen_sync_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        // 26. gen_mtx_lock(m: ptr) — CAS loop 0->1
        {
            ...
            if func.count_basic_blocks() > 0 { /* skip */
                ...
                let old = builder
                    ...
                    .or_llvm_err()?;
                let was_free = builder
                    .build_int_compare(IntPredicate::EQ, old, i32_type.const_zero(), "ok")
                    .or_llvm_err()?;
                builder
                    .build_conditional_branch(was_free, locked, spin)
                    .or_llvm_err()?;
                builder.position_at_end(locked);
                builder.build_return(None).or_llvm_err()?;
            }
        ...
        // 29. gen_cv_wait(cv: ptr, m: ptr) — load seq, unlock, futex_wait, lock
        {
            ...
            if func.count_basic_blocks() > 0 { /* skip */
                ...
                let unlock_fn = self.get_or_declare_fn(
                    module,
                    "gen_mtx_unlock",
                    void_type.fn_type(&[ptr_type.into()], false),
                );
                let lock_fn = self.get_or_declare_fn(
                    module,
                    "gen_mtx_lock",
                    void_type.fn_type(&[ptr_type.into()], false),
                );
                let futex_wait_fn = self.get_or_declare_fn(
                    module,
                    "verum_futex_wait",
                    i64_type.fn_type(&[i64_type.into(), i64_type.into(), i64_type.into()], false),
                );
                ...
                let m = func
                    ...
                    .into_pointer_value();
                let seq = builder
                    .build_load(i32_type, cv, "seq")
                    .or_llvm_err()?
                    .into_int_value();
                let seq64 = builder
                    .build_int_z_extend(seq, i64_type, "seq64")
                    .or_llvm_err()?;
                builder
                    .build_call(unlock_fn, &[m.into()], "")
                    .or_llvm_err()?;
                let cv_i64 = builder.build_ptr_to_int(cv, i64_type, "ci").or_llvm_err()?;
                builder
                    .build_call(
                        futex_wait_fn,
                        &[cv_i64.into(), seq64.into(), i64_type.const_zero().into()],
                        "",
                    )
                    .or_llvm_err()?;
    ...
    fn emit_gen_thread_entry_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let gen_mtx_lock_fn = self.get_or_declare_fn(
            module,
            "gen_mtx_lock",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        let gen_mtx_unlock_fn = self.get_or_declare_fn(
            module,
            "gen_mtx_unlock",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        let gen_cv_wait_fn = self.get_or_declare_fn(
            module,
            "gen_cv_wait",
            void_type.fn_type(&[ptr_type.into(), ptr_type.into()], false),
        );
        let gen_cv_signal_fn = self.get_or_declare_fn(
            module,
            "gen_cv_signal",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        ...
        if let Some(cg) = module.get_global("__verum_current_generator") {
            builder
                .build_store(cg.as_pointer_value(), gen_ptr)
                .or_llvm_err()?;
        }
        ...
        // SAFETY: GEP at a known offset within an allocated object; the pointer is valid and the offset does not exceed the allocation size
        let cv_gen_ptr = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(48, false)], "cvg")
                .or_llvm_err()?
        };
        // SAFETY: GEP at a fixed offset within a known struct layout; the pointer is valid from prior allocation
        let cv_caller_ptr = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(44, false)], "cvc")
                .or_llvm_err()?
        };
        ...
        let status_ptr = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(32, false)], "sp")
                .or_llvm_err()?
        };

        // Lock mutex, wait until status != GEN_STATUS_CREATED (0)
        builder
            .build_call(gen_mtx_lock_fn, &[mtx_ptr.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(wait_check)
            .or_llvm_err()?;

        builder.position_at_end(wait_check);
        let status = builder
            .build_load(i64_type, status_ptr, "s")
            .or_llvm_err()?
            .into_int_value();
        let is_created = builder
            .build_int_compare(IntPredicate::EQ, status, i64_type.const_zero(), "ic")
            .or_llvm_err()?;
        builder
            .build_conditional_branch(is_created, wait_loop, call_body)
            .or_llvm_err()?;

        builder.position_at_end(wait_loop);
        builder
            .build_call(gen_cv_wait_fn, &[cv_gen_ptr.into(), mtx_ptr.into()], "")
            .or_llvm_err()?;
        builder
            .build_unconditional_branch(wait_check)
            .or_llvm_err()?;
        ...
        builder
            .build_call(gen_mtx_unlock_fn, &[mtx_ptr.into()], "")
            .or_llvm_err()?;
        // Load func_ptr and num_args, call with args
        let fn_val = builder
            .build_load(i64_type, gen_ptr, "fn")
            .or_llvm_err()?
            .into_int_value();
        let fn_as_ptr = builder
            .build_int_to_ptr(fn_val, ptr_type, "fnp")
            .or_llvm_err()?;
        // SAFETY: GEP into the 64-byte generator struct at offset 8 (num_args field)
        let nargs_ptr = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(8, false)], "nap")
                .or_llvm_err()?
        };
        let num_args = builder
            .build_load(i64_type, nargs_ptr, "na")
            .or_llvm_err()?
            .into_int_value();
        // SAFETY: GEP into the 64-byte generator struct at offset 16 (args_ptr field)
        let args_i64_ptr = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(16, false)], "argsp")
                .or_llvm_err()?
        };
        let args_val = builder
            .build_load(i64_type, args_i64_ptr, "argsv")
            .or_llvm_err()?
            .into_int_value();
        let args_ptr = builder
        ...
        builder
            ...
            .or_llvm_err()?;
        builder
            .build_store(status_ptr, i64_type.const_int(3, false))
            .or_llvm_err()?;
        // SAFETY: GEP into the 64-byte generator struct at offset 24 (yielded_value) to clear it on completion
        let yv_ptr = unsafe {
            builder
                .build_gep(i8_type, gen_ptr, &[i64_type.const_int(24, false)], "yvp")
                .or_llvm_err()?
        };
        builder
            .build_store(yv_ptr, i64_type.const_zero())
            .or_llvm_err()?;
        builder
            .build_call(gen_cv_signal_fn, &[cv_caller_ptr.into()], "")
            .or_llvm_err()?;
        builder
            .build_call(gen_mtx_unlock_fn, &[mtx_ptr.into()], "")
            .or_llvm_err()?;
        ...
        if let Some(cg) = module.get_global("__verum_current_generator") {
            builder
                .build_store(cg.as_pointer_value(), ptr_type.const_null())
                .or_llvm_err()?;
        }
        builder
            .build_return(Some(&ptr_type.const_null()))
    ...
    fn emit_gen_yield_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let gen_ptr = if let Some(cg) = module.get_global("__verum_current_generator") {
            builder
                .build_load(ptr_type, cg.as_pointer_value(), "gen")
                .or_llvm_err()?
                .into_pointer_value()
        } else {
            builder.build_return(None).or_llvm_err()?;
        ...
        builder
            .build_store(status_ptr, i64_type.const_int(2, false))
            .or_llvm_err()?; // GEN_STATUS_YIELDED=2
        builder
            .build_call(gen_cv_signal_fn, &[cv_caller_ptr.into()], "")
            .or_llvm_err()?;
        builder
    ...
    fn emit_thread_entry_darwin_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let func_ptr_field = unsafe {
            builder
                .build_gep(i8_type, arg, &[i64_type.const_int(24, false)], "fp")
                .or_llvm_err()?
        };
        let func_val = builder
            .build_load(i64_type, func_ptr_field, "fv")
            .or_llvm_err()?
            .into_int_value();
        let fn_as_ptr = builder
            .build_int_to_ptr(func_val, ptr_type, "fnp")
            .or_llvm_err()?;
        // SAFETY: GEP into the 40-byte thread struct at offset 32 (arg field)
        let arg_field = unsafe {
            builder
                .build_gep(i8_type, arg, &[i64_type.const_int(32, false)], "af")
                .or_llvm_err()?
        };
        let arg_val = builder
            .build_load(i64_type, arg_field, "av")
            .or_llvm_err()?;
        // Call func(arg) -> i64
        ...
        let result_field = unsafe {
            builder
                .build_gep(i8_type, arg, &[i64_type.const_int(8, false)], "rf")
                .or_llvm_err()?
        };
        ...
        let cond_broadcast_fn = self.get_or_declare_fn(
            module,
            "verum_cond_broadcast",
            void_type.fn_type(&[ptr_type.into()], false),
        );
        ...
        let cond_ptr = unsafe {
            builder
                .build_gep(i8_type, arg, &[i64_type.const_int(20, false)], "cp")
                .or_llvm_err()?
        };
        builder
            .build_call(cond_broadcast_fn, &[cond_ptr.into()], "")
            .or_llvm_err()?;
        builder
            .build_return(Some(&ptr_type.const_null()))
            .or_llvm_err()?;
    ...
    fn emit_spawn_trampoline_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let packed = func
            ...
            .or_internal("missing first param")?
            .into_int_value();
        let packed_ptr = builder
            .build_int_to_ptr(packed, ptr_type, "pp")
            .or_llvm_err()?;
        // Packed struct: [func_ptr: i64, arg_count: i64, args: i64...]
        let func_val = builder
            .build_load(i64_type, packed_ptr, "fv")
            .or_llvm_err()?
            .into_int_value();
        let fn_as_ptr = builder
            .build_int_to_ptr(func_val, ptr_type, "fnp")
        ...
        let cnt_ptr = unsafe {
            builder
                .build_gep(i8_type, packed_ptr, &[i64_type.const_int(8, false)], "cp")
                .or_llvm_err()?
        };
        let arg_count = builder
            .build_load(i64_type, cnt_ptr, "ac")
            .or_llvm_err()?
            .into_int_value();
        ...
        let a0 = builder
            .build_load(i64_type, args_base, "a0")
            .or_llvm_err()?;
        ...
        let ret_bb = ctx.append_basic_block(func, "ret");
        builder
            .build_conditional_branch(is1, call1_bb, call2plus_bb)
            .or_llvm_err()?;
        ...
        let a1_p = unsafe {
            builder
                .build_gep(i64_type, args_base, &[i64_type.const_int(1, false)], "a1p")
                .or_llvm_err()?
        };
        ...
        builder
            .build_conditional_branch(is2, call2_bb, call3plus_bb)
            .or_llvm_err()?;
        ...
        let a2_p = unsafe {
            builder
                .build_gep(i64_type, args_base, &[i64_type.const_int(2, false)], "a2p")
                .or_llvm_err()?
        };
        ...
        builder
            .build_conditional_branch(is3, call3_bb, call4plus_bb)
            .or_llvm_err()?;
        ...
        let a3_p = unsafe {
            builder
                .build_gep(i64_type, args_base, &[i64_type.const_int(3, false)], "a3p")
                .or_llvm_err()?
        };
        ...
        builder
            .build_conditional_branch(is4, call4_bb, call5plus_bb)
            .or_llvm_err()?;
        ...
        let a4_p = unsafe {
            builder
                .build_gep(i64_type, args_base, &[i64_type.const_int(4, false)], "a4p")
                .or_llvm_err()?
        };
        ...
        builder
            .build_conditional_branch(is5, call5_bb, call6_bb)
            .or_llvm_err()?;
        ...
        let a5_p = unsafe {
            builder
                .build_gep(i64_type, args_base, &[i64_type.const_int(5, false)], "a5p")
                .or_llvm_err()?
        };
        ...
        builder
            .build_return(Some(&result.as_basic_value()))
            .or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_thread_spawn_multi_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let packed_i64 = func
            ...
            .into_int_value();
        let packed_ptr = builder
            .build_int_to_ptr(packed_i64, ptr_type, "pack")
            .or_llvm_err()?;

        // Load func_ptr from offset 0
        let fn_ptr_val = builder
            .build_load(i64_type, packed_ptr, "fn_ptr")
            .or_llvm_err()?
            .into_int_value();
        let fn_ptr = builder
            .build_int_to_ptr(fn_ptr_val, ptr_type, "fp")
            .or_llvm_err()?;
        ...
        let count = builder
            .build_load(i64_type, cnt_p, "count")
            .or_llvm_err()?
            .into_int_value();
        ...
        builder
            .build_return(Some(&result_phi.as_basic_value().into_int_value()))
            .or_llvm_err()?;
        Ok(())
    }
    ...
    fn emit_cond_wait_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let seq64 = builder
            ...
            .or_llvm_err()?;
        builder
            .build_call(mutex_unlock_fn, &[m.into()], "")
            .or_llvm_err()?;
        let cv_i64 = builder.build_ptr_to_int(cv, i64_type, "ci").or_llvm_err()?;
        builder
            .build_call(
    ...
    fn emit_cond_timedwait_ir(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let ret = builder
            .build_call(
                futex_wait_fn,
                &[cv_i64.into(), seq64.into(), timeout_ns.into()],
                "ret",
            )
            .or_llvm_err()?
            .basic_value_or("expected basic value")?;
    ...
    pub fn emit_wasi_declarations(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let wasi_fns: Vec<(&str, verum_llvm::types::FunctionType<'ctx>)> = vec![
            ...
            (
                "clock_time_get",
                i32_type.fn_type(&[i32_type.into(), i64_type.into(), ptr_type.into()], false),
            ),
            ...
            (
                "proc_exit",
                ctx.void_type().fn_type(&[i32_type.into()], false),
            ),
        ...
        for (name, fn_type) in wasi_fns {
            if module.get_function(name).is_none() {
                let func =
                    module.add_function(name, fn_type, Some(verum_llvm::module::Linkage::External));
    ...
    pub fn emit_wasm_memory_grow(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let pages_needed = builder
            .build_int_add(size, i64_type.const_int(65535, false), "round")
            .or_llvm_err()?;
        let pages = builder
            .build_int_unsigned_div(pages_needed, page_size, "pages")
            .or_llvm_err()?;
        let pages_i32 = builder
            .build_int_truncate(pages, i32_type, "p32")
            .or_llvm_err()?;

        // Call LLVM's memory.grow intrinsic
        // wasm.memory.grow(i32 pages) -> i32 old_size_in_pages (-1 on failure)
        let grow_fn = self.get_or_declare_fn(
            module,
        ...
        let old_pages_i64 = builder
            .build_int_z_extend(old_pages, i64_type, "op64")
            .or_llvm_err()?;
        let base_addr = builder
            .build_int_mul(old_pages_i64, page_size, "base")
            .or_llvm_err()?;
        let base_ptr = builder
            .build_int_to_ptr(base_addr, ptr_type, "ptr")
            .or_llvm_err()?;
...
mod runtime_bridge_zero_overhead_tests {
    ...
    fn bridge_global_async_worker_threads_carries_manifest_value() {
        ...
        let pir = PlatformIR::new(&ctx).with_runtime_bridge(RuntimeBridgeValues {
    ...
    fn bridge_getters_re_emission_is_idempotent() {
        ...
        let func = module
            .get_function("verum_get_runtime_async_worker_threads")
            .unwrap();
