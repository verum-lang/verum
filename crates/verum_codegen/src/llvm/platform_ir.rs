...
impl<'ctx> PlatformIR<'ctx> {
    ...
    fn emit_linux_syscall(
        ...
        let result = builder
            ...
            .basic()
            .ok_or_else(|| {
                super::error::LlvmLoweringError::internal("syscall returned void".to_string())
            })?
            .into_int_value();
        Ok(result)
    }
    ...
    fn emit_file_read_all(&self, module: &Module<'ctx>) -> super::error::Result<()> {
        ...
        let fd_ok = builder
            ...
            .or_llvm_err()?;
        builder
            .build_conditional_branch(fd_ok, open_ok, ret_empty)
            .or_llvm_err()?;
        ...
        let text = builder
            .build_call(
                text_alloc_fn,
                &[rd_buf_val.into(), cur_len.into(), cur_len.into()],
                "text",
            )
            .or_llvm_err()?
            .try_as_basic_value()
