...
impl WellKnownTypeExt for WellKnownType {
    fn from_type_id(tid: TypeId) -> Option<WellKnownType> {
        if tid == TypeId::INT {
            return Some(WellKnownType::Int);
        }
        if tid == TypeId::FLOAT {
            return Some(WellKnownType::Float);
        }
        if tid == TypeId::BOOL {
            return Some(WellKnownType::Bool);
        }
        if tid == TypeId::TEXT {
            return Some(WellKnownType::Text);
        }
        if tid == TypeId::LIST {
            return Some(WellKnownType::List);
        }
        if tid == TypeId::MAP {
            return Some(WellKnownType::Map);
        }
        if tid == TypeId::SET {
            return Some(WellKnownType::Set);
        }
        if tid == TypeId::DEQUE {
            return Some(WellKnownType::Deque);
        }
        if tid == TypeId::MAYBE {
            return Some(WellKnownType::Maybe);
        }
        if tid == TypeId::RESULT {
            return Some(WellKnownType::Result);
        }
        if tid == TypeId::HEAP {
            return Some(WellKnownType::Heap);
        }
        if tid == TypeId::SHARED {
            return Some(WellKnownType::Shared);
        }
        if tid == TypeId::CHANNEL {
            return Some(WellKnownType::Channel);
        }
        None
    }
...
pub fn resolve_well_known_from_type_ref(
    ...
    match type_ref {
        verum_vbc::types::TypeRef::Concrete(tid) => {
            WellKnownType::from_type_id(*tid).or_else(|| {
                vbc_mod
                    .get_type_name(*tid)
                    .and_then(|n| WellKnownType::from_name(&n))
            })
        }
        verum_vbc::types::TypeRef::Instantiated { base, .. } => WellKnownType::from_type_id(*base)
            .or_else(|| {
                vbc_mod
                    .get_type_name(*base)
                    .and_then(|n| WellKnownType::from_name(&n))
