...
impl<'ctx> VbcToMlirGpuLowering<'ctx> {
    ...
    fn lower_instruction(
        ...
        match instr {
            // Phase 4: legacy `Instruction::TensorNew/Binop/Unop/Matmul/
            // Reduce` MLIR lowering arms deleted along with the variants
            // themselves.  All these ops now flow through
            // `Instruction::TensorExtended { sub_op, operands }` which
            // is the canonical post-Этап-Б GPU-pipeline gateway.  When
            // Этап C (compute unification) wires the MLIR `linalg.*` /
            // `vector` lowering for `TensorExtended`, the equivalent
            // body shapes (`tensor.empty` / `arith.addf` / `linalg.matmul`
            // / `linalg.reduce` etc.) get reinstated under that arm.
            // Until then `unhandled_gpu_extended` surfaces the gap.
