...
fn test_parse_generic_type_variant_access() {
    ...
    match &expr.kind {
        ExprKind::Field {
            ...
            assert!(
                matches!(&object.kind, ExprKind::TypeExpr(_)),
                "Expected TypeExpr for Maybe<TcpStream>, got {:?}",
                object.kind
            );
...
fn test_parse_generic_type_variant_access_nested() {
    ...
    match &expr.kind {
        ExprKind::Field {
            ...
            assert!(
                matches!(&object.kind, ExprKind::TypeExpr(_)),
                "Expected TypeExpr for Maybe<List<Int>>, got {:?}",
                object.kind
            );
            assert_eq!(field.as_str(), "None");
        }
...
fn test_parse_generic_type_variant_access_with_call() {
    ...
    match &expr.kind {
        ExprKind::MethodCall {
            ...
            assert!(
                matches!(&receiver.kind, ExprKind::TypeExpr(_)),
                "Expected TypeExpr for Maybe<Int>, got {:?}",
                receiver.kind
            );
...
fn test_parse_generic_type_variant_in_let_stmt() {
    ...
    let file_id = FileId::new(0);
    let parser = VerumParser::new();
    let result = parser.parse_module_str("fn test() { let x = Maybe<TcpStream>.None; }", file_id);
