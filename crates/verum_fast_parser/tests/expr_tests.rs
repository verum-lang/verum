...
#[test]
fn test_parse_generic_type_variant_access() {
    // Maybe<TcpStream>.None should parse as a field access on a generic TypeExpr
    let expr = parse_expr_test("Maybe<TcpStream>.None");
    match &expr.kind {
        ExprKind::Field { expr: object, field } => {
            assert!(matches!(&object.kind, ExprKind::TypeExpr(_)),
                "Expected TypeExpr for Maybe<TcpStream>, got {:?}", object.kind);
...
#[test]
fn test_parse_generic_type_variant_access_nested() {
    // Maybe<List<Int>>.None should parse with nested generics
    let expr = parse_expr_test("Maybe<List<Int>>.None");
    match &expr.kind {
        ExprKind::Field { expr: object, field } => {
            assert!(matches!(&object.kind, ExprKind::TypeExpr(_)),
                "Expected TypeExpr for Maybe<List<Int>>, got {:?}", object.kind);
            assert_eq!(field.as_str(), "None");
        }
        _ => panic!("Expected Field expression, got {:?}", expr.kind),
    }
...
#[test]
fn test_parse_generic_type_variant_access_with_call() {
    // Maybe<Int>.Some(42) should parse as a method call on a generic TypeExpr
    let expr = parse_expr_test("Maybe<Int>.Some(42)");
    match &expr.kind {
        ExprKind::MethodCall { receiver, method, args, .. } => {
            assert!(matches!(&receiver.kind, ExprKind::TypeExpr(_)),
                "Expected TypeExpr for Maybe<Int>, got {:?}", receiver.kind);
...
#[test]
fn test_parse_generic_type_variant_in_let_stmt() {
    // Full statement context: let x = Maybe<TcpStream>.None;
    let file_id = FileId::new(0);
    let parser = VerumParser::new();
    let result = parser.parse_module_str(
        "fn test() { let x = Maybe<TcpStream>.None; }",
        file_id,
    );
...
#[test]
fn test_parse_generic_type_variant_in_return() {
    // Return context: return Maybe<TcpStream>.None;
    let file_id = FileId::new(0);
    let parser = VerumParser::new();
    let result = parser.parse_module_str(
        "fn test() -> Maybe<TcpStream> { return Maybe<TcpStream>.None; }",
        file_id,
    );
    assert!(result.is_ok(), "Failed to parse: {:?}", result.err());
}
...
#[test]
fn test_parse_generic_type_variant_in_match_arm() {
    // Match arm context
    let file_id = FileId::new(0);
    let parser = VerumParser::new();
    let result = parser.parse_module_str(
        r#"fn test(x: Int) -> Maybe<Int> {
            match x {
                0 => Maybe<Int>.None,
                n => Maybe<Int>.Some(n),
            }
