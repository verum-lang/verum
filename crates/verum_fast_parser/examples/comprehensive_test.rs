...
fn test_file(path: &Path, expect_success: bool) -> Result<(), String> {
    ...
    if expect_success {
        ...
        match result {
            Ok(_) => Ok(()),
            Err(errors) => {
                let error_info: Vec<String> = errors
                    .iter()
                    .map(|e| {
            ...
            Err(errors) => {
                ...
                if let Some(expected) = &expected_error {
                    if error_codes.iter().any(|c| c == expected) {
                        ...
                        Err(format!(
                            "{}: expected {}, got {:?}",
                            file_name, expected, error_codes
                        ))
                    }
                } else {
        ...
        match test_file(file_path, expect_success) {
            ...
            Err(e) => {
                ...
                failures.push(format!(
                    "{}: {}",
                    rel_path.display(),
                    e.split(": ").skip(1).collect::<Vec<_>>().join(": ")
                ));
            }
        }
    }
...
fn main() {
    ...
    let (s_passed, s_failed, s_failures) = test_directory(
        concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../../vcs/specs/parser/success"
        ),
        true,
    );
    let s_total = s_passed + s_failed;
    let s_pct = if s_total > 0 {
        (s_passed as f64 / s_total as f64) * 100.0
    } else {
        0.0
    };
    println!(
        "Success tests: {}/{} passed ({:.1}%)",
        s_passed, s_total, s_pct
    );
    ...
    let f_pct = if f_total > 0 {
        (f_passed as f64 / f_total as f64) * 100.0
    } else {
        0.0
    };
    println!(
        "Fail tests: {}/{} passed ({:.1}%)",
        f_passed, f_total, f_pct
    );
