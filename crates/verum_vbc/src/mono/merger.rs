...
impl ModuleMerger {
    ...
    fn fixup_references(&mut self, output: &mut VbcModule) -> Result<(), MergeError> {
        ...
        //
        // **L1-WIDTH (task #44)**: a specialized id that needs a WIDER
        // varint than the generic id it replaces cannot be patched in
        // place (the fits-guard used to leave such sites on the generic
        // body — correct but unspecialized, capping mono coverage).
        // Functions with at least one wide site are rewritten
        // STRUCTURALLY: decode the whole body → convert jump offsets to
        // instruction-index form (`jump_offsets_to_instr_indices`, the
        // canonical inverse) → remap ids on the instruction values →
        // re-encode with `fixup_jump_offsets` recomputing byte offsets
        // against the new widths.  The blob is then rebuilt once with
        // updated descriptor offsets.  Overlapping/undecodable regions
        // demote to the in-place path for that function (no regression:
        // wide sites stay generic exactly as before).
        let mut rewritten: std::collections::HashMap<usize, Vec<u8>> =
            std::collections::HashMap::new();
        let trace_mono = std::env::var_os("VERUM_TRACE_MONO").is_some();
        for (fi, func) in output.functions.iter().enumerate() {
            let start = func.bytecode_offset as usize;
            let end = start + func.bytecode_length as usize;
            ...
            if self.function_has_wide_site(&output.bytecode, start, end, &id_remap, &site_route) {
                match self.rewrite_function_structurally(
                    &output.bytecode[start..end],
                    &id_remap,
                    &site_route,
                ) {
                    Some(bytes) => {
                        if trace_mono {
                            eprintln!(
                                "[mono-l1] fn@{} structurally re-encoded {}B -> {}B (wide spec id)",
                                start,
                                end - start,
                                bytes.len()
                            );
                        }
                        rewritten.insert(fi, bytes);
                        continue;
                    }
                    None => {
                        if trace_mono {
                            eprintln!(
                                "[mono-l1] fn@{} undecodable — wide sites stay generic",
                                start
                            );
                        }
                    }
        ...
        if !rewritten.is_empty() {
            self.rebuild_blob_with_rewrites(output, &rewritten)?;
        }
    ...
    /// L1-WIDTH (task #44): does this function contain at least one
    /// call-family site whose remapped id does NOT fit the existing
    /// varint width?  Decode-walk twin of `fixup_function_bytecode`'s
    /// target computation — same mapping authority, no writes.
    fn function_has_wide_site(
        &self,
        bytecode: &[u8],
        start: usize,
        end: usize,
        ...
        while pc < end {
            ...
            let mut probe = instr_start;
            let Ok(instr) = crate::bytecode::decode_instruction(bytecode, &mut probe) else {
                return false; // undecodable — in-place path handles/stops identically
            };
            if probe <= instr_start || probe > end {
                return false;
            }
            let target: Option<(u32, Option<u32>, usize)> = match &instr {
                I::Call { func_id, .. } | I::NewClosure { func_id, .. } => {
                    let dst_len = if bytecode[instr_start + 1] < 128 { 1 } else { 2 };
                    Some((*func_id, id_remap.get(func_id).copied(), instr_start + 1 + dst_len))
                }
                I::TailCall { func_id, .. } => {
                    Some((*func_id, id_remap.get(func_id).copied(), instr_start + 1))
                }
                I::CallG { func_id, type_args, .. } => {
                    let dst_len = if bytecode[instr_start + 1] < 128 { 1 } else { 2 };
                    let routed = site_route
                        .get(&(*func_id, type_args.clone()))
                        .copied()
            ...
            if let Some((old_id, Some(new_id), fid_pos)) = target
                && new_id != old_id
            {
                let (_, fid_len) = self.read_varint(bytecode, fid_pos);
                let fits = fid_len >= 10 || (new_id as u64) < (1u64 << (7 * fid_len as u32));
                if !fits {
                    return true;
                }
    ...
    /// L1-WIDTH (task #44): structural rewrite of one function body.
    /// Returns the re-encoded bytes, or `None` when the region does not
    /// decode cleanly end-to-end (caller falls back to the in-place
    /// path — wide sites stay generic, the pre-L1 behaviour).
    fn rewrite_function_structurally(
        &self,
        region: &[u8],
        id_remap: &HashMap<u32, u32>,
        site_route: &HashMap<(u32, Vec<TypeRef>), u32>,
        ...
        use crate::instruction::Instruction as I;
        let mut instrs = crate::bytecode::decode_instructions(region).ok()?;
        // byte-form → instruction-index form against ORIGINAL widths
        crate::bytecode::jump_offsets_to_instr_indices(&mut instrs);
        for instr in instrs.iter_mut() {
            match instr {
                I::Call { func_id, .. } | I::NewClosure { func_id, .. } | I::TailCall { func_id, .. } => {
                    if let Some(&new_id) = id_remap.get(func_id) {
                        *func_id = new_id;
                    }
                }
                I::CallG { func_id, type_args, .. } => {
        ...
        // index form → byte form against NEW widths, then encode.
        crate::bytecode::fixup_jump_offsets(&mut instrs);
        let mut out = Vec::with_capacity(region.len() + 16);
        for instr in instrs.iter() {
            crate::bytecode::encode_instruction(instr, &mut out);
        }
        Some(out)
    }

    /// L1-WIDTH (task #44): rebuild the module blob after structural
    /// rewrites changed function byte lengths.  Regions are laid back
    /// in ascending original-offset order; inter-region bytes are
    /// copied verbatim; every descriptor (functions AND specialization
    /// entries keyed by exact old region) gets its offset/length
    /// updated.  Overlapping function regions abort the rebuild
    /// (defensive: variants sharing bytecode) — the caller's rewrites
    /// are then discarded and behaviour is pre-L1.
    fn rebuild_blob_with_rewrites(
        &self,
        output: &mut crate::module::VbcModule,
        rewritten: &std::collections::HashMap<usize, Vec<u8>>,
    ) -> Result<(), MergeError> {
        let mut order: Vec<usize> = (0..output.functions.len())
            .filter(|i| {
                let f = &output.functions[*i];
                (f.bytecode_offset as usize + f.bytecode_length as usize)
                    <= output.bytecode.len()
                    && f.bytecode_length > 0
            })
            .collect();
        order.sort_by_key(|&i| output.functions[i].bytecode_offset);
        // Overlap check
        ...
        for &i in &order {
            ...
            if start < prev_end {
                if std::env::var_os("VERUM_TRACE_MONO").is_some() {
                    eprintln!(
                        "[mono-l1] overlapping function regions — blob rebuild aborted, wide sites stay generic"
                    );
                }
                return Ok(());
            }
            prev_end = start + f.bytecode_length as usize;
        }
        let mut new_blob: Vec<u8> = Vec::with_capacity(output.bytecode.len() + 256);
        let mut region_map: std::collections::HashMap<(u32, u32), (u32, u32)> =
            std::collections::HashMap::new();
        let mut cursor = 0usize; // read cursor over the OLD blob
        let mut new_offsets: Vec<(usize, u32, u32)> = Vec::with_capacity(order.len());
        for &i in &order {
            let (old_off, old_len) = {
                let f = &output.functions[i];
                (f.bytecode_offset as usize, f.bytecode_length as usize)
            };
            // verbatim gap before this function
            if old_off > cursor {
                new_blob.extend_from_slice(&output.bytecode[cursor..old_off]);
            }
            let new_off = new_blob.len();
            match rewritten.get(&i) {
                Some(bytes) => new_blob.extend_from_slice(bytes),
                None => new_blob
                    .extend_from_slice(&output.bytecode[old_off..old_off + old_len]),
            }
            let new_len = new_blob.len() - new_off;
            region_map.insert(
                (old_off as u32, old_len as u32),
                (new_off as u32, new_len as u32),
            );
            new_offsets.push((i, new_off as u32, new_len as u32));
            cursor = old_off + old_len;
        }
        if cursor < output.bytecode.len() {
            new_blob.extend_from_slice(&output.bytecode[cursor..]);
        }
        for (i, off, len) in new_offsets {
            output.functions[i].bytecode_offset = off;
            output.functions[i].bytecode_length = len;
        }
        for spec in output.specializations.iter_mut() {
            if let Some(&(off, len)) =
                region_map.get(&(spec.bytecode_offset, spec.bytecode_length))
            {
                spec.bytecode_offset = off;
                spec.bytecode_length = len;
            }
        }
        output.bytecode = new_blob;
        Ok(())
    }
