...
pub(in super::super) fn handle_ffi_extended(state: &mut InterpreterState) -> InterpreterResult<DispatchResult> {
    ...
    match sub_op {
        ...
        Some(FfiSubOpcode::TypedArrayElementAddr) => {
            ...
            let offset = idx_usize.checked_mul(elem_size).ok_or_else(|| {
        ...
        Some(FfiSubOpcode::ArrayToC) => {
            ...
            #[cfg(feature = "ffi")]
            {
                ...
                let buf_size = count.checked_mul(elem_size).ok_or_else(|| {
                ...
                state.ffi_array_buffers.push(super::super::super::state::FfiArrayBuffer {
                    buffer,
                    layout,
                    array_obj_ptr: arr_ptr as *mut u8,
                    count,
                    element_type,
                    is_mutable,
                });
        ...
        Some(FfiSubOpcode::CallFfiC) => {
            ...
            #[cfg(feature = "ffi")]
            {
                ...
                if let Some(sym) = module.get_ffi_symbol(symbol_id) {
                    if matches!(sym.signature.return_type, crate::module::CType::StructValue) {
                        if let Some(layout_idx) = sym.signature.return_layout_idx {
                            ...
                            if !c_buf_ptr.is_null() {
                                ...
                                unsafe { let _ = Box::from_raw(std::slice::from_raw_parts_mut(c_buf_ptr as *mut u8, 256)); }
