...
pub(in super::super) fn handle_call_method(
        ...
        if r.is_fat_ref() {
            ...
            if !ptr.is_null()
                && (ptr as usize)
                    .is_multiple_of(std::mem::align_of::<heap::ObjectHeader>())
            {
                // SAFETY: pointer alignment verified; every heap object
                // begins with an ObjectHeader, read-only.
                unsafe { heap::ObjectHeader::try_type_id(ptr).map(|t| t.0) }
            } else {
                None
            }
        } else if r.is_int() {
    ...
    if let Some(target_func_id) = found_func_id {
        ...
        if let Some(key) = cache_key {
            state.method_cache.insert(key, target_func_id);
        }
