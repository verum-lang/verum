...
pub(in super::super) fn handle_call_method(state: &mut InterpreterState) -> InterpreterResult<DispatchResult> {
    ...
    if is_cbgr_ref(&receiver) {
        if let Some(result) = dispatch_primitive_method(state, &receiver, &method_name, &args)? {
    ...
    if dispatch_receiver.is_ptr() && !dispatch_receiver.is_nil() {
        if let Some(result) = dispatch_array_method(state, dispatch_receiver, &bare_method_name, &args)? {
            state.set_reg(dst, result);
            return Ok(DispatchResult::Continue);
        }
    }
    ...
    if dispatch_receiver.is_ptr() && !dispatch_receiver.is_nil() {
        if let Some(result) = dispatch_variant_method(state, dispatch_receiver, &bare_method_name, &args)? {
            state.set_reg(dst, result);
            return Ok(DispatchResult::Continue);
        }
    }
    ...
    let is_builtin_collection = if dispatch_receiver.is_ptr() && !dispatch_receiver.is_nil() {
        ...
        let header = unsafe { &*(ptr as *const heap::ObjectHeader) };
        let is_builtin = header.type_id == TypeId::MAP || header.type_id == TypeId::SET || header.type_id == TypeId::LIST
    ...
    if !is_builtin_collection {
        if let Some(&cached_fid) = state.method_cache.get(&method_id) {
...
pub(super) fn dispatch_primitive_method(
    ...
    if receiver.is_int() {
        ...
        let result = match method {
            ...
            "is_ascii" => Value::from_bool(v >= 0 && v <= 127),
            ...
            "byte$is_ascii" => Value::from_bool(v >= 0 && v <= 127),
    ...
    if receiver.is_ptr() && !receiver.is_nil() {
        ...
        if header.type_id == TypeId::UNIT && header.size as usize == 4 * std::mem::size_of::<Value>() {
            ...
            if iter_type >= ITER_TYPE_LIST && iter_type <= ITER_TYPE_RANGE {
                match iter_method {
                    "next" => {
                    ...
                    "count" => {
                        ...
                        let remaining = if front_idx >= back_idx { 0 } else { back_idx - front_idx };
...
pub(super) fn dispatch_variant_method(
    ...
    if type_id_val < 0x8000 || type_id_val >= 0xA000 {
...
pub(super) fn dispatch_array_method(
    ...
    if type_id_val >= TypeId::FIRST_USER && type_id_val < 256 {
        return Ok(None);
    }
