...
pub(in super::super) fn handle_call(
    ...
    // diagnosis.
    // Canonical stub-id ranges live in `crate::stub_ranges` (stages
    // 1-4). When the producing module's real body lands, the registry
    // entry is overwritten, BUT any bytecode that compiled BEFORE the
    // overwrite still holds the stub_id at its Call sites.
    // `ArchiveBodyRemap::map_function` actively rejects stub-range ids
    // in Tier 2a/2b so the remap can chase the real id via the
    // archive-wide name index — this branch is a runtime safety net
    ...
    // the marshaling contract.
    {
        ...
        if !func_name.is_empty() {
            ...
            if let Some(result) = super::file_runtime::try_intercept_file_runtime(
                state,
                &func_name,
                args.start.0,
                args.count,
            ...
            if let Some(result) = super::process_runtime::try_intercept_process_runtime(
                state,
                &func_name,
                args.start.0,
                args.count,
...
fn try_dispatch_intrinsic_by_name(
    ...
    match name {
        ...
        // ================================================================
        // Bit manipulation (T0612)
        // ================================================================
        // `core/math/bits.vr` declares these BODYLESS `@intrinsic`, so at
        // Tier 0 a call reaches them only through this table. Every one had
        // zero arms here and zero anywhere else in the interpreter — searched
        // both the intrinsic tag and the bare name — so `popcnt(x)` and
        // friends hit the declaration stub or a LoadNil, the same silent-wrong
        // fate this row reported for `sqrt` (whose arm, unlike these, already
        // existed).
        //
        // Keyed on the BARE FUNCTION NAME, not only the intrinsic tag,
        // because the tag is AMBIGUOUS across widths: `rotl` and `rotl32`
        // both carry `@intrinsic("rotate_left")`, and both take (value,
        // amount), so neither the tag nor the arity distinguishes them. The
        // callee name does.
        //
        // Declared operand type is UInt64/UInt32; the interpreter carries
        // them as i64, so each op casts to the unsigned width, computes, and
        // returns through i64. Counts are UInt32 by declaration and fit
        // trivially.
        "clz" | "llvm.ctlz.i64" => {
            let x = get_i64_arg(state, 0) as u64;
            Ok(Some(Value::from_i64(x.leading_zeros() as i64)))
        }
        "ctz" | "llvm.cttz.i64" => {
            let x = get_i64_arg(state, 0) as u64;
            Ok(Some(Value::from_i64(x.trailing_zeros() as i64)))
        }
        "popcnt" | "llvm.ctpop.i64" => {
            let x = get_i64_arg(state, 0) as u64;
            Ok(Some(Value::from_i64(x.count_ones() as i64)))
        }
        "bswap" | "llvm.bswap.i64" => {
            let x = get_i64_arg(state, 0) as u64;
            Ok(Some(Value::from_i64(x.swap_bytes() as i64)))
        }
        "bswap16" | "llvm.bswap.i16" => {
            let x = get_i64_arg(state, 0) as u16;
            Ok(Some(Value::from_i64(x.swap_bytes() as i64)))
        }
        "bitreverse" | "llvm.bitreverse.i64" => {
            let x = get_i64_arg(state, 0) as u64;
            Ok(Some(Value::from_i64(x.reverse_bits() as i64)))
        }
        // Rotates take (value, amount). Rust's `rotate_*` already reduce the
        // amount modulo the width, matching LLVM's `fshl`/`fshr` semantics
        // for these declarations.
        "rotl" => {
            let x = get_i64_arg(state, 0) as u64;
            let n = get_i64_arg(state, 1) as u32;
            Ok(Some(Value::from_i64(x.rotate_left(n) as i64)))
        }
        "rotr" => {
            let x = get_i64_arg(state, 0) as u64;
            let n = get_i64_arg(state, 1) as u32;
            Ok(Some(Value::from_i64(x.rotate_right(n) as i64)))
        }
        "rotl32" => {
            let x = get_i64_arg(state, 0) as u32;
            let n = get_i64_arg(state, 1) as u32;
            Ok(Some(Value::from_i64(x.rotate_left(n) as i64)))
        }
        "rotr32" => {
