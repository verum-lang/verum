...
use handlers::bitwise::{
    handle_addg, handle_band, handle_bnot, handle_bor, handle_bxor, handle_divg, handle_mulg,
    handle_shl, handle_shr, handle_subg, handle_ushr,
};
use handlers::comparison::{
    handle_cmp_extended, handle_cmpg, handle_eqf, handle_eqg, handle_eqi, handle_eqref, handle_gef,
    handle_gei, handle_gtf, handle_gti, handle_lef, handle_lei, handle_ltf, handle_lti, handle_nef,
    handle_nei,
};
use handlers::control_flow::{
    handle_jump, handle_jump_eq, handle_jump_ge, handle_jump_gt, handle_jump_if,
    handle_jump_if_not, handle_jump_le, handle_jump_lt, handle_jump_ne, handle_land, handle_lnot,
    handle_lor, handle_lxor, handle_return, handle_return_unit,
};
use handlers::data_movement::{
    handle_cvt_bi, handle_cvt_ci, handle_cvt_fi, handle_cvt_ic, handle_cvt_if, handle_cvt_tof,
    handle_cvt_toi, handle_load_false, handle_load_nil, handle_load_smalli, handle_load_true,
    handle_load_unit, handle_loadf, handle_loadi, handle_loadk, handle_loadt, handle_mov,
    handle_nop,
};
use handlers::debug::{
    handle_assert, handle_debug_print, handle_ensures, handle_guard, handle_invariant,
    handle_panic, handle_requires, handle_spec, handle_unreachable,
};
use handlers::float_arith::{
    handle_absf, handle_addf, handle_divf, handle_modf, handle_mulf, handle_negf, handle_powf,
    handle_subf,
};
use handlers::integer_arith::{
    handle_absi, handle_addi, handle_dec, handle_divi, handle_inc, handle_modi, handle_muli,
    handle_negi, handle_powi, handle_subi, handle_udivi, handle_umodi,
};
...
pub(crate) use handlers::memory_collections::{value_eq, value_hash};

use handlers::cbgr::{
    handle_cbgr_extended, handle_chk_ref, handle_deref, handle_deref_mut, handle_drop_ref,
    handle_ref_checked, handle_ref_create, handle_ref_mut, handle_ref_unsafe,
};
use handlers::exceptions::{handle_get_exception, handle_throw, handle_try_begin, handle_try_end};
use handlers::generators::{
    handle_generator_create, handle_generator_has_next, handle_generator_next,
    handle_generator_yield,
};
use handlers::iterators::{handle_iter_new, handle_iter_next, handle_new_range};
use handlers::memory_collections::{
    handle_array_len, handle_clone, handle_get_field, handle_get_index, handle_list_pop,
    handle_list_push, handle_map_contains, handle_map_get, handle_map_set, handle_new,
    handle_new_array, handle_new_channel, handle_new_deque, handle_new_generic, handle_new_list,
    handle_new_map, handle_new_set, handle_pop, handle_push, handle_set_contains, handle_set_field,
    handle_set_index, handle_set_insert, handle_set_remove,
};
...
use handlers::string_ops::{handle_char_to_str, handle_concat, handle_to_string};

// Call operations
use handlers::calls::{
    handle_call, handle_call_cached, handle_call_closure, handle_call_generic,
    handle_call_indirect, handle_call_virtual, handle_new_closure, handle_tail_call_op,
};
...
use handlers::context::{
    handle_attenuate, handle_ctx_get, handle_ctx_pop, handle_ctx_provide, handle_ctx_push,
    handle_has_capability, handle_require_capability,
};

// Meta operations
use handlers::meta::{
    handle_meta_eval, handle_meta_quote, handle_meta_reflect, handle_meta_splice,
};

// System operations
use handlers::system::{
    handle_atomic_cas, handle_atomic_fence, handle_atomic_load, handle_atomic_store,
    handle_grad_accumulate, handle_grad_begin, handle_grad_checkpoint, handle_grad_end,
    handle_grad_stop, handle_io_poll, handle_io_submit, handle_mmap, handle_munmap, handle_syscall,
...
use handlers::log_extended::handle_log_extended;
use handlers::mem_extended::handle_mem_extended;
...
pub(crate) fn call_closure_sync(
    ...
    state
        .registers
        .try_push_frame(reg_count)
        .map_err(|new_top| InterpreterError::StackOverflow {
            depth: new_top,
...
pub(crate) fn execute_pending_task(
    state: &mut InterpreterState,
    task_id: TaskId,
) -> InterpreterResult<()> {
...
pub(crate) fn alloc_list_from_values(
    ...
    let backing_data =
        unsafe { (backing.as_ptr() as *mut u8).add(super::heap::OBJECT_HEADER_SIZE) as *mut Value };
    ...
    let data_ptr =
        unsafe { (obj.as_ptr() as *mut u8).add(super::heap::OBJECT_HEADER_SIZE) as *mut Value };
...
pub fn dispatch_loop_table_with_entry_depth(
    ...
    let timeout_deadline: Option<std::time::Instant> = if state.config.timeout_ms > 0 {
        Some(std::time::Instant::now() + std::time::Duration::from_millis(state.config.timeout_ms))
    } else {
        None
    };
    ...
    loop {
        ...
        if state.global_instruction_count > state.config.max_instructions
            && state.config.max_instructions > 0
        {
            return Err(InterpreterError::InstructionLimitExceeded {
                count: state.global_instruction_count,
...
mod tests {
    use super::*;
    use crate::bytecode;
    use crate::instruction::{ArithSubOpcode, Instruction};
    ...
    fn test_poly_add_integers() {
        ...
        state
            .call_stack
            .push_frame(FunctionId(0), 4, 0, Reg(0))
            .unwrap();
