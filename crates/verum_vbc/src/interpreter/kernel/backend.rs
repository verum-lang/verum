...
impl BackendRegistry {
 ...
 // MLIR-JIT compute backend.  Registered alongside `CpuBackend`
 // under `DeviceId::mlir_jit(0)` when the `mlir-jit` feature is on.
 // Routes tensor compute through `linalg`/`vector`/`gpu` dialects
 // → `ExecutionEngine` → cached function pointer; the per-dtype
 // scalar fallback in `kernel/cpu.rs` handles the cases the JIT
 // doesn't yet wire (and is the only path on no-`mlir-jit` builds).
