...
impl DeviceId {
 ...
 /// MLIR-JIT base.  Logically runs on the host CPU but goes through
 /// the MLIR `linalg` / `vector` / `gpu` dialect → JIT compile →
 /// cached function-pointer pipeline instead of the scalar fallback
 /// in `kernel/cpu.rs`.
