...
pub fn diag_f32_scalar(
    ...
    if a.ndim == 2 {
        ...
        let diag_len = if k >= 0 {
            let k = k as usize;
            if k >= n { return Some(TensorHandle::zeros(&[0], DType::F32)?); }
            (m).min(n - k)
        } else {
            let k = (-k) as usize;
            if k >= m { return Some(TensorHandle::zeros(&[0], DType::F32)?); }
            (m - k).min(n)
        };
...
pub fn diag_f64_scalar(
    ...
    if a.ndim == 2 {
        ...
        let diag_len = if k >= 0 {
            let k = k as usize;
            if k >= n { return Some(TensorHandle::zeros(&[0], DType::F64)?); }
            (m).min(n - k)
        } else {
            let k = (-k) as usize;
            if k >= m { return Some(TensorHandle::zeros(&[0], DType::F64)?); }
            (m - k).min(n)
        };
...
pub fn complex_mul_f32(a: &TensorHandle, b: &TensorHandle) -> Option<TensorHandle> {
    if a.numel != b.numel || a.numel % 2 != 0 {
        return None;
    }
...
pub fn complex_pow_f32(base: &TensorHandle, exp: &TensorHandle) -> Option<TensorHandle> {
    if base.numel != exp.numel || base.numel % 2 != 0 {
        return None;
    }
