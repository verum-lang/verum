...
pub struct FfiStructField {
    ...
    /// Field alignment.
    pub align: u16,
    /// For a field whose type is itself a record laid out for C
    /// (`c_type == CType::StructValue`), the index into
    /// `VbcModule::ffi_layouts` of THAT record's layout.
    ///
    /// `CType` is a bare tag, so it can say "a struct by value" and not
    /// which struct. Without this index the marshaller had no way to
    /// reach the nested layout and skipped such a field in both
    /// directions — a `DarwinStat.st_mtime` never travelled to C and
    /// never came back, so `core/io/fs.vr`'s `metadata()` reported every
    /// file as modified at the epoch (T1359). `None` for every scalar
    /// and pointer field, which is all of them in a flat C struct.
    ///
    /// `serde(default)` matches the two fields `FfiSignature` added the
    /// same way, and covers a self-describing format. It does NOT make
    /// this additive on the WIRE: the `.vbc` writer is `bincode`, which
    /// is positional and non-self-describing, so an archive written
    /// before this field fails to decode rather than defaulting. That is
    /// what `VERSION_MINOR` 15 marks, and the failure is loud — a
    /// `VbcError::Serialization`, not a misread. Every archive in the
    /// tree is regenerated by the build.
    #[serde(default)]
    pub nested_layout: Option<u16>,
}
