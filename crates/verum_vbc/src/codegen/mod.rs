...
impl VbcCodegen {
    ...
    fn resolve_field_index(&mut self, type_name: Option<&str>, field_name: &str) -> u32 {
        if let Some(tn) = type_name {
            ...
            // **FUNDAMENTAL FIX 2026-05-27** — Defensive `self.types`-by-name fallback.
            //
            // When `type_name_to_id` AND `type_field_layouts` BOTH miss a
            // type name that the caller has affirmatively passed in (typical:
            // a stdlib record type loaded from the precompiled archive where
            // an earlier registration path populated `self.types` via
            // `push_type_dedupe` but the name-to-id and layout caches got
            // missed — e.g. the first-wins gate at
            // `import_archive_type_with_protocol_remap:15820` returns early
            // when a sibling descriptor was already in place, OR an
            // intermediate codegen phase invalidated the caches), scan
            // `self.types` directly by simple name.  The TypeDescriptor
            // is the single source of truth for field layout — every
            // other cache must be derivable from it.
            //
            // Closes the `UseAfterFreeError.new(...)` +2-field-shift defect
            // class documented in
            // `memory/use_after_free_error_field_shift_2026-05-27.md` and
            // `core-tests/mem/thin_ref/audit.md §6`.  Same root applies to
            // every cross-module record-return field-access pattern where
            // the user-side type-annotation is correct but the type-layout
            // caches are stale: BTreeMap (`[[btree_pattern_match_ref_generic_class]]`),
            // Enactment (`[[enactment_field_access_oob_2026-05-24]]`),
            // MemHeaderView, SpanRange, and the 8+ other types tracked
            // across audit files.
            //
            // The scan also backfills `type_field_layouts` so subsequent
            // lookups hit the fast path (no repeated O(N_types) scan).
            if let Some(td) = self.types.iter().find(|td| {
                matches!(td.kind, crate::types::TypeKind::Record)
                    && self.ctx.strings.get(td.name.0 as usize)
                        .is_some_and(|name| name == stripped)
            }) {
                let td = td.clone();  // borrow split: subsequent insert needs &mut self
                let layout: Vec<String> = td.fields.iter()
                    .filter_map(|f| self.ctx.strings.get(f.name.0 as usize).cloned())
                    .collect();
                let pos = layout.iter().position(|f| f == field_name);
                // Backfill caches for future lookups regardless of whether
                // THIS lookup found the field — the layout is authoritative
                // and other field-access sites benefit from the cached form.
                if !layout.is_empty() {
                    self.type_field_layouts
                        .entry(stripped.to_string())
                        .or_insert_with(|| layout.clone());
                    // Backfill type_name_to_id as well so subsequent
                    // `type_name_to_id.get(stripped)` lookups (line 13458)
                    // can short-circuit via the descriptor's own id.
                ...
                if let Some(idx) = pos {
                    if std::env::var("VERUM_DEBUG_FIELDS").is_ok() {
                        tracing::debug!(
                            "[FIELD] {}.{} → self.types backfill idx {} (layout: {:?})",
                            tn,
                            field_name,
                            idx,
                            layout
