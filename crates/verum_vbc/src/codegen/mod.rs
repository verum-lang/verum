...
impl VbcCodegen {
        ...
        for item in module.items.iter() {
            ...
            if let ItemKind::Type(type_decl) = &item.kind {
                if let TypeDeclBody::Protocol(protocol_body) = &type_decl.body {
                    ...
                    for protocol_item in &protocol_body.items {
                        if let ProtocolItemKind::Function { decl, default_impl } = &protocol_item.kind {
                            // Check if method has a default implementation
                            if let verum_common::Maybe::Some(body) = default_impl {
            ...
            if let ItemKind::Context(ctx_decl) = &item.kind {
                ...
                if !self.type_name_to_id.contains_key(&ctx_name) {
                    ...
                    type_desc.id = type_id;
                    type_desc.name = StringId(self.ctx.intern_string_raw(&ctx_name));
                    type_desc.kind = crate::types::TypeKind::Protocol;
    ...
    fn extract_intrinsic_name(&self, func: &FunctionDecl) -> Option<String> {
            ...
            if attr.name.as_str() == "intrinsic" {
                if let verum_common::Maybe::Some(args) = &attr.args
                    && let Some(first_arg) = args.first() {
                        if let verum_ast::ExprKind::Literal(lit) = &first_arg.kind {
        ...
        if let verum_common::Maybe::Some(body) = &func.body {
            ...
            if let Some(expr) = body_expr {
                if let verum_ast::ExprKind::MetaFunction { name, args } = &expr.kind {
                    if name.name.as_str() == "intrinsic" {
        ...
        for attr in attrs.iter() {
            match attr.name.as_str() {
                ...
                "repr" => {
                    if let verum_common::Maybe::Some(ref args) = attr.args {
                        if let Some(first) = args.first() {
                            if let Some(repr_name) = Self::attr_arg_as_ident(first) {
    ...
    fn register_function(&mut self, func: &FunctionDecl) -> CodegenResult<()> {
        ...
        let return_type = if let verum_common::Maybe::Some(ref ret_ty) = func.return_type {
            Some(self.ast_type_to_type_ref(ret_ty))
        } else {
            None
        };
            ...
            MountTreeKind::Glob(glob_path) => {
                ...
                for (name, info) in self.ctx.functions.iter() {
                    ...
                    let matches = name.starts_with(&prefix_dot)
                        || core_prefix_dot.as_ref().map_or(false, |cp| name.starts_with(cp));
                    if matches {
    ...
    fn register_impl_function(&mut self, func: &FunctionDecl, type_name: &str) -> CodegenResult<()> {
        ...
        let takes_self_mut_ref = func.params.first().map_or(false, |param| {
    ...
    fn emit_context_transforms(&mut self, func: &verum_ast::decl::FunctionDecl) -> usize {
        ...
        for ctx in &func.contexts {
            ...
            if let verum_common::Maybe::Some(ref cond) = ctx.condition {
                if !Self::evaluate_context_condition(cond) {
                    continue;
                }
            }
    ...
    fn register_type_constructors(
        ...
        match &type_decl.body {
            ...
            TypeDeclBody::Record(fields) => {
                ...
                let mut type_desc = TypeDescriptor::default();
                type_desc.id = type_id;
                type_desc.name = StringId(self.ctx.intern_string_raw(&type_name));
                type_desc.kind = crate::types::TypeKind::Record;
                ...
                for generic in &type_decl.generics {
                    if let verum_ast::ty::GenericParamKind::Type { name, bounds, default } = &generic.kind {
                        ...
                        let mut param_desc = crate::types::TypeParamDescriptor::default();
                        param_desc.name = param_name_id;
                        param_desc.id = crate::types::TypeParamId(type_desc.type_params.len() as u16);
                        param_desc.bounds = bound_ids;
                        param_desc.default = default_ref;
            ...
            TypeDeclBody::Protocol(protocol_body) => {
                ...
                for protocol_item in &protocol_body.items {
                    if let verum_ast::decl::ProtocolItemKind::Function { decl, .. } = &protocol_item.kind {
                        ...
                        let mut variant_desc = crate::types::VariantDescriptor::default();
                        variant_desc.name = method_name_id;
                        variant_desc.tag = type_desc.variants.len() as u32;
                        variant_desc.payload = Some(method_type_ref);
    ...
    fn register_constant_with_value(
        ...
        let intrinsic_name = value_expr
            .and_then(|expr| Self::extract_const_literal_value(expr))
    ...
    fn build_module(&mut self) -> CodegenResult<VbcModule> {
        ...
        for ty in &self.types {
            ...
            if let Some(drop_fn) = remapped_ty.drop_fn {
                if let Some(&new_id) = func_id_remap.get(&drop_fn) {
            ...
            if let Some(clone_fn) = remapped_ty.clone_fn {
                if let Some(&new_id) = func_id_remap.get(&clone_fn) {
            ...
            for proto_impl in remapped_ty.protocols.iter_mut() {
                for fn_id in proto_impl.methods.iter_mut() {
                    if *fn_id != u32::MAX {
                        if let Some(&new_id) = func_id_remap.get(fn_id) {
                            *fn_id = new_id;
                        }
                    }
                }
