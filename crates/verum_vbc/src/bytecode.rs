...
pub fn encode_instruction(instr: &Instruction, output: &mut Vec<u8>) -> usize {
    ...
    match instr {
        ...
        Instruction::GpuExtended { sub_op, operands } => {
            // GpuExtended carrier uses the same length-prefix convention as all
            // other extended families (Ffi/Math/Arith/Text/…) so the linker,
            // disassembler, and validator can round-trip it without knowing
            // per-sub_op operand schemas.  The interpreter skips the length byte
            // before dispatching on sub_op.
            output.push(Opcode::GpuExtended.to_byte());
            output.push(*sub_op);
            encode_varint(operands.len() as u64, output);
            output.extend_from_slice(operands);
...
pub fn decode_instruction(data: &[u8], offset: &mut usize) -> VbcResult<Instruction> {
    ...
    match opcode {
        ...
        // ====================================================================
        // The GpuExtended carrier uses length-prefix encoding (same as all
        // other extended families) so the linker/disassembler can round-trip
        // it without knowing per-sub_op operand schemas.  The interpreter
        // skips the length prefix in handle_gpu_extended before dispatching.
        //
        // Structured per-sub_op decode (previously here) was removed because
        // the intrinsic codegen (emit_intrinsic_gpu_extended) writes only
        // register operands while many structured decoders expected extra
        // non-register fields (direction bytes, kernel_id varints, etc.),
        // causing systematic TruncatedBytecode failures in the linker.
        Opcode::GpuExtended => {
            let sub_op_byte = decode_u8(data, offset)?;
            let operands = decode_extended_operands(data, offset)?;
            Ok(Instruction::GpuExtended { sub_op: sub_op_byte, operands })
        }
        // ====================================================================
...
impl TryFrom<u8> for TensorUnaryOp {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            0x16 => Ok(TensorUnaryOp::Exp2),
            0x17 => Ok(TensorUnaryOp::Expm1),
            0x18 => Ok(TensorUnaryOp::Log10),
            0x19 => Ok(TensorUnaryOp::Log1p),
            0x1A => Ok(TensorUnaryOp::Square),
            0x1B => Ok(TensorUnaryOp::Asin),
            0x1C => Ok(TensorUnaryOp::Acos),
            0x1D => Ok(TensorUnaryOp::Atan),
            0x1E => Ok(TensorUnaryOp::Sinh),
            0x1F => Ok(TensorUnaryOp::Cosh),
            0x20 => Ok(TensorUnaryOp::Asinh),
            0x21 => Ok(TensorUnaryOp::Acosh),
            0x22 => Ok(TensorUnaryOp::Atanh),
            0x23 => Ok(TensorUnaryOp::Trunc),
            0x24 => Ok(TensorUnaryOp::Erfc),
            _ => Err(()),
        }
    }
}
...
mod tests {
    ...
    fn test_gpu_launch_roundtrip() {
        // GpuExtended carrier form (length-prefix encoding used by all
        // extended families including GpuExtended).  The structured
        // GpuLaunch variant uses the same 0xF8 opcode but the decoder
        // always returns the carrier — so we test the carrier directly.
        use crate::instruction::GpuSubOpcode;
        let operands = vec![
            42u8, // kernel_id varint (fits in 1 byte)
            0, 1, 2, // grid regs
            3, 4, 5, // block regs
            6,    // shared_mem
            7,    // stream
            2, 8, 9, // reg_vec: count=2, r8, r9
        ];
        test_roundtrip(&Instruction::GpuExtended {
            sub_op: GpuSubOpcode::Launch as u8,
            operands,
        });
    }
