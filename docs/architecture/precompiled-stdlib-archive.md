...
* **Phase 6 (6b)** — `d94314bc` — **VbcLinker**: cross-archive merge with shared remap pool. 1037 lines + 5 unit tests + 1 e2e integration test. Real 9 MB stdlib archive (563 modules, 3964 functions) merges through the linker without panics. Two-pass `add_archive` walks the archive once to populate a shared per-archive remap table, then walks again to append descriptors / bodies. String pool content-deduped. Lenient identity-mapping for cross-module ContextRef / ProtocolId / TypeId / StringId references handles the `compile_core` global-registration pattern.
...
* **Symbol-graph sidecar** — T0753 — `runtime.symbol_graph` beside `runtime.vbca`, embedded through `STDLIB_SYMBOL_GRAPH_PATH`. The archive's call-graph index (which entry defines a qualified name, which names share a leaf or a first segment, what each function calls) is now WRITTEN at bake time and READ as bytes, instead of being rebuilt at every compiler start by decoding all 590 entries and disassembling every body. Measured before the change, on `fn main() { print("hello") }`: 350–810 ms, 87 % of that program's whole archive-load phase, plus the bulk of a ~700 MB peak — for an answer that does not depend on the program at all. Format and reader: `crates/verum_compiler/src/symbol_graph_baked.rs` (one byte block, string blob + `u32` index arrays, binary search over sorted keys, no decode step). The scan remains as the fallback for a compiler built without a bake and is the producer the bake itself calls, so both paths share one reader; `VERUM_NO_BAKED_SYMBOL_GRAPH=1` forces the fallback for A/B from a single binary, and an in-tree test compares the sidecar against a fresh scan row by row.
...
| Phase | Status | Note |
|-------|--------|------|
| **6c bytecode rewriter** | not started | `rewrite_bytecode_block` is currently a verbatim-copy stub. The decode/rewrite/encode walker for the ~40 id-bearing instruction variants (Call / TailCall / CallM / CallClosure / MakeClosure / LoadK / TypeOf / IsType / NewS / NewV / FfiCall and others) lands as the next batch. Required for user-on-top-of-stdlib merge — until then user module bytecode references can't traverse into stdlib-archive function IDs. **Estimated: 600-800 lines.** |
| **6d compile_ast_to_vbc switch** | blocked on 6c | Replace source-driven `collect_imported_stdlib_modules + per-module codegen` in `compile_ast_to_vbc` with `VbcLinker::new(triple).add_archive(embedded_stdlib_vbc::get_runtime_archive()).add_user_module(user_only_codegen).finalize()`. Hot path goes from ~25 min to <50 ms. Keeps `VERUM_NO_PRECOMPILED_STDLIB=1` dev-mode fall-through. **Estimated: 200 lines + measurement.** |
