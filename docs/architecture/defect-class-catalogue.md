...
| **Counter-case for the fix discipline (measured 2026-09-09)** | `.0` DOES NOT WORK when the newtype came from an FFI wrapper, and the two origins have OPPOSITE representations. This row's trigger is a value that lost its box; a value produced by a `safe_*` shim KEEPS one. Fifteen lines, Tier 0, no AOT: `let fd = safe_open(...)` then `f"{fd}"` and `f"{fd.0}"` print the SAME thing — `{3}` — so `.0` returns the receiver rather than the field; `fd.0 + 0` yields 36066691904 (the object's address), `fd.0 == 3` is FALSE and `fd.0 > 0` is TRUE, which is why every `if fd >= 0` guard passes. `as_raw()`, the type's own unwrapper, returns `{3}` as well. Control in the same program: a locally built `FileDesc(3)` prints `3` and `loc.0 + 1` is 4, so the newtype is transparent when it is not FFI-produced. Consequence for the advice above: **reading `.0` is origin-dependent and silently wrong for one of the two origins** — it returns an address that compares greater than zero. Tracked as T1192; the file layer's Tier-1 failure is this, reached through `File { fd: fd.as_raw() }`. |
