...
| A3 | **Lenient-SKIP BugClass warnings** — tracked in-code as #166. PROGRESS 2026-07-11: .vr-side burn-down **176 → 132** (four bare-variant qualification waves across 25 files: tls/bls/verifiers/sqlite pragma+record+status families/ocsp/thread — each was stubbing whole functions incl. the entire l3_btree page-parsing path). REMAINDER 132 is compiler-side, precisely classified: (a) undefined FUNCTION ~39 (Text.new ×11 …) = the #47 cross-module bake forward-ref architecture item; (b) closure-captured outer vars in f-string-in-closure-args ~20 (`map_err(|_| E(f"{i}"))` — fresh minimal class); (c) meta const-generic params in bodies (N/DIMS) ~6; (d) braced-mount const resolution (WAL_HEADER_SIZE); (e) arity drifts ×5. Flip-to-hard-error stays gated on (a). | P0→P1 | `codegen/error.rs:255`; waves: 98d78edfc, 856431e34 + wave-4 | zero BugClass skips; then promote to hard error |
