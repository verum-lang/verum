...
| # | Item | Pri | Anchor | Acceptance |
|---|------|-----|--------|------------|
| C1 | **Duplicate public type names** — 155 names declared in ≥2 files (326 decls); 64 kind-divergent genuine collisions, of which **27 are protocol-vs-record hard collisions**. PROGRESS 2026-07-11: **21/27 resolved** (batches 1-3: sqlite-internal Capability family unified, Capability family unified onto database/common; CommitDecision/AuthDecision/UpdateOp/ProgressDecision deduped to their authorities; TraceEvent→TraceEventKind, BusyDecision→BusyHookDecision (ms-vs-ns unit honesty), TraceCtx→TraceHookCtx). REMAINING 6: `Context` (async/waker canonical futures Context KEEPS bare name; verify kernel_v0-vs-kernel_soundness twins → unify within the verify-kernel consolidation), `RetryPolicy` ×3 (db common / runtime recovery / shell — cross-domain unification per redundancy directive, design decision), `Tokenizer` (sqlite fts5 ×2 same-domain unify + math/agent), `PageHeader` (sqlite btree vs mem allocator/heap → rename mem side), `InclusionProof` (merkle vs provenance vs sigstore — same security domain, unify), `Proof` (math/logic vs zk twins). 91 same-kind twins (e.g. `Proof`, `AllocHandle`, `JoinHandleOpaque`) unify per redundancy directive. | P1 | cat1_dups.txt | remaining 16 hard collisions renamed/qualified; twins unified |
