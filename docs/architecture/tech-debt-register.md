...
| # | Item | Pri | Anchor | Acceptance |
|---|------|-----|--------|------------|
| C1 | **Duplicate public type names** — 155 names declared in ≥2 files (326 decls); 64 kind-divergent genuine collisions, of which **27 are protocol-vs-record hard collisions**. PROGRESS 2026-07-11: **11/27 resolved** (batch 1, sqlite-internal: Capability family unified onto database/common; CommitDecision/AuthDecision/UpdateOp/ProgressDecision deduped to their authorities; TraceEvent→TraceEventKind, BusyDecision→BusyHookDecision (ms-vs-ns unit honesty), TraceCtx→TraceHookCtx). REMAINING 16: `Row` (context/standard record vs database/common protocol vs postgres record — postgres→PgRow, context mounts common), `Group` (cli/spec record vs math/algebra protocol vs meta/token), `Limit` (sqlite soft_heap record vs math analysis+category protocols — rename sqlite→SoftHeapLimit), `Transaction` (common protocol vs sqlite l7_api record → SqliteTransaction), `ProgressHandler` (ext/hooks protocol vs progress/handler — compare shapes, dedup or →ProgressHook), `Constraint`, `ColumnType`, `Context`, `RetryPolicy`, `Tokenizer`, `PageHeader`, plus kernel_v0-vs-kernel_soundness verify twins (same-domain, unify). 91 same-kind twins (e.g. `Proof`, `AllocHandle`, `JoinHandleOpaque`) unify per redundancy directive. | P1 | cat1_dups.txt | remaining 16 hard collisions renamed/qualified; twins unified |
