...
| A3 | **Lenient-SKIP BugClass warnings** — tracked in-code as #166. PROGRESS 2026-07-11: .vr burn-down **176 → 132**, then three class closures landed: (b) f-string-in-closure capture — `FreeVarAnalyzer` gained the `InterpolatedString` arm (embedded exprs now captured; regression test `free_var_analyzer_sees_through_interpolated_strings`); (d) braced-mount consts — **stage-4 pre-registration** (`pre_register_unique_public_consts`): uniquely-named public consts register order-independently, literal initializers as inline `__const_val_N`, the rest as name-remapped stubs on canonical `verum_vbc::stub_ranges` (which de-duplicated 10 mirrored range definitions); (e) arity drifts ×5 — callers aligned with real decls (fulcio alias→qualified call, `range()` shadow renamed, `topk` axis arg dropped ×3, `propagate_status` whole-theory pass). REMAINDER pending bake re-measure: (a) undefined FUNCTION ~39 (Text.new ×11 …) = #47 cross-module bake forward-ref; (c) meta const-generic params in bodies (N/DIMS) ~6. Flip-to-hard-error stays gated on (a). || A4 | **Field-intern fallback positional layout** — record field access falls back to positional offsets that are "NOT guaranteed" (wrong-offset landmine; defect-class §40). Being retired by the P2 dual-keying pillar. | P0 | `verum_vbc/src/codegen/mod.rs:16092` | P2 stages 2–4 land; §40/§46/#27 canaries green |
