...
| A2 | **Tensor/GPU unknown sub-ops silently write 0** — unlike every other category (which routes to `emit_unimplemented_sub_op`) the Tensor and GPU dispatchers const-zero unknown sub-ops with no marker. Silent-miscompile class. | P0 | `instruction.rs:32010` (tensor), `:32744` (gpu) | fallthroughs route to `emit_unimplemented_sub_op` |
| A3 | **Lenient-SKIP BugClass warnings** — codegen replaces failing fn bodies with panic-stubs and *warns*; the BugClass half (UndefinedFunction, TypeMismatch, arity drift, …) indicates real defects and should be loud. Tracked in-code as #166. | P0→P1 | `verum_vbc/src/codegen/mod.rs:4080/4237/2441`, class split `codegen/error.rs:255` | zero BugClass skips in a stdlib bake; then promote BugClass to hard error |
| A4 | **Field-intern fallback positional layout** — record field access falls back to positional offsets that are "NOT guaranteed" (wrong-offset landmine; defect-class §40). Being retired by the P2 dual-keying pillar. | P0 | `verum_vbc/src/codegen/mod.rs:16092` | P2 stages 2–4 land; §40/§46/#27 canaries green |
...
| A11 | **Tier-0 futex FFI gap** — sync/barrier (15 pins) + sync/condvar (9) blocked on futex from the interpreter. | P1 | INVENTORY rows | barrier/condvar suites green under `--interp` |
...
| # | Item | Pri | Anchor | Acceptance |
|---|------|-----|--------|------------|
| B1 | **type_id=0 record/variant field-access OOB** — highest-fanout runtime class (btree 13, RsaError 4, runtime/recovery §F, journal 17). | P0 | `core-tests/database/common/migrations/journal/unit_test.vr:95` | btree/journal/recovery pins un-ignore |
...
| # | Item | Pri | Anchor | Acceptance |
|---|------|-----|--------|------------|
| C1 | **Duplicate public type names** — 155 names declared in ≥2 files (326 decls); 64 kind-divergent genuine collisions, of which **27 are protocol-vs-record hard collisions** feeding the bare-name first-wins dispatch class (top offenders: `Capability` ×5 dirs, `Row`, `Group`, `Limit`, `Transaction`, `ProgressHandler`). 91 same-kind twins (e.g. `Proof` zk-halo2/zk-stark) unify per the redundancy directive. | P1 | cat1_dups.txt; e.g. `database/common/protocol.vr:50` vs `context/standard.vr:226` (`Row`) | 27 hard collisions renamed/qualified; twins unified |
| C2 | **Mount drift** — 44 genuine broken mounts / 14 distinct paths (earlier ~250 ballpark counted resolvable relative forms). Dominant (~30): `core.maybe`/`core.result`/`core.ordering`/`core.ops`/`core.protocols`/`core.iterator` short-paths that skip `base.` — dead mounts surviving via the auto-injected prelude. Plus `core.async.spawn` ×4, `async.duration` → `core.time.duration`, `.mod`-suffix antipattern ×2, `core.memory.null_ptr` ×2. | P1 | cat2_drift.txt; e.g. `base/result.vr:17-24`, `database/common/pool.vr:27` | bake-verified zero broken mounts |
| C3 | **Unregistered `@intrinsic` calls** — 564 distinct names / 578 occurrences. 339 occ are expression-form → **silent LoadNil** (`expressions.rs:26463`); 239 are bodyless-decl form → silent Unit. Split: (a) ~bare families with existing opcode handlers (GPU set, SIMD compares, checked/overflowing/widening arith, transmute/ptr_read, sincos) = registry `name:`/alias entries; (b) `llvm.*.with.overflow.*` = alias arms; (c) ~250 `verum.<host>.*` (crypto/tls/quic/k8s/xds/compress/cloud) = deliberate not-yet-implemented host subsystems — track separately, do not mix with (a)/(b). Also: the miss path itself should be LOUD (compile warning), not LoadNil. | P0(a,b)/P2(c) | cat4_unresolved.txt; `math/checked.vr:117`, `simd/mod.vr:372`, `intrinsics/arithmetic.vr` | (a)+(b) resolve in registry; miss path warns at bake |
| C4 | **Dead/drifted surface** — `proof/contracts_old.vr` orphan (no successor, still mounted via `proof/mod.vr:40`, 19 public decls); decls-vs-impls drift: `AllocHandle` (`is RawAllocHandle` vs `is (Int)`) and `JoinHandleOpaque` diverged between `intrinsics/runtime/*` and `runtime/*`. | P2 | audit-stdlib cat 3 | contracts_old retired; twins reconciled |
