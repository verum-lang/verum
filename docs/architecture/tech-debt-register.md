...
| # | Item | Pri | Anchor | Acceptance |
|---|------|-----|--------|------------|
| C1 | **Duplicate public type names** — 155 names declared in ≥2 files (326 decls); 64 kind-divergent genuine collisions, of which **27 are protocol-vs-record hard collisions**. PROGRESS 2026-07-11: **25/27 resolved** (batches 1-4: sqlite-internal Capability family unified, Capability family unified onto database/common; CommitDecision/AuthDecision/UpdateOp/ProgressDecision deduped to their authorities; TraceEvent→TraceEventKind, BusyDecision→BusyHookDecision (ms-vs-ns unit honesty), TraceCtx→TraceHookCtx). REMAINING 2 (design-level): verify-kernel `Context`/`Judgment`/`Derivation`/`CoreTerm`/`LemmaStatus` twins — kernel_v0 vs kernel_soundness parallel systems need consolidation, not renames; `InclusionProof` triple (merkle / provenance / sigstore — one security-domain contract). 91 same-kind twins (e.g. `Proof`, `AllocHandle`, `JoinHandleOpaque`) unify per redundancy directive. | P1 | cat1_dups.txt | remaining 16 hard collisions renamed/qualified; twins unified |
