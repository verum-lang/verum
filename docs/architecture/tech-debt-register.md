...
| B1b | **FFI-BYTE-BUFFER-STRIDE-1 (net-runtime root, P0).** No Tier-0 .vr construct materializes PACKED ABI bytes for a `&unsafe Byte` FFI arg: `List<Byte>.as_ptr()` exposes NaN-boxed 8-byte-strided storage (witness `raw[1]=0` where sin_family=2 expected, `raw[6..7]=0xF9,0x7F` = Int NaN-box header); `[Byte; N]` and `@repr(C) DarwinSockaddrIn` address-of both read all-zeros (record header, not packed layout); ONLY `Text.as_bytes()` (BYTE_SLICE) is packed. `BytewiseFfi` (core.base.coercion) is a TYPE-SYSTEM marker with NO runtime packing lowering. Same gap `fill_secure` routes around via random_u64 instead of getentropy. Effect: `bind`/`sendto`/`recvfrom` see sin_family=0 → EAFNOSUPPORT(47) on macOS — the WHOLE net family's runtime failure, user-replica reproduces identically (NOT a stdlib codegen defect). FUNDAMENTAL FIX: interpreter FFI-call handler must MATERIALIZE a packed byte buffer from NaN-boxed List<Byte>/[Byte;N]/BytewiseFfi-record args before the call (and unpack out-params after); AOT mirror. 4 reproducible probes in scratchpad (probe_stride/asbytes/arr/pack). | P0 | probe_udp.vr; scratchpad probes | UDP loopback green; net pins un-ignore || B3 | **Intrinsic resolution pins** — ARITH-MISSING-INTRINSICS-1 (13), INTRINSIC-GENERIC-WRAPPER-ARCHIVE-1 (5), nested-call (1), MEM-TRANSMUTE-FLOAT-1 (3). | P1 | `core-tests/intrinsics/*` | intrinsics suites unpinned |
