...
| # | Item | Pri | Anchor | Acceptance |
|---|------|-----|--------|------------|
| B1 | **type_id=0 record/variant field-access OOB** — LAYER-1 **CLOSED 2026-07-12** (TUPLE-POSITIONAL-FIELD-1): `_N` fields are positional by construction; `FileDesc._0` intern-fallback (idx 4) wrote field 4 into the 1-slot baked ctor — dump witness `NEW r18 fields=1 + SET_F r18.4` (bakeAF, via new VERUM_DUMP_FN tool) → now `SET_F r18.0` (bakeAG), probe_udp advances past safe_socket. LAYER-2 (fresh witness): net path panics `'from_os' not found on receiver kind OSError, caller passed 2 args incl. self` — `IoError.from_os(e)` (alias `type IoError is StreamError`) compiled as INSTANCE CallM instead of static Call; candidates EngineIoError/StreamError.from_os arity-1 exist. Alias-static dispatch family (#17/#39, ALIAS-VS-MARKER door) — coordinate with dispatch campaign. btree/journal/recovery re-test after layer-2. | P0 | probe_udp.vr; bakeAG dump | UDP loopback probe green; then net pins un-ignore || B2 | **Dispatch classes**: iterator/Range (15), static-vs-instance (`SystemTime.now`, by-value-temporary chains), Float-Ord small-gap (~4), MUTSELF-MATCH (2). | P1 | audit-pins part 1 | per-class canary tests green |
