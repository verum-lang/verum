...
| B1c | **@ffi_name / @link_name ignored by FFI symbol resolution (P0, blocks UDP recv).** `@ffi_name("recvfrom") fn recvfrom_ffi(…)` dlsym'd `recvfrom_ffi` (the Verum fn name) instead of `recvfrom` → runtime "symbol 'recvfrom_ffi' not found". The attribute was never parsed into codegen's FFI symbol name. Affects ALL 5 `_ffi`-suffixed externs: mmap/recvfrom/rename/symlink/vm_allocate. FIX (landed, verify pending): extract_ffi_symbol_name reads @ffi_name/@link_name; ffi_symbol_link_names side-map populated in both register paths; build_module prefers it over the fn name for the dlsym key — one place, both tiers (interpreter dlsym + AOT extern both read module.ffi_symbols[].name). | P0 | codegen/mod.rs FFI symbol name | UDP recv round-trip green || B4 | **net/ test harness absent** — ~40–77 pins are "requires live fixture/mock" (DNS/TCP/UDP/UNIX/TLS/QUIC/H3/WS/proxy). One loopback-fixture harness retires them wholesale. | P1 | audit-pins class 9 | harness lands; net pins converted to fixture tests |
