...
Implementation `core/theory_interop/` is already partially landed — it is a MODULE DIRECTORY (`core/mod.vr:517` declares `public module theory_interop;`), not the single file this line named; VVA stabilises the API.
...
**Source**: the Diakrisis corpus, 02-canonical-primitive/02-axiomatics (Diakrisis preprint, Sereda 2026). This subsection is the *integration surface* that maps Diakrisis's 13-axiomatic canonical-primitive specification onto Verum's kernel typing layer (§4.4) and `core.math.frameworks.diakrisis` package.
...
Existing productions in `grammar/verum.ebnf` (§ 2.12 `refinement`, § 2.14 `sigma_binding`, § 2.18 `refinement_method`). VVA reaffirms the three canonical forms and **deprecates the historical "five rules"** (cf. `crates/verum_ast/src/ty.rs` `RefinementPredicate` doc-comment, `docs/detailed/03-type-system.md` (no such file — `docs/detailed/` does
not exist; the type system's own truth is `crates/verum_types/src/infer/`
and `docs/architecture/unified-type-theory.md`) § 1.5):
...
**§8.5 V2 proof-replay framework — V8.1 #196 (foundation)**: new module `crates/verum_smt/src/proof_replay/` defines the cross-target lowering surface. Components:
...
$$
where `canonicalize` is the Nieuwenhuis–Oliveras congruence-closure followed by the Lurie idempotent-completion (Construction 16.3 in the Diakrisis corpus, 03-formal-architecture/16-gauge-decision). The complexity bound is $O\!\left(2^{O(|\alpha|)}\right)$ wallclock per Theorem 16.5.
...
**V2 enforcement** ✓ V8.1 (#196) — `verum audit --hygiene-strict` walks every top-level free function body for raw `self` occurrences (functions with no self-receiver param). Methods inside `implement` / `protocol` blocks and `&self` / `self` parameterised free functions are skipped (their `self` is properly bound). Violations surface as `E_HYGIENE_UNFACTORED_SELF`; CLI exits non-zero. Plain + JSON output (schema_version 1). 5 integration tests in `crates/verum_cli/tests/audit_hygiene_strict.rs`. Deeper coverage (typed `Self::Item` resolution, await/closure walker) remains a V2.1 follow-up — current walker covers the spec ask (raw `self` in free function bodies) at the syntactic AST layer.
...
Noesis (the Diakrisis corpus, 11-noesis) is a **separate project** that will use Verum. VVA defines what Verum owes Noesis and what Noesis handles internally.
...
- **Task F3** (V1 ✓ shipped + V2 ✓ shipped V8.1 #196): V1 — Articulation Hygiene reporter at `verum audit --hygiene` covering six §13.2 surface forms (inductive / coinductive / higher-inductive / newtype / `@recursive` / `@corecursive`); plain + JSON output. V2 — `verum audit --hygiene-strict` walks every top-level free function body for raw `self`; methods (functions with self-receiver param) skipped; violations surface as `E_HYGIENE_UNFACTORED_SELF` and CLI exits non-zero. 5 integration tests in `crates/verum_cli/tests/audit_hygiene_strict.rs`.
...
7. **[OPEN] OC/DC duality proof.** Restated: the auto-induced
   ...
   alongside the Diakrisis corpus.
...
- internal design note “03-type-system” (not in repo) — base type system, three refinement forms (descriptive).
- internal design note “09-verification-system” (not in repo) — verification system (descriptive).
- internal design note “12-dependent-types” (not in repo) — dependent types (planned, now superseded in part by VVA §7).
- internal design note “13-formal-proofs” (not in repo) — formal proofs (planned, now superseded in part by VVA §8).
- internal design note “17-meta-system” (not in repo) — meta-system (planned, now superseded in part by VVA §9).
- the MSFS preprint (not in repo) — MSFS preprint.
- the Verum-MSFS integration preprint (not in repo) — Verum-MSFS integration preprint.
- the Diakrisis corpus, 02-canonical-primitive/02-axiomatics.md — Diakrisis axioms.
- the Diakrisis corpus, 06-limits/10-maximality-theorems.md — 103.T–106.T maximality proofs.
- the Diakrisis corpus, 09-applications/01-verum-integration.md — Verum integration plan.
- the Diakrisis corpus, 11-noesis/16-verum-implementation.md — Noesis implementation.
- the Diakrisis corpus, 12-actic/* — Actic dual.
...
> primitive axiomatic stack (the Diakrisis axiomatic corpus)
> and the Verum-MSFS preprint
> (the MSFS preprint (not in repo)) directly
...
**Source**: the MSFS preprint (not in repo)
...
8. **Corpus-side proof-honesty validator + CI gate** ✓ — M0.E +
   ...
   trivial-true. website:docs/verification/proof-honesty.md
...
### A.Z code-side refactor status (#225 stages)
  ...
  * Stage 2.6 (rename across `docs/` + `vcs/` + remaining stragglers + lowercase `vuva-version` CLI flag → `vva-version`, `vuva_*` test idents → `vva-*`): ✓ shipped this batch. Verified `grep -rln "VUVA\|VFE"` returns 0 hits across all source-controlled `.rs` `.vr` `.md` `.toml` files (excluding external Diakrisis + MSFS source materials — external upstream source materials intentionally preserved as authoritative references). User-facing `vfe_<N>` annotation identifiers (`@require_extension(vfe_1)` etc.) are STABLE rollout-calendar markers retained verbatim per VVA §3 governance. **100%**.
...
website:docs/reference/grammar-ebnf.md#22-visibility-and-attributes
...
**Grammar impact**: расширяет `verify_attribute` enum в `grammar/verum.ebnf:441-445` с 9 до 12 strategies.
...
## 17. Grammar impact analysis (`grammar/verum.ebnf`)
