...
| `platform_ir.rs::emit_exception_handling` + `vbc_lowering.rs:3270` | `setjmp`/`longjmp`    | Exception unwinding primitive.  Cross-compilation bug fixed (now reads `target_is_darwin(module)` instead of host `cfg!`); on macOS uses libSystem `_setjmp` (acceptable per architecture rule).  Linux body strategy: emit `llvm.eh.sjlj.setjmp` LLVM intrinsic (lowers to inline asm, not libc).  **Cross-compile fix landed; Linux body open.** |
