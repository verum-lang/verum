...
| 🟡 `runtime.rs::get_or_declare_getaddrinfo` / `get_or_declare_freeaddrinfo` | `getaddrinfo` / `freeaddrinfo` | DNS resolver path. **Native resolver LANDED** in `core/net/dns.vr` — a pure RFC-1035 DNS-over-UDP/TCP client on the working B1d UDP stack (no FFI): query build/parse with 0xC0 pointer-decompression (depth-capped, pointer-bomb-safe), `/etc/resolv.conf` nameserver parsing via raw syscalls (`read_system_file`, no libc `fopen`), `/etc/hosts` + hardcoded RFC-6761 loopback (`localhost`/`*.localhost` → 127.0.0.1/::1) resolved **before** any network query so offline `localhost` connect works, timeout+retry, UDP→TCP truncation fallback, and A/AAAA/CNAME/MX/TXT/SRV/NS/PTR/SOA records. The `.vr` consumer surface (`addr.vr`'s `ToSocketAddrs for (&Text,Int)` / `for Text`, used by `TcpStream.connect`/`TcpListener.bind`/`UdpSocket.bind`) already routes hostname resolution through `dns.resolve` — **no libc getaddrinfo on that path**. **Remaining (Rust-crate, separate task):** the AOT `verum_tcp_connect` runtime IR (reached only via the legacy `RawTcpStream.connect` shim → `@intrinsic("tcp_connect")` in `core/intrinsics/runtime/os.vr`) still open-codes a `getaddrinfo`/`freeaddrinfo` call to resolve+connect in one step. To close: make `verum_tcp_connect` **IP-literal-only** (reuse the already-libc-free `verum_internal_inet_pton`, mirror the `inet_pton` closure) and let callers resolve hostnames at the `.vr` layer via `dns.resolve` first — deleting both `get_or_declare_getaddrinfo` and `get_or_declare_freeaddrinfo`. Interp path (`net_runtime::tcp_connect`) uses host Rust `std::net` and is unaffected by the AOT no-libc invariant. |
